...
'******************************************************************************************+
'******************************************************************************************+
MustInherit Public Class clsPermisos
    ...
    Public Shared Function carregaOrganigramaFills(ByVal objConn As OleDbConnection, ByVal relPare As Integer) As String
        ...
        Dim ds As New DataSet()
        Dim dbrow As DataRow
        GAIA.bdR(objConn, "select DISTINCT NODINNOD, RELINCOD FROM METLNOD WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE RELCDRSU=" & relPare & "  AND RELINFIL=NODINNOD AND RELCDSIT<98 AND NODCDTIP=12", ds)
        For Each dbrow In ds.Tables(0).Rows
    ...
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblNode.Text = GAIA.descNode(objconn, Request("nodes"))
        If Not Page.IsPostBack Then
            ...
            GAIA.generaArbre(objconn, RadTree1, 9, idUsuari, relbuida, Request("arbre1"), 0, 3)
        End If
        GAIA.bdFi(objconn)
    End Sub 'Page_Load
    ...
    Private Sub generaArbre(ByVal objConn As OleDbConnection)
        ...
        For Each dbRow In DS.Tables(0).Rows
            ...
            node = GAIA.creaNodePantalla(objConn, dbRow("NODDSTXT"), dbRow("NODINNOD").ToString() & "-" & dbRow("NODCDTIP").ToString() & "_" & dbRow("RELINCOD"), "arbre", "", 0, idUsuari, rel, 0, 0)
    ...
    Private Sub RadTree1_NodeExpand(ByVal o As Object, ByVal e As Telerik.WebControls.RadTreeNodeEventArgs) Handles RadTree1.NodeExpand
        ...
        If (e.NodeClicked.Nodes.Count = 0) Then
            GAIA.afegeixNodesFillsPantallaLlista(objconn, nodo, e.NodeClicked, arbre, 0, 1, 0, 0, 1)
        End If
        GAIA.bdFi(objconn)
    End Sub
    ...
    Protected Sub clickCercarNode(ByVal sender As Object, ByVal e As EventArgs)
        ...
        GAIA.cercarNodeByTxt(objconn, RadTree1, cerca.Text, GAIA.llegirCodiArbrePantalla(objconn, RadTree1.Nodes.Item(0)), idUsuari, 1, GAIA.obtenirPathRelacioPantalla(objconn, selNode) + "_" + GAIA.obtenirNodePantalla(objconn, selNode))
        GAIA.bdFi(objconn)
...
'            clsPermisos.tractaPermisosHeretats(objconn, 0, Request("nodes"), llistaPersonesAmbCanviPermisDirecte, "", 1)
    ...
    Protected Sub clickAfegirPersones(ByVal sender As Object, ByVal e As EventArgs)
        ...
        For Each treenode In RadTree1.SelectedNodes
            nodo = GAIA.descNode(objconn, treenode.Value.Substring(0, treenode.Value.IndexOf("-")))
...
Public Shared Sub assignarPermisos2(ByVal objConn As OleDbConnection, ByVal permisosIni As Integer, ByVal numNodeOrganigrama As Integer, ByVal codiNode As Integer, ByVal recursiu As Boolean)
        ...
        GAIA.bdR(objConn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELINFIL= " & codiNode & " AND RELCDSIT<95", ds)
        For Each dbrow In ds.Tables(0).Rows
            rel.bdget(objConn, dbrow("RELINCOD"))
        ...
        If recursiu Then
            ...
            GAIA.bdR(objConn, "SELECT RELINFIL FROM METLREL WITH(NOLOCK) WHERE RELCDSIT<95 AND RELCDRSU=" & rel.incod, ds)
            For Each dbrow In ds.Tables(0).Rows
    ...
    Public Shared Function tePermisEdicio(ByVal objConn As OleDbConnection, ByVal numNodeOrganigrama As Integer, ByVal node As Integer, ByRef strPermisos As String) As Integer
        ...
        GAIA.bdR(objConn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELINFIL=" + node.ToString() + " and RELCDSIT<98", DS)
...
'	GAIA.bdSR(objconn, "INSERT INTO METLMAP2 (MAPINNOD, MAPCDREL, MAPDSLPE, MAPDTTIM, MAPCDERR) VALUES (" & node & ",0,'" & llistaPersones & "', GETDATE(),0)")
    ...
    Public Shared Sub mantenimentPermisosHeretats()
        ...
        DS = New DataSet()     
        objconn = GAIA.bdIni()
		gaia.bdr(objconn, "SELECT * FROM METLMAP2 WITH(NOLOCK) WHERE MAPCDERR=0 ORDER BY MAPINCOD ASC", DS)
		For Each dbRow In DS.Tables(0).Rows
            clsPermisos.tractaPermisosHeretats2(objconn, Nothing, dbRow("MAPINNOD"), dbRow("MAPDSLPE"), 1, 0, True)
            GAIA.bdSR(objconn, "UPDATE METLMAP2 SET MAPCDERR=2 WHERE  MAPINNOD=" & dbRow("MAPINNOD"))
		Next dbRow
		...
		Gaia.bdr(objconn, "UPDATE METLPER2 SET PERCDPE9=0,PERCDPE10=1 WHERE PERINORG=115969 AND PERCDREL IN (select RELINCOD FROM METLREL WITH(NOLOCK),METLPER2 WITH(NOLOCK) WHERE RELCDRSU=198336 AND PERCDREL=RELINCOD AND RELCDSIT<98 AND PERINORG=115969 AND PERCDPE10=0)", DS)
    ...
    Public Shared Function GetGroupsAD(ByVal usuariXarxa As String, ByVal pwd As String) As String
        ...
        Try
            ...
            Dim oResultSearch As SearchResult = oDSearcher.FindOne()
...
'Faig el canvi al nou sistema de permisos
    ...
    Public Shared Function FillsRelacioAmbPermis(ByVal objConn As OleDbConnection, ByVal permis As Integer, ByVal numNodeOrganigramaini As Integer, ByVal usuariGAIA As Integer, ByVal relPare As clsRelacio, ByRef heretat As Integer, ByVal usuariXarxa As String, ByRef grupsAD As String) As String
        ...
        GAIA.bdR(objConn, "select RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELCDRSU=" & relPare.incod & " AND RELCDSIT<98", DS)
        For Each dbRow In DS.Tables(0).Rows
    ...
    Public Shared Function completarLlistaAmbParesOrganigrama(ByVal objconn As OleDb.OleDbConnection, ByVal usuariGaia As String, ByVal posaTots As Boolean) As String
        ...
        GAIA.bdr(Nothing, "SELECT RELINPAR FROM METLREL WITH(NOLOCK) WHERE RELINFIL=" & usuariGaia & " AND RELCDHER LIKE '_56261%' AND RELCDSIT<98 ", DS)
...
#Region "Permisos 2"
    ...
    Public Shared Sub gravaPermis2(ByVal objConn As OleDbConnection, ByVal numNodeOrganigrama As Integer, ByVal rel As clsRelacio, ByVal heretat As Boolean, ByVal permisNou1 As Integer, ByVal permisNou2 As Integer, ByVal permisNou3 As Integer, ByVal permisNou4 As Integer, ByVal permisNou5 As Integer, ByVal permisNou6 As Integer, ByVal permisNou7 As Integer, ByVal permisNou8 As Integer, ByVal permisNou9 As Integer, ByVal permisNou10 As Integer, ByVal tractaPermisosEndiferit As Boolean, Optional ByVal posarPermisosAscendents As Boolean = True)
        If rel.incod <> 0 Then
            ...
            GAIA.bdr(objConn, "(SELECT RELINPAR FROM METLREL WHERE RELINFIL=" & numNodeOrganigrama & "  and RELCDHER like '_56261_115969%' )", DS2)
            ...
            '********************************************************************************************************************
            GAIA.bdr(objConn, "SELECT  * FROM METLPER2 WITH(NOLOCK) WHERE PERINORG=" & numNodeOrganigrama & " AND PERCDREL=" & rel.incod, DS)
    ...
    Public Shared Sub tractaPermisosHeretats2(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal node As Integer, ByVal llistaPersonesInicial As String, ByVal tractarFillsORG As Integer, ByVal tractarPermisosEnDiferit As Boolean, Optional ByVal posarPermisosAscendents As Boolean = False)
        ...
        If tractarPermisosEnDiferit Then
            GAIA.bdSR(objconn, "  INSERT INTO METLMAP2 (MAPINNOD, MAPCDREL, MAPDSLPE, MAPDTTIM, MAPCDERR)  VALUES (" & rel.infil & "," & rel.incod & ",'" & llistaPersonesInicial & "', GETDATE(),0)")
        Else
            Dim strsql As String = ""
            ...
            If IsNothing(rel) Then
                ...
                GAIA.bdr(objconn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELCDSIT<99 AND RELINFIL=" & node, DS)
                For Each dbRow In DS.Tables(0).Rows
    ...
    Private Shared Sub assignaPermisosHeretats2(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal llistaPersones As String, ByVal tractarFillsORG As Integer)
       ...
       If Not String.IsNullOrEmpty(llistaPersones) Then
        ...
        GAIA.bdr(objconn, "SELECT * FROM METLREL WITH(NOLOCK) WHERE RELCDRSU=" & rel.incod & " AND (RELCDSIT<96  OR RELCDSIT =98)", DS)
        ...
        If Not String.IsNullOrEmpty(llistaPendents) Then
            ...
            If usuariXarxa = "" Then
                If usuariGAIA = 0 Or usuariGAIA = cstUsuariTots Then 				
                    ...
                    If Not pendentsSonNodes Then
                        GAIA.bdr(Nothing, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELINCOD IN (" & llistaPendents & ") AND (RELCDARB<>8 AND NOT RELCDHER LIKE '_57135%')", DS)
                        For Each dbRow In DS.Tables(0).Rows
                    ...
                    Else 'llistapendents són nodes
                        ...
                        GAIA.bdr(Nothing, "SELECT RELINCOD, RELINFIL, RELCDHER FROM METLREL WITH(NOLOCK) WHERE RELINFIL IN (" & llistaPendents & ") AND (RELCDARB=8 OR RELCDHER LIKE '_57135%')", DS)
            ...
            Else
                If usuariGAIA <> cstUsuariTots and usuariGAIA<>0 Then
                    'afegeixo l'usuari GAIA si hi ha
                     GAIA.bdr(objConn, "SELECT FORDSWIN FROM METLFOR WITH(NOLOCK) WHERE FORINNOD=" & usuariGAIA & " AND FORINNOD IN (SELECT RELINFIL FROM METLREL WITH(NOLOCK) WHERE RELCDHER LIKE '_56261_115969%' AND RELCDSIT<98)", DS)
                    If DS.Tables(0).Rows.Count > 0 Then
            ...
            If Not String.IsNullOrEmpty(llistaPendents) Then
                ...
                If usuariGAIA > 0 Then
                    ...
                    'Busco els nodes organigrama superiors per si hi ha un permís directe a nivell de grup
                    GAIA.bdr(objConn, "SELECT RELINPAR FROM METLREL WITH(NOLOCK) WHERE RELINFIL=" & usuariGAIA & " AND RELCDHER LIKE '_56261%' AND RELCDSIT<98 ", DS)
                ...
                GAIA.bdr(objConn, "select  * FROM METLPER2 WITH(NOLOCK) WHERE PERINORG IN (" & llistaUsuarisGaia & ") AND " & strSqlCamp & " IN (" & llistaPendents & ") " & strOrderBy, DS)
    ...
    Public Shared Sub afegeixNodesFillsPantallaLlista2(ByVal objConn As OleDbConnection, ByVal codiRelacio As String, ByRef nodePare As RadTreeNode, ByVal codiArbre As Integer, ByVal recursiu As Integer, ByVal nivells As Integer, ByVal hsNodes As Hashtable, ByVal ds As DataSet, ByVal idUsuari As Integer, ByVal veureTots As Integer, ByVal nroMaxim As Integer)
        ...
        If hsNodes.Contains(codiRelacio) Then
            For Each relaciofill In hsNodes(codiRelacio).ToString().Split(",")  'he d'agregar tots els fills al node pare
                ...
                node = GAIA.creaNodePantalla(objConn, dbrowTmp("NODDSTXT"), dbrowTmp("NODINNOD").ToString() & "-" & dbrowTmp("RELCDARB").ToString() & "_" & dbrowTmp("RELINCOD"), rel.tipdsdes, "", 0, idUsuari, rel, 0, 150)
                If recursiu = 1 Then
                    GAIA.afegeixNodesFillsPantallaLlista2(objConn, relaciofill, node, dbrowTmp("RELCDARB"), recursiu, nivells - 1, hsNodes, ds, idUsuari, veureTots, nroMaxim)
                    node.Expanded = False
...
'************************** Funcions per generar arbres de codificacio en XML ***********************************
    Public shared Sub generarArbresCodificacio()
		...
		GAIA.generaArbreXml(objConn, 12365, "47")
		'directori
		GAIA.generaArbreXml(objConn, 50349, "47")
		'tramits
		GAIA.generaArbreXml(objConn, 69762, "47")		
		'Agenda
		GAIA.generaArbreXml(objConn, 12391, "47")		
		GAIA.generaArbreXml(objConn, 12392, "47")		
...
'****************************************************************************************************************
	 ...
	 Public Shared FUNCTION creaNodePantalla(byVal objConn as OleDbConnection, text AS String, valor AS  String, imatge AS String, nomImatge as string, fills as integer, byVal idUsuari as integer, byVal rel as clsRelacio, byVal pasDeCircuit as string, byVal nroMaxim as integer)  AS RadTreeNode		
		...
		IF limit > 55 THEN
			limit = 55			
		END IF	
		text = GAIA.tallaText(text, limit)
		...
		IF imatge.length>0 THEN
			SELECT CASE imatge.Trim
				...
				CASE "node codificacio"
					...
					GAIA.bdr(objConn, "select top 1 NCOSWCOD FROM METLNCO WITH(NOLOCK) WHERE NCOINIDI=1 AND NCOINNOD=" + rel.infil.ToString(), DS)
                    If DS.Tables(0).Rows.Count > 0 Then
...
'************************************************************************************************************
 ...
 Public Shared function bdSR(byRef objConn as oleDbConnection, byVal strSql as String, byVal dataSource as string, byVal bd as string) as integer
	    ...
	    Catch ex As SqlException
            ...
            GAIA.f_logError(objConn, "Base de dades", "BDSR", "error nro: " & ex.ErrorCode & " / " & ex.Message & "/" & strSql)
...
'************************************************************************************************************
	 ...
	 End FUNCTION
  	Public Shared FUNCTION bdSR_id(byRef objConn as oleDbConnection, byVal strSql as String, byVal dataSource as string, byVal bd as string) as integer 
       ...
       Catch ex As SqlException
            GAIA.f_logError(objConn, "Base de dades", "BDSR_ID.",  "error nro: " & ex.ErrorCode & " / " & ex.Message & "/" & strSql)				
        End Try
 ...
 Public Shared function bdR(byRef objConn as oleDbConnection, byVal strSql as String,byRef DS As DataSet, byVal dataSource as string, byVal bd as string) as integer
		...
		END IF
           ...
           ' objConn.ConnectionString = "Provider=SQLOLEDB;Data Source=" + dataSource + ";Initial Catalog=" + bd + "; pwd=secopc; user id=CPDsa"
            ...
            Catch ex As SqlException
                GAIA.f_logError(objConn, "Base de dades", "BDR", Err.Description + ".." + strSql)
				resultat = ex.Number
            End Try
...
#If RealExec Then
        ...
        GAIA.bdSR(objConn, "DELETE  FROM CSTLRAC WHERE RACINCOD=" & codiNode)
        GAIA.bdSR(objConn, "DELETE  FROM CSTLACC WHERE ACCINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE  FROM METLNOT WHERE NOTINNOD=" & codiNode)
#End If
    End Sub
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE  FROM METLLNK WHERE LNKINNOD=" & codiNode)
#End If
    End Sub
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLWEB WHERE WEBINNOD=" & codiNode)
#End If
    End Sub
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLPLT WHERE PLTINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLLCW WHERE LCWINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLFCI WHERE FCIINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLNOR WHERE NORINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLFOR WHERE FORINNOD=" & codiNode)
#End If
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLMIS WHERE MISINNOD=" & codiNode)
#End If
    End Sub 'fullaOrganigrama
...
#If RealExec Then
        GAIA.bdSR(objConn, "DELETE FROM METLNES WHERE NESINNOD=" & codiNode)
#End If
...
#If RealExec Then
        ...
        DS = New DataSet()
        GAIA.bdr(objConn, "SELECT DOCWNBOR, DOCDSFIT, isnull(RELDSFIT, '') as RELDSFIT, RELCDHER FROM METLREL WITH(NOLOCK),METLDOC WITH(NOLOCK) WHERE RELINCOD=" & rel.incod & "  AND DOCINNOD=RELINFIL ", DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
            ...
            If path.Length > 0 Then
                ...
                port = ""
                URL = ""
                GAIA.trobaServidorDesti(objConn, rel, servidor, usuariFTP, pwdFTP, pathDestiArrel, pathDocsDesti, port, URL)
                GAIA.ftpEsborrarFitxer(objConn, servidor, usuariFTP, pwdFTP, pathDocsDesti & "/" & fitxer, codiUsuari, rel, dbRow("DOCWNBOR"))
...
#If RealExec Then
        ...
        insertarNode = GAIA.bdSR_id(objConn, "INSERT INTO METLNOD (NODCDTIP,NODDSTXT,NODCDUSR,NODDTTIM,NODSWSIT,NODDTPUB,NODDTCAD) VALUES (" & tipusNode.ToString() & ",'" & GAIA.netejaHTML(nomNode.Replace("'", "''")) & "','" & codiOrg & "','" & noddttim & "'," & nodswsit & ",'" & noddtpub & "','" & noddtcad & "')")
#End If
    End Function
    ...
    Public Shared SUB moureNode(byVal objConn as OleDbConnection, byRef relOrigen as clsRelacio, byVal relDesti as clsRelacio, byVal moureFills as Integer, byVal posicioEstructura as integer, byVal tipusMoviment as integer, byVal relDestiInicial as clsRelacio, byVal tipusOrdre as integer)
        ...
        GAIA.modificaRelacio(objConn, relOrigen, nodePathNou, nroArbreDesti, nroNodeDesti, 0, posicioEstructura, -1, 1)
...
'Fa una copia d'un contingut i retorna la nova relació creada
    Public Shared Function ferCopiaContingut(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiUsuari As Integer, Optional ByRef prefix As String = "", Optional ByVal reldesti As clsRelacio = Nothing) As Integer
        ...
        '1
        codiNode = GAIA.insertarNode(objConn, rel.tipintip, prefix + rel.noddstxt, codiUsuari)
        ...
        Select Case UCase(rel.tipdsdes.Trim())
            Case "NODE WEB"
                ...
                If GAIA.esGAIA2(objConn, rel.infil) Then
                    GAIA.bdr(objConn, "SELECT NWEINNOD, NWEINIDI, isNull(NWEDSTIT,'') as NWEDSTIT, NWEDSUSR, NWEDSTCO,NWEDSPLA, NWEDSEST, NWEDSHTM, NWEDSLCW, NWEDSCAR, isnull(NWEDSCSS,'') as NWEDSCSS,NWEDSEBO,isnull(NWEDSMET,'') as NWEDSMET,isnull(NWEDSPEU,'') as NWEDSPEU, isnull(NWEDSCSP,'') as NWEDSCSP, isnull(NWEDSCSI,'') as NWEDSCSI FROM METLNWE2  WITH(NOLOCK) WHERE NWEINNOD=" & rel.infil, DS)
                    For Each dbRow In DS.Tables(0).Rows
            ...
            Case "FULLA DOCUMENT"
                ...
                For Each dbRow In DS.Tables(0).Rows
                    ...
                    If docAnt <> docact Then
                        ...
                        'Si es una imagen copiamos las de todos los tamaños
                        If dbRow("DOCINTDO").ToString() = 16 Then
                            'If InStr(nomVell, ".jpg") > 0 Then
                            If File.Exists(savePath & nomVell.Replace(".jpg", "P.jpg")) Then
                                File.Copy(savePath & nomVell.Replace(".jpg", "P.jpg"), savePath & nomDocument.Replace(".jpg", "P.jpg"))
                            End If
                            If File.Exists(savePath & nomVell.Replace(".jpg", "P100.jpg")) Then
                                File.Copy(savePath & nomVell.Replace(".jpg", "P100.jpg"), savePath & nomDocument.Replace(".jpg", "P100.jpg"))
                            End If
                            If File.Exists(savePath & nomVell.Replace(".jpg", "P1366.jpg")) Then
                                File.Copy(savePath & nomVell.Replace(".jpg", "P1366.jpg"), savePath & nomDocument.Replace(".jpg", "P1366.jpg"))
                            End If
                        End If
                ...
                For Each dbRow In DS.Tables(0).Rows
                    Try
						...
						GAIA.bdSR(objConn, "INSERT INTO METLREI VALUES (" & novaRel.incod & "," & dbRow("REIINIDI") & ",'" & CDate(dbRow("REIDTPUB")) & "','" & CDate(dbRow("REIDTCAD")) & "','/gdocs/" & hDocs.Item(nomVell) & "','" & hDocs.Item(nomVell) & "','" & dbRow("REIDSHAS") & "')")
                    Catch ex As Exception
            ...
            Case "FULLA PLANTILLAWEB"
				'Teresa: añado la posibilidad de GAIA2
                If GAIA.esGAIA2(objConn, rel.infil) Then
                    GAIA.bdr(objConn, "SELECT * FROM METLPLT2  WITH(NOLOCK) WHERE PLTINNOD=" & rel.infil, DS)
                    If DS.Tables(0).Rows.Count > 0 Then
                        dbRow = DS.Tables(0).Rows(0)
            ...
            Case "FULLA WEB"
				'Teresa: añado la posibilidad de GAIA2
                If GAIA.esGAIA2(objConn, rel.infil) Then
                    GAIA.bdr(objConn, "SELECT * FROM METLWEB2  WITH(NOLOCK) WHERE WEBINNOD=" & rel.infil, DS)
                    For Each dbRow In DS.Tables(0).Rows
    ...
    Public Shared Sub corregirErrorsEstructura(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal idioma As Integer, ByVal posicioEstructura As Integer)
        ...
        GAIA.trobaFills(objConn, rel, rel.infil, nodesFills, nroFills, relacions, 0, 0)
        For Each item In relacions
            If item <> 0 Then
                reltmp.bdget(objConn, item)

                posicioEstructuraNova = GAIA.assignaAutomaticamentCella(objConn, reltmp.tipintip, rel, 0, idioma, posicioAL, posicioEstructura)
                ...
                End If
                If sql <> "" Then
                    GAIA.bdSR(objConn, "UPDATE METLREL SET " + sql + " WHERE RELINCOD=" + item.ToString())
        ...
        posicioEstructuraNova = GAIA.assignaAutomaticamentCella(objConn, tipusNode, relSuperior, 0, idioma, posicioAL, posicioEstructura)
        If posicioEstructuraNova >= 0 Then
            sql = " RELCDEST=" + posicioEstructuraNova.ToString()
        End If
        If posicioAL >= 0 Then
        ...
        End If
        If sql <> "" Then
            GAIA.bdSR(objConn, "UPDATE METLREL SET " + sql + " WHERE RELINCOD=" + codiRElacio.ToString())
        End If
        ...
        For Each dbRow In DS.Tables(0).Rows
            Select Case tipusOrdre
                Case 1 'poso rel el primer de la llista
                    ...
                    Else
                        If cont = 1 Then
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=1 WHERE RELINCOD=" + rel.incod.ToString())
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=2 WHERE RELINCOD=" + dbRow("RELINCOD").ToString())
                            cont = 3
                        Else
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=" + cont.ToString() + " WHERE RELINCOD=" + dbRow("RELINCOD").ToString())
                            cont = cont + 1
                        End If
                    End If
                ...
                Case 2 'possició seleccionada
                    ...
                    Else
                        If dbRow("RELINCOD") = relDestiInicial.incod Then
                            ...
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=" + cont.ToString() + " WHERE RELINCOD=" + dbRow("RELINCOD").ToString())
                            cont = cont + 1
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=" + cont.ToString() + " WHERE RELINCOD=" + rel.incod.ToString())
                            cont = cont + 1
                        Else
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDORD=" + cont.ToString() + " WHERE RELINCOD=" + dbRow("RELINCOD").ToString())
                            cont = cont + 1
 ...
 Public Shared Function creaRelacio(ByVal objConn As OleDbConnection, ByVal arbre As Integer, ByVal pare As Integer, ByVal fill As Integer, ByVal versio As Integer, ByVal herencia As String, ByVal posicioEstructura As Integer, ByVal situacioRelacio As Integer, ByVal ordre As Integer, ByVal contingutEsVisibleAInternet As Integer) As clsRelacio
	...
	Public Shared Function creaRelacio(ByVal objConn As OleDbConnection, ByVal arbre As Integer, ByVal pare As Integer, ByVal fill As Integer, ByVal versio As Integer, ByVal herencia As String, ByVal posicioEstructura As Integer, ByVal situacioRelacio As Integer, ByVal ordre As Integer, ByVal contingutEsVisibleAInternet As Integer, byVal tractarPermisosEnDiferit as boolean, byVal usuari as integer) As clsRelacio	
        ...
        If DS.Tables(0).Rows.Count > 0 Then
            ...
            rel.bdget(objConn, dbRow("RELINCOD"))
            relSuperior = GAIA.obtenirRelacioSuperior(objConn, rel)
            ...
            If sqlUpdate.Length > 0 Then
                GAIA.bdSR(objConn, "UPDATE METLREL SET " & sqlUpdate & " WHERE RELINCOD=" & rel.incod)
            End If
        ...
        Else ' no existeix, vaig a buscar l'ordre
            ...
            'Inserto
            codiRElacio = GAIA.bdSR_id(objConn, "INSERT INTO METLREL (RELCDARB, RELCDVER,RELINPAR, RELINFIL, RELCDHER,RELCDEST,RELCDORD,RELCDSIT, RELSWVIS) VALUES (" + arbre.ToString() + "," + versio.ToString() + "," + pare.ToString() + "," + fill.ToString() + ",'" + herencia.ToString() + "'," + posicioEstructura.ToString() & "," & ordre & "," & situacioRelacio & "," & contingutEsVisibleAInternet & ")")
            ...
            If contingutEsVisibleAInternet = 0 Then
                ...
                GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDRSU=" & relSuperior.incod & " WHERE RELINCOD=" & rel.incod)
            Else
                If relSuperior.swvis = 1 Then
                    GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDRSU=" & relSuperior.incod & ", RELSWVIS=" & relSuperior.swvis & " WHERE RELINCOD=" & rel.incod)
                Else
                    GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDRSU=" & relSuperior.incod & " WHERE RELINCOD=" & rel.incod)
                End If
            End If
    ...
    Public Shared Function cercarNodeByTxt(ByVal objConn As OleDbConnection, ByVal radtree1 As RadTreeView, ByVal text As String, ByVal codiArbre As Integer, ByVal idusuari As Integer, ByVal nroArbre As Integer, ByVal herencia As String) As String
        ...
        For Each rel In rels
            ...
            GAIA.buscaParesRel(objConn, rel, nodosPerObrir)
        Next rel
        ...
        For i = 0 To (nodosPerObrir.Length - 1)
            ...
            For Each treenode In GAIA.buscaNodeArbrePantalla(objConn, radtree1, nodosPerObrir(i))
                If (Not treenode Is Nothing) Then
                    If treenode.Nodes.Count = 0 Then

                        GAIA.afegeixNodesFillsPantallaLlista(objConn, nodosPerObrir(i), treenode, GAIA.llegirCodiArbrePantalla(objConn, treenode), idusuari, nroArbre, 0, 0, 1)
                    End If
        ...
        For Each nodo In nodos
            For Each treenode In GAIA.buscaNodeArbrePantalla(objConn, radtree1, nodo)
    ...
    Public Shared Function insertaNodeArbrePersonal(ByVal objConn As OleDbConnection, ByVal tipusFulla As Integer, ByRef codiFulla As Integer, ByVal codiOrg As String, ByVal nomCarpetaBasica As String) As clsRelacio
        ...
        '
        Dim codiArbre As Integer = GAIA.tipusNodeByTxt(objConn, "arbre elMeuEspai")
        Dim tipusNodeElMeuEspai As Integer = GAIA.tipusNodeByTxt(objConn, "node elMeuEspai")
        ...
        Dim dbRow As DataRow
        DS = New DataSet()
        nomUsuari = GAIA.obtenirNomNode(objConn, codiOrg)
        ...
        If codiNodeElMeuEspai = 0 Then        'no he trobat el node i el creo				
            ...
            GAIA.bdSR(objConn, "INSERT INTO METLNES (NESINNOD,NESINIDI,NESDSTIT, NESDSUSR,NESCDORG) VALUES (" + codiNodeElMeuEspai.ToString() + ",1, 'El meu espai de " + nomUsuari + "',0," + codiOrg.ToString() + ")")
        ...
        If codiNodeSenseClassificar = 0 Then
            ...
            GAIA.bdSR(objConn, "INSERT INTO METLNES (NESINNOD,NESINIDI,NESDSTIT, NESDSUSR,NESCDORG) VALUES (" + codiNodeSenseClassificar.ToString() + ",1,'Sense classificar','0'," + codiOrg.ToString() + ")")
        End If
        ...
        If codiFulla <> 0 And tipusFulla <> 13 Then 'una fulla organigrama no la posaré al sense classificar del creador
            ...
            End If
            GAIA.log(objConn, rel, codiOrg, "", TAINSERTAR)
        Else
            GAIA.log(objConn, rel, codiOrg, "Creació de l'usuari: " + nomUsuari, TAINSERTAR)
        End If
        insertaNodeArbrePersonal = rel
    ...
    Public Shared Sub UsuariGaia(ByVal objconn As OleDbConnection, ByVal usuari As String, ByRef nif As String, ByRef codiOrg As String, ByRef nom As String, Optional ByVal crearUsuari As Boolean = False)
        ...
        DS = New DataSet()
        GAIA.bdr(objconn, "SELECT FORDSNIF, FORDSTIT, FORINNOD FROM METLFOR WITH(NOLOCK) ,METLREL WITH(NOLOCK)  WHERE FORDSWIN like '" & usuari & "' AND FORINIDI=1 AND RELCDSIT<98 AND FORINNOD=RELINFIL", DS)
        ...
        If nom = "" And crearUsuari Then
            ...
            Try
                ...
                If nom <> "" Then
                    ...
                    tipusNode = GAIA.tipusNodeByTxt(objconn, "fulla organigrama")
                    'Inserto el node "fulla organigrama " amd codiUsuari=9999 per que és un procés automàtic
                    Dim codinode As Integer = GAIA.insertarNode(objconn, tipusNode, nom, 9999)
                    codiOrg = codinode
                    'Inserto el node fulla organigrama en l'arbre personal de l'usuari	
                    GAIA.insertaNodeArbrePersonal(objconn, tipusNode, codinode, codinode, "")


                    'Creo la fulla  "fulla organigrama"
                    GAIA.bdSR(objconn, "INSERT INTO METLFOR (FORINNOD,FORINIDI,FORDSTIT, FORDSUSR,FORDSCOD, FORDSNIF, FORDSWIN) VALUES (" & codinode & ",1,'" & nom.Replace("''", "'") & "','9999','0','" & nif & "','" & usuari & "')")
    ...
    Public Shared Sub escriuResultat(ByVal objConn As OleDbConnection, ByRef lbl As Label, ByVal text As String, ByVal enllaç As String)
        GAIA.escriuResultat(objConn, lbl, text, enllaç, 1)
    ...
    Public Shared Sub escriuResultat(ByVal objConn As OleDbConnection, ByRef lbl As Label, ByVal text As String, ByVal enllaç As String, ByVal esborraResultatAnterior As Integer)
        ...
        If GAIA.llistatWarnings.Length > 0 Then
            lbl.Text &= "missatgeAvis"
        Else
            If GAIA.llistatErrors.Length > 0 Then
    ...
    Public Shared Sub modificaRelacio(ByVal objConn As OleDbConnection, ByRef relOrigen As clsRelacio, ByVal nodePathNou As String, ByVal codiArbreNou As Integer, ByVal nodePareNou As Integer, ByVal nodeFillNou As Integer, ByVal posicioEstructuraNova As Integer, ByVal estatRelacioNova As Integer, ByVal moureFills As Integer)
        ...
        If relOrigen.cdher <> nodePathNou Or Not moureFills Then
            ...
            If moureFills = 1 Then
                ...
                For Each dbRow In DS.Tables(0).Rows
                    ...
                    GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDARB=" & codiArbreNou & ",RELCDHER='" & nodePath & "',RELSWVIS=" & contingutEsVisibleAInternet & " WHERE RELINCOD=" & dbRow("RELINCOD"))
                    If relDesti.cdsit < 95 Then
                        GAIA.afegeixAccioManteniment(objConn, relDesti, 0, 99,Now, Now, relDesti, 1, posicioEstructuraNova,true)
            ...
            Else
                ...
                For Each dbRow In DS.Tables(0).Rows
                    ...
                    GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDHER='" & nodePath & "', RELINPAR=" & noupare & ", RELCDRSU = " & relOrigen.incod & ", RELSWVIS=" & contingutEsVisibleAInternet & " WHERE RELINCOD=" & dbRow("RELINCOD"))
                    relDesti.bdget(objConn, dbRow("RELINCOD"))
                    If Not (dbRow("RELCDSIT") = 96 Or dbRow("RELCDSIT") = 97 Or dbRow("RELCDSIT") = 95) Then
                        est = plantillaPerDefecte(objConn, relDesti, 1)
                        relSuperior = GAIA.obtenirRelacioSuperior(objConn, relDesti)
                        est = assignaAutomaticamentCella(objConn, relDesti.tipintip, relSuperior, 1, 1, 0, 0)
                        If est <> relDesti.cdest Then
                            GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDEST=" + est.ToString() + " WHERE RELINCOD=" + relDesti.incod.ToString())
                            relDesti.bdget(objConn, relDesti.incod)
                        End If
                    Else

                        GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDEST=-1 WHERE RELINCOD=" + relDesti.incod.ToString())
                    End If
                    If relDesti.cdsit < 95 Then
                        GAIA.afegeixAccioManteniment(objConn, relDesti, 0, 99, "", "", relDesti, 0, posicioEstructuraNova)
                    End If
                Next dbRow
            ...
            relSuperior = GAIA.obtenirRelacio(objConn, nodePareNou, nodePathNou.Substring(0, InStrRev(nodePathNou, "_" & nodePareNou) - 1))
            ...
            If relOrigen.cdsit < 95 Then
                ...
                GAIA.afegeixAccioManteniment(objConn, relOrigen, 0, 99, Now, Now, relOrigen, 1, posicioEstructuraNova, True)
                If relOrigen.cdrsu <> relSuperior.incod Then
                    GAIA.afegeixAccioManteniment(objConn, relSuperior, 0, 99, Now, Now, relSuperior, 1, posicioEstructuraNova, True)
                End If
		...
		if estatRelacioNova = 98 or estatRelacioNova=99 THEN
			GAIA.esborrarCelles(nothing,"CELCDNOD=" & relDesti.infil)
	...
	'   29-1-2019 modificada para GAIA2
    '************************************************************************************************************
 Public Shared Function existeixContingut(ByVal objConn As OleDbConnection, ByVal codiNode As Integer, ByVal codiRelacio As Integer, ByVal codiIdioma As Integer) As Integer
    ...
    Public Shared Function heretarPropietatsWeb(ByVal objConn As OleDbConnection, ByVal codiRelacio As Integer, ByRef width As Integer) As Integer
        ...
        DS = New DataSet()
		'Teresa: incluyo GAIA2
        GAIA.bdr(objConn, "SELECT WEBTPHER,WEBWNMTH FROM METLWEB WITH(NOLOCK) , METLREL WITH(NOLOCK)  WHERE RELINFIL=WEBINNOD AND RELINCOD=" + codiRelacio.ToString() & " UNION SELECT WEBTPHER,WEBWNMTH FROM METLWEB2 WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE RELINFIL=WEBINNOD AND RELINCOD=" & codiRelacio.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Function heretarPropietatsWeb(ByVal objConn As OleDbConnection, ByVal codiRelacio As Integer) As Integer
        ...
        Return (GAIA.heretarPropietatsWeb(objConn, codiRelacio, width))
    ...
    Public Shared Sub debug(ByVal objConn As OleDbConnection, ByVal texto As String)
        GAIA.bdSR(objConn, "INSERT INTO METLDEB (TEXT,data) VALUES ('" + texto.Replace("'", "''") + "','" + Now + "')")
    End Sub 'debug			
                    ...
                    If (arrayPlt.Length > 0) Then
                        If (arrayPlt(orden).Trim.Length > 0) Then
                            ...
                            For Each item In PlantillesPerCelda
                                ...
                                selectPlantilles += GAIA.obtenirNomPlantilla(objConn, item)
                            Next item
                        End If
                    End If
                End If
    ...
    'N
    '************************************************************************************************************
    '	Funció: GAIA.maquetarHTML
    '	
    '	Procés: 
    '					Recorre la estructura de la página a publicar y busca el contenido asociado a cada celda.
    '					Si es desitja heretar les propietats dels nodes superiors recorrerà els nodes desde el node arbre cap avall, anidando las correspondientes estructuras.
    '                   Devuelve el string html a representar
    ...
    Public Shared Function maquetarHTML(ByVal objConn As OleDbConnection, ByVal estructura As String, ByVal rel As clsRelacio, ByRef fDesti As String, ByRef urlDesti As String, ByRef nrocontingut As Integer, ByRef llistaDocuments As String(), ByRef publicar As Integer, ByVal idioma As Integer, ByVal relIni As clsRelacio, ByVal dataSimulacio As DateTime, ByVal codiUsuari As Integer, ByRef tagsMeta As String, ByRef titol As String, ByRef estilbody As String, ByRef esForm As String, ByRef esEML As String, ByRef esSSL As String, ByRef htmlPeu As String, ByRef strCSSPantalla As String, ByRef strCSSImpressora As String) As String
        Dim html As String = ""
        Dim arrayEstructura As String(), arrayPathRelacio As String()
        Dim cont As Integer = 1, idCel As Integer
        Dim strIdCel As String = "", htmlTmp As String = "", strsql As String, pathRelacio As String, estructuraFullaWeb As String = "", codiWeb As String = "", codiWeb2 As String = ""
        Dim heretaPropietats As String = ""
        Dim DS As DataSet, DS2 As DataSet
        Dim dbRow As DataRow, dbRowFullaWeb As DataRow
        ...
        ' Propiedades
        Dim nom As String
        Dim destiDocs As String
        Dim midaHoMaxima As String
        Dim LCW As String
        Dim LC2 As String
        Dim arrayLCW(-1) As String
        Dim arrayLC2(-1) As String
        Dim item As String
        Dim nomFulla As String = ""

        ' Busco la estructura y otras propiedades de la fulla web
        DS = New DataSet()
        GAIA.bdr(Nothing, "SELECT * FROM METLWEB2 WITH(NOLOCK) WHERE WEBINNOD=" & rel.infil & " AND WEBINIDI=" & idioma, DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRowFullaWeb = DS.Tables(0).Rows(0)
            estructuraFullaWeb = dbRowFullaWeb("WEBDSEST")
            heretaPropietats = dbRowFullaWeb("WEBTPHER")  ' S o N 

            LCW = dbRowFullaWeb("WEBDSLCW")
            arrayLCW = LCW.Split(",")
            LC2 = dbRowFullaWeb("WEBDSLC2")
            arrayLC2 = LC2.Split(",")

            ' Recogo las propiedades de la fulla web - ¿se tendrían que usar solo si no hereda propiedades?
            estilbody = dbRowFullaWeb("WEBDSEBO")
            If dbRowFullaWeb("WEBDSFIT").ToString().Length > 0 Then
                nomFulla = dbRowFullaWeb("WEBDSFIT")
                urlDesti = dbRowFullaWeb("WEBDSURL")
            Else
                nomFitxerPublicat = rel.dsfit
                nomFulla += "/" + rel.incod.ToString() + "_" + idioma.ToString() + ".aspx"
                urlDesti += "/" + rel.incod.ToString() + "_" + idioma.ToString() + ".aspx"
            End If

            esForm = dbRowFullaWeb("WEBSWFRM")
            esEML = dbRowFullaWeb("WEBSWEML") 'Es email?
			
			If esEML = "S" Then  'Teresa: para poder conservar el historico de boletines, se añade la fecha de publicacion al nombre del archivo.

                Dim pos As String = InStr(nomFulla, ".")
                nomFulla = nomFulla.Substring(0, pos - 1) & CDate(Now).ToString("yyyyMMdd") & ".aspx"

                pos = InStr(urlDesti, ".")
                urlDesti = urlDesti.Substring(0, pos - 1) & CDate(Now).ToString("yyyyMMdd") & ".aspx"

            End If
			
            esSSL = dbRowFullaWeb("WEBSWSSL") 'Es ssl?

            If dbRowFullaWeb("WEBDSDES").length > 0 Then  'Description
                tagsMeta += "<meta name=""DC.Description"" content=""" + dbRowFullaWeb("WEBDSDES") + """/>"
                tagsMeta += "<meta name=""Description"" content=""" + dbRowFullaWeb("WEBDSDES") + """/>"
            End If

            If dbRowFullaWeb("WEBDSPCL").length > 0 Then  'Paraulas clau
                tagsMeta += "<meta name=""keywords"" content=""" + dbRowFullaWeb("WEBDSPCL") + """/>"
                tagsMeta += "<meta name=""DC.subject"" content=""" + dbRowFullaWeb("WEBDSPCL") + """/>"
            End If

            If dbRowFullaWeb("WEBTPBUS") = "S" Then 'Incloure al cercador ¿siguen usandose estas etiquetas?
                tagsMeta += "<meta name=""robots"" content=""INDEX,FOLLOW""/>"
            Else
                tagsMeta += "<meta name=""robots"" content=""NOINDEX,NOFOLLOW""/>"
            End If

            Dim width As Integer = dbRowFullaWeb("WEBWNMTH") 'Mida hotitzontal máxima
            titol = dbRowFullaWeb("WEBDSTIT")
        End If

        'Recorro cada cel·la i la represento
        arrayEstructura = Split(estructuraFullaWeb, "id='d")
		...
		' Teresa: los idCel no tienen porque ser correlativos, puedo haber borrado celdas. Por ejempo que una página contenga 2 celdas que sean la d3 y d4
        
		'For Each idCel In strIdCel.Split(",")
		For i As Integer = 0 To arrayEstructura.Length - 2
            idCel = arrayEstructura(i + 1).Substring(0, arrayEstructura(i + 1).IndexOf("'"))
            '*******************************************************************
            ' POSO LES LLIBRERIES DE CODI WEB 
            '*******************************************************************                
            ' Abans del contingut
			If arrayLCW.Length > i Then
                If (arrayLCW(i).Trim().Length > 0) Then
                    For Each item In arrayLCW(i).Split("|")
                        codiWeb = GAIA.trobaCodiWeb2(objConn, plantilla, item, idioma, rel, relIni, 1, idCel, "", 0, codiUsuari, "", tagsMeta)
                    Next item
                End If
			End If
            'If arrayLCW.Length > idCel Then
                'If (arrayLCW(idCel).Trim().Length > 0) Then
                    'For Each item In arrayLCW(idCel).Split("|")
                        'codiWeb = GAIA.trobaCodiWeb2(objConn, plantilla, item, idioma, rel, relIni, 1, idCel, "", 0, codiUsuari, "", tagsMeta)
                    'Next item
                'End If
            'End If

            'LC2 Després del contingut
			If arrayLC2.Length > i Then
                If (arrayLC2(i).Trim().Length > 0) Then
                    For Each item In arrayLC2(i).Split("|")
                        codiWeb = GAIA.trobaCodiWeb2(objConn, plantilla, item, idioma, rel, relIni, 1, idCel, "", 0, codiUsuari, "", tagsMeta)
                    Next item
                End If
            End If
            'If arrayLC2.Length > idCel Then
                'If (arrayLC2(idCel).Trim().Length > 0) Then
                    'For Each item In arrayLC2(idCel).Split("|")
                        'codiWeb2 = GAIA.trobaCodiWeb2(objConn, plantilla, item, idioma, rel, relIni, 1, idCel, "", 0, codiUsuari, "", tagsMeta)
                    'Next item
                'End If
            'End If
            ...
            If DS.Tables(0).Rows.Count > 0 Then 'Tinc contingut que penja d'aquesta cel·la de l'estructura  
                For Each dbRow In DS.Tables(0).Rows
                    ...
                    If dbRow("REIDTPUB") < Now And (dbRow("REIDTCAD") > Now() Or dbRow("REIDTCAD") = "1/1/1900") Then
                        ...
                        htmlTmp = GAIA.maquetarPlantilla(Nothing, plantilla, relInf, idioma, rel, CDate(Now), 49730, llistaDocuments, urlDesti, fDesti, 0, "N", tagsMeta)

                        estructuraFullaWeb = estructuraFullaWeb.Replace("id='d" & dbRow("RELCDEST") & "'>", ">" & codiWeb & htmlTmp & codiWeb2)

                    End If
                Next dbRow
            Else ' Cuando no hay contenido pongo las LCW
                estructuraFullaWeb = estructuraFullaWeb.Replace("id='d" & idCel & "'>", ">" & codiWeb & codiWeb2)
            End If

        Next i

        If heretaPropietats = "N" Then

            fDesti = nomFulla
            Return estructuraFullaWeb

        Else ' hereda propiedades


            ' Busco los nodos superiores y los recorro
            pathRelacio = rel.cdher
            ...
            If pathRelacio.Length > 0 Then
                ...
                For cont = 0 To arrayPathRelacio.Length - 1
                    GAIA.tipusNodebyNro(objConn, arrayPathRelacio(cont), descTipus)
                    Select Case descTipus
                        Case "arbre web"
                            'Busco la relacio
                            'DS2 = New DataSet()
                            'GAIA.bdr(objConn, "SELECT RELINCOD FROM METLREL WHERE RELINPAR=" & arrayPathRelacio(cont), DS2)
                            'If DS2.Tables(0).Rows.Count > 0 Then
                                'relInf.bdget(objConn, DS2.Tables(0).Rows(0)("RELINCOD"))
                            'End If

                            'Tinc un node web, llegeixo l'estructura
                            DS2 = New DataSet()
                            GAIA.bdr(objConn, "SELECT AWEDSEST, AWEDSTIT, AWEDSNOM, AWEDSROT, AWEDSDOC, AWEDSEBO, AWEDSPEU, AWEDSCSP, AWEDSCSI, AWEDSMET, SERDSURL, SERDSPRT, AWEDSLCW FROM METLAWE2 WITH(NOLOCK), METLSER WITH(NOLOCK) WHERE AWEDSSER = SERINCOD AND AWEINNOD=" & arrayPathRelacio(cont), DS2)
                            If DS2.Tables(0).Rows.Count > 0 Then
                                ...
                                nom = dbRow("AWEDSNOM")
                                fDesti = dbRow("AWEDSROT").Trim()          ' Camí del disc
                                destiDocs = dbRow("AWEDSDOC")               ' Destí pels documents
                                estilbody += " " + dbRow("AWEDSEBO")        ' Estils body
                                strCSSPantalla += " " + dbRow("AWEDSCSP")   ' CSS per pantalla
                                strCSSImpressora += " " + dbRow("AWEDSCSI") ' CSS per impressora
                                tagsMeta += dbRow("AWEDSMET")               ' Valors META
                                htmlPeu += dbRow("AWEDSPEU")                ' HTML al peu

                                urlDesti = dbRow("SERDSURL").Trim()         ' Servidor FTP destí
                        ...
                        Case "node web"
                            ...
                            'Tinc un node web, llegeixo l'estructura
                            DS2 = New DataSet()
                            GAIA.bdr(objConn, "SELECT NWEDSEST, NWEDSTIT, NWEDSCAR, NWEDSEBO, NWEDSPEU, NWEDSCSP, NWEDSCSI, NWEDSMET, NWEDSLCW FROM METLNWE2 WHERE NWEINNOD=" & arrayPathRelacio(cont), DS2)
                            If DS2.Tables(0).Rows.Count > 0 Then
                                dbRow = DS2.Tables(0).Rows(0)

                                estructura = dbRow("NWEDSEST")

                                'Recogo las propiedades. Todavía no las uso.
                                titol = dbRow("NWEDSTIT")
                                estilbody += " " + dbRow("NWEDSEBO")            'Estils body

                                If dbRow("NWEDSCSP").Trim().length > 0 Then     'CSS per pantalla
                                ...
                                If dbRow("NWEDSCSI").Trim().length > 0 Then     'CSS per impressora
                                ...
                                tagsMeta &= dbRow("NWEDSMET")                   ' Valors META
                                ...
                                If InStr(htmlPeu, "google-analytics") > 0 And InStr(dbRow("NWEDSPEU"), "google-analytics") > 0 Then
                                    htmlPeu = dbRow("NWEDSPEU")                 'HTML al peu
                                Else
                                    htmlPeu = dbRow("NWEDSPEU") + htmlPeu
                                End If

                                'Continuo representant informació						
                                If Not TypeOf dbRow("NWEDSCAR") Is DBNull Then  'Carpeta
                        ...
                        Case "fulla web"
                            ...
                            'Node
                            DS2 = New DataSet()
                            GAIA.bdr(objConn, "SELECT WEBDSEST, WEBDSLCW, WEBDSLC2, WEBTPHER, WEBDSEBO, WEBDSFIT, WEBDSURL, WEBSWFRM, WEBSWEML, WEBSWSSL, WEBWNMTH, WEBDSPCL, WEBDSDES, WEBDSTIT, WEBTPBUS FROM METLWEB2 WHERE WEBINNOD=" & arrayPathRelacio(cont), DS2)
                            If DS2.Tables(0).Rows.Count > 0 Then
                                dbRow = DS2.Tables(0).Rows(0)
                                ...
                                ' Buscar las propiedades que se heredarán. Todavía no las uso.
                                If InStr(titol, dbRow("WEBDSTIT")) <= 0 And InStr(titol, "|") <= 0 Then
                                    titol = dbRow("WEBDSTIT") + " | " + titol
                                End If
                                estilbody += " " + dbRow("WEBDSEBO")                                                            'Estils body

                                If dbRow("WEBDSFIT").ToString().Length > 0 Then                                                 'Nom fitxer
                                    Dim strTmp As String = ""
                                    ...
                                    urlDesti = dbRow("WEBDSURL").Trim()                                                         'URL
                                Else
                                    fDesti += "/" + relIni.ToString() + "_" + idioma.ToString() + ".aspx"
                                    urlDesti += "/" + relIni.ToString() + "_" + idioma.ToString() + ".aspx"
                                End If

                                esForm = dbRow("WEBSWFRM")                                                                      'Te un formulari de servidor?
                                esEML = dbRow("WEBSWEML")                                                                       'Es un correu electróònic?
                                
								 If esEML = "S" Then  'Teresa: para poder conservar el historico de boletines, se añade la fecha de publicacion al nombre del archivo.

                                    Dim pos As String = InStr(fDesti, ".")
                                    fDesti = fDesti.Substring(0, pos - 1) & CDate(Now).ToString("yyyyMMdd") & ".aspx"
								...
								esSSL = dbRow("WEBSWSSL")                                                                       'Es una pàgina segura?
                                midaHoMaxima = dbRow("WEBWNMTH")                                                                'Mida horitzontal maxima

                                If dbRow("WEBDSDES").length > 0 Then                                                            'Descripció
                                    tagsMeta += "<meta name=""DC.Description"" content=""" + dbRow("WEBDSDES") + """/>"
                                    tagsMeta += "<meta name=""Description"" content=""" + dbRow("WEBDSDES") + """/>"
                                End If

                                If dbRow("WEBDSPCL").length > 0 Then                                                            'Paraulas clau
                                    tagsMeta += "<meta name=""keywords"" content=""" + dbRow("WEBDSPCL") + """/>"
                                    tagsMeta += "<meta name=""DC.subject"" content=""" + dbRow("WEBDSPCL") + """/>"
                                End If

                                If dbRow("WEBTPBUS") = "S" Then                                                                 'Incloure al cercador
                                    tagsMeta += "<meta name=""robots"" content=""INDEX,FOLLOW""/>"
                                Else
                                    tagsMeta += "<meta name=""robots"" content=""NOINDEX,NOFOLLOW""/>"
                                End If
                    ...
                    If cont = 0 Then
                        html = estructura
                    Else
                        ' Busco la posición que ocupa en el nodo anterior
                        DS2 = New DataSet()
                        GAIA.bdr(objConn, "SELECT RELCDEST FROM METLREL WHERE RELINFIL=" & arrayPathRelacio(cont), DS2)

                        ' Recorro la estructura y pego el nodo en la celda correspondiente del anterior
                        arrayEstructura = Split(html, "id='d")
                        html = arrayEstructura(0)

                        For i As Integer = 1 To arrayEstructura.Length - 1
                            idCel = arrayEstructura(i).Substring(0, arrayEstructura(i).IndexOf("'"))
                            If idCel = DS2.Tables(0).Rows(0)("RELCDEST") Then
                                arrayEstructura(i) = arrayEstructura(i).Insert(arrayEstructura(i).IndexOf(">") + 1, estructura)
                            End If
                            html += arrayEstructura(i).Substring(arrayEstructura(i).IndexOf("'") + 1)    'Desaparecen los identificadores del nodo anterior
                        Next i

                    End If
                Next  ' cont recorrer arrayPathRelacio
                ' tratar contenido del último nodo  rel.incod
            ...
            ' Busco la posición que la fulla web ocupa en el nodo anterior
            DS2 = New DataSet()
            GAIA.bdr(objConn, "SELECT RELCDEST FROM METLREL WHERE RELINCOD=" & rel.incod, DS2)
            ...
            For i As Integer = 1 To arrayEstructura.Length - 1
                idCel = arrayEstructura(i).Substring(0, arrayEstructura(i).IndexOf("'"))
                If idCel = DS2.Tables(0).Rows(0)("RELCDEST") Then
                    arrayEstructura(i) = arrayEstructura(i).Insert(arrayEstructura(i).IndexOf(">") + 1, estructuraFullaWeb)
                End If
                html += arrayEstructura(i).Substring(arrayEstructura(i).IndexOf("'") + 1)    'Desaparecen los identificadores del nodo anterior
            Next i
    ...
    Public Shared Function maquetarPlantilla(ByVal objConn As OleDbConnection, ByVal plantilla As clsPlantilla2, ByVal rel As clsRelacio, ByVal idioma As Integer, ByVal relIni As clsRelacio, ByVal dataSimulacio As DateTime, ByVal codiUsuari As Integer, ByRef llistaDocuments As String(), ByRef urlDesti As String, ByVal fdesti As String, ByVal publicar As Integer, ByVal esEML As String, ByVal tagsMeta As String) As String
        ...
        For i As Integer = 1 To arrayEstructura.Length - 1
            ...
            htmlTmp = GAIA.maquetarContingut(objConn, plantilla, rel, i - 1, 0, llistaDocuments, urlDesti, idioma, relIni, 0, fdesti, publicar, codiUsuari, dataSimulacio, esEML, tagsMeta, 0)
            ...
            If plantilla.innod > 0 Then
                    ...
                    If (plantilla.arrayLCW(idCel).Trim().Length > 0) Then
                        ...
                        For Each item In plantilla.arrayLCW(idCel).Split("|")
                            If width = "" Then
                                width = "0"
                            End If
                            lcwANT = lcwANT + GAIA.trobaCodiWeb2(objConn, plantilla, item, idioma, rel, relIni, forçarCodiAnt, idCel, "", width, codiUsuari, "", tagsMeta)
                ...
                If plantilla.arrayCampsPlantilla.Length > idCel Then
                    ...
                    If (plantilla.arrayCampsPlantilla(idCel).IndexOf(".") >= 0) Then
                        ...
                        GAIA.bdr(objConn, strSQL & strSQLWHERE & strSQLWHEREIdioma & strSQLOrdre, DS)
                        ...
                        If DS.Tables(0).Rows.Count = 0 Then
                            ...
                            End If
                            GAIA.bdr(objConn, strSQL & strSQLWHERE & strSQLOrdre, DS)
                            idiomaDiferent = 1
                        End If
                        ...
                        If DS.Tables(0).Rows.Count > 0 Then
                            ...
                            If titolContingut = "" Then
                                titolContingut = GAIA.obtenirTitolContingut(objConn, relIni, idioma)
                            End If
                            ...
                            If Not rel.cdher.StartsWith("_57135") Then
                                ds2 = New DataSet()
                                GAIA.bdr(objConn, "SELECT REIDTPUB,REIDTCAD FROM METLREI WITH(NOLOCK) WHERE REIINCOD=" & rel.incod, ds2)
                                If ds2.Tables(0).Rows.Count > 0 Then
                            ...
                            If dataIni <= CDate(dataSimulacio) And dataFi > CDate(dataSimulacio) Then
                                ...
                                Else 'no és imatge                  
                                    ...
                                    If prefixCamp = "LNK" And camp.Substring(0, 3) = "LNK" Then
                                                ...
                                                Select Case plantilla.arrayEnllaços(idCel).Substring(plantilla.arrayEnllaços(idCel).IndexOf(".") + 1)
                                                    Case "IMATGE"
                                                        ...
                                                        If plantilla.arrayPLTDSPLT(idCel).Length = 0 Then
                                                            GAIA.f_logError(objConn, "maquetarContingut", "", "S'ha intentat representar una imatge o document dins d'un contingut sense plantilla secund&agrave;ria per representar-lo")
                                                        End If
                                        ...
                                        Else
                                            If enllaç.Trim.Length = 0 Then
                                                enllaç = ""
                                            Else
                                                enllaç = GAIA.obtenirEnllaçSimple(objConn, dbRow("LNKINNOD"), dbRow("idioma"), target, urlDesti)
                                            End If
                                        End If
                                    End If
                                    ...
                                    If camp.Trim() <> "" Then
                                        ...
                                        If plantilla.arrayDSNUM(idCel) > 0 Then
                                            strContingut = GAIA.tallaText(dbRow(camp), plantilla.arrayDSNUM(idCel)) & "..."
                                        Else
                                ...
                                If (rel.tipdsdes = "fulla document" And idiomaDiferent) Or (Not idiomaDiferent) Then
                                    If (enllaç + "").Trim().Length > 0 Then
                                        ...
                                        If rel.tipintip = "49" Then
                                            target = GAIA.obtenirTarget(objConn, rel)
                                        Else
                                            Try
                                        ...
                                        If codiRelacioArxiuPerVisorImatges = 0 Then
                                            ...
                                            celdaFormateada += GAIA.posaFormat2(objConn, strContingut, plantilla.arrayCSSPlantilla(idCel), enllaç, esImatge, width, 0, 0, estat, textalternatiu, 1, 0, target, rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, 0, "")
                                        End If
                                    ...
                                    Else
                                        ...
                                        End If
                                        celdaFormateada += GAIA.posaFormat2(objConn, strContingut, plantilla.arrayCSSPlantilla(idCel), "", esImatge, width, 0, 0, estat, textalternatiu, 1, 0, target, rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, 0, "")
                                    End If
                        ...
                        If plantilla.arrayCampsPlantilla(idCel) = "IMATGE" Or plantilla.arrayCampsPlantilla(idCel) = "DOCUMENT" Then
                            ...
                            Else
                                ...
                                If plantillaTMP.innod > 0 Then
                                    If plantilla.arrayCampsPlantilla(idCel) = "IMATGE" Then
                                        GAIA.bdr(objConn, "SELECT " & sqlTOP & " RELINFIL,RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELINCOD IN (SELECT MIN(RELINCOD) FROM METLREL WHERE RELINPAR=" & rel.infil & " AND RELCDSIT=96 GROUP BY RELINFIL)  ORDER BY RELCDORD", ds3)
                                    ...
                                    Else
                                        ...
                                        GAIA.bdr(objConn, "SELECT " & sqlTOP & " RELINFIL,RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELINCOD IN (SELECT MIN(RELINCOD) FROM METLREL WHERE RELINPAR=" & rel.infil & " AND RELCDSIT=97 GROUP BY RELINFIL)  ORDER BY RELCDORD", ds3)
                                    ...
                                    For Each dbrow3 In ds3.Tables(0).Rows
                                        If codiNodeTMP <> dbrow3("RELINFIL") Then
                                            ...
                                            celdaFormateada = GAIA.maquetarContingut(objConn, plantillaTMP, relarxiu, 0, 0, llistaDocuments, urlDesti, idioma, relIni, 0, fdesti, publicar, codiUsuari, dataSimulacio, esEML, tagsMeta, heretaPropietatsWeb)
                            ...
                            If ((plantilla.arrayEnllaços(idCel) = "IMATGE" And plantillaTMP.innod = 0) Or (plantilla.arrayEnllaços(idCel) = "DOCUMENT")) And plantilla.arrayEnllaços(idCel) <> plantilla.arrayCampsPlantilla(idCel) Then
                                If plantilla.arrayEnllaços(idCel) = "IMATGE" Then
                                    ...
                                    GAIA.bdr(objConn, "SELECT " + sqlTOP + " * FROM METLREL WITH(NOLOCK),METLDOC WITH(NOLOCK),METLTDO WITH(NOLOCK)WHERE DOCINTDO=TDOCDTDO AND RELINPAR=" & rel.infil & " AND RELCDSIT=96 AND DOCINNOD=RELINFIL AND DOCINIDI=" + idioma.ToString() + " ORDER BY RELCDORD", ds3)
                                Else
                                    GAIA.bdr(objConn, "SELECT " + sqlTOP + " * FROM METLREL WITH(NOLOCK),METLDOC WITH(NOLOCK),METLTDO WITH(NOLOCK) WHERE DOCINTDO=TDOCDTDO AND RELCDRSU=" + rel.incod.ToString() + "  AND RELCDSIT=97 AND DOCINNOD=RELINFIL AND DOCINIDI=" + idioma.ToString() + " ORDER BY RELCDORD", ds3)
                                End If
                                If ds3.Tables(0).Rows.Count = 0 Then 'si no ho trobo amb l'idioma, busco a d'altres idiomes
                                    If plantilla.arrayEnllaços(idCel) = "IMATGE" Then
                                        ...
                                        GAIA.bdr(objConn, "SELECT " + sqlTOP + " * FROM METLREL WITH(NOLOCK),METLDOC WITH(NOLOCK),METLTDO WITH(NOLOCK) WHERE DOCINTDO=TDOCDTDO AND  RELINPAR=" & rel.infil & "  AND RELCDSIT=96 AND DOCINNOD=RELINFIL  ORDER BY RELCDORD", ds3)
                                    Else
                                        GAIA.bdr(objConn, "SELECT " + sqlTOP + " * FROM METLREL WITH(NOLOCK),METLDOC WITH(NOLOCK),METLTDO WITH(NOLOCK) WHERE DOCINTDO=TDOCDTDO AND RELCDRSU=" + rel.incod.ToString() + "  AND RELCDSIT=97 AND DOCINNOD=RELINFIL  ORDER BY RELCDORD", ds3)
                                    End If
                                End If
                                If ds3.Tables(0).Rows.Count > 0 Then
                                    ...
                                    If pospunt > 0 Then
                                        ...
                                        strvoid = GAIA.trobaEstilCSS2(objConn, plantilla.arrayCSSPlantilla(idCel), desfaseHoritzontal, heretaPropietatsWeb, strvoid, estilsCSSSenseFons, esEML, posaWidthGAIA, "", posafloatGAIA, "", plantilla.arrayEsImatge(idCel))
                                        ...
                                        Else
                                            celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) + "<a href=""" & strURL & """ title=""" & GAIA.netejaHTML(titolContingut) & IIf(target = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """ " & estilsCSSSenseFons & IIf(target = 1, " target=""_blank"" ", " target=""_self"" ") & ">" & celdaFormateada.Substring(pospunt - 1) & "</a>"
                                        End If
                            ...
                            Else
                                ...
                                Select Case plantilla.arrayEnllaços(idCel)
                                    Case "METLLNK.LNKCDREL"
                                        ...
                                        If pospunt > 0 Then
                                            strURL = GAIA.obtenirEnllaçSimple(objConn, rel.infil, idioma, target, urlDesti)
                                            If esEML = "S" Then
                                                botoMesImatge = "<img src=""http://www.l-h.cat/img/common/mesimg.gif"" style=""float:right; position:relative; top:-18px; right:3px; border:0"" class=""noImprimible"" alt=""" & IIf(idioma = 2, "Ver imagen de ", IIf(idioma = 3, "﻿seeing image of ", "Veure imatge de ")) & titolContingut & IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)")) & """/>"
                                            Else
                                                botoMesImatge = "<img src=""http://www.l-h.cat/img/common/mesimg.gif"" class=""simbolmes noImprimible"" alt=""" & IIf(idioma = 2, "Ver imagen de ", IIf(idioma = 3, "﻿seeing image of ", "Veure imatge de ")) & titolContingut & IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)")) & """/>"
                                    ...
                                    Case "METLLNK.LNKDSLNK"
                                        ...
                                        If pospunt > 0 Then
                                            GAIA.bdr(objConn, "SELECT LNKDSLNK,LNKWNTIP FROM METLLNK WITH(NOLOCK) WHERE LNKINNOD=" & rel.infil & " AND LNKINIDI=" & idioma, ds3)

                                            If ds3.Tables(0).Rows.Count = 0 Then
                                                GAIA.bdr(objConn, "SELECT LNKDSLNK,LNKWNTIP FROM METLLNK WITH(NOLOCK) WHERE LNKINNOD=" & rel.infil & " ORDER BY LNKINIDI", ds3)
                                            ...
                                            If ds3.Tables(0).Rows.Count > 0 Then
                                                ...
                                                If dbrow3("LNKDSLNK") <> "" Then
                                                    ...
                                                    If esEML = "N" Then
                                                        celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & strURL & """ title=""" & GAIA.netejaHTML(titolContingut) & IIf(dbrow3("LNKWNTIP") = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """ " & estilsCSSSenseFons & IIf(dbrow3("LNKWNTIP") = 1, " target=""_blank""", " target=""_self""") & ">" & celdaFormateada.Substring(pospunt - 1, InStrRev(celdaFormateada, "</div>") - pospunt) & "</a></div>"
                                                    Else
                                                        celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & strURL & """ title=""" & GAIA.netejaHTML(titolContingut) & IIf(dbrow3("LNKWNTIP") = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """ " & estilsCSSSenseFons & IIf(dbrow3("LNKWNTIP") = 1, " target=""_blank""", " target=""_self""") & ">" & celdaFormateada.Substring(pospunt - 1) & "</a>"
                                                    End If
                                    ...
                                    Case "METLNOT.NOTDSLNK"
                                        GAIA.bdr(objConn, "SELECT " & plantilla.arrayEnllaços(idCel) & " FROM METLNOT WITH(NOLOCK) WHERE NOTINNOD=" & rel.infil & " AND NOTINIDI=" & idioma, ds3)
                                        If ds3.Tables(0).Rows.Count > 0 Then
                                            dbrow3 = ds3.Tables(0).Rows(0)
                                            If titolContingut = "" Then
                                                titolContingut = GAIA.obtenirTitolContingut(objConn, rel, idioma)
                                            ...
                                            strvoid = GAIA.trobaEstilCSS(objConn, plantilla.arrayCSSPlantilla(idCel), desfaseHoritzontal, heretaPropietatsWeb, strvoid, estilsCSSSenseFons, esEML, posaWidthGAIA, "", posafloatGAIA, plantilla.arrayEsImatge(idCel))

                                            If strTMp.Length And celdaFormateada.Length > 0 Then
                                                If esEML = "N" Then
                                                    celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTMp) & """  " & estilsCSSSenseFons & " title=""" & GAIA.netejaHTML(titolContingut) & """>" & celdaFormateada.Substring(pospunt - 1, InStrRev(celdaFormateada, "</div>") - pospunt) & "</a></div>"
                                                Else
                                                    celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTMp) & """  " & estilsCSSSenseFons & " title=""" & GAIA.netejaHTML(titolContingut) & """>" & celdaFormateada.Substring(pospunt - 1) & "</a>"
                                                End If
                                            End If
                                    ...
                                    Case "METLINF.INFDSLNK"
                                        GAIA.bdr(objConn, "SELECT METLINF.INFDSLNK FROM METLINF WITH(NOLOCK) WHERE INFINNOD=" & rel.infil & " AND INFINIDI=" & idioma, ds3)
                                        If ds3.Tables(0).Rows.Count > 0 Then
                                            dbrow3 = ds3.Tables(0).Rows(0)
                                            'If titolContingut = "" Then
                                            '    titolContingut = GAIA.obtenirTitolContingut(objConn, rel, idioma)
                                            ...
                                            If strTmp.Length > 0 Then
                                                If esEML = "N" Then
                                                    celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strHref) & """  " & estilsCSSSenseFons & " title=""" & GAIA.netejaHTML(strText) & IIf(target = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """>" & celdaFormateada.Substring(pospunt - 1, InStrRev(celdaFormateada, "</div>") - pospunt) & "</a></div>"
                                                Else
                                                    celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strHref) & """  " & estilsCSSSenseFons & " title=""" & GAIA.netejaHTML(strText) & IIf(target = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """>" & celdaFormateada.Substring(pospunt - 1) & "</a>"

                                                End If
                                            End If
                                        End If
                                    Case "METLAGD.AGDDSLNK"
                                        GAIA.bdr(objConn, "SELECT " & plantilla.arrayEnllaços(idCel) & " FROM METLAGD WITH(NOLOCK) WHERE AGDINNOD=" & rel.infil & " AND AGDINIDI=" & idioma, ds3)
                                        If ds3.Tables(0).Rows.Count > 0 Then
                                            dbrow3 = ds3.Tables(0).Rows(0)
                                            If titolContingut = "" Then
                                                titolContingut = GAIA.obtenirTitolContingut(objConn, rel, idioma)
                                    ...
                                    Case "AUTO-ENLLAÇ"
                                        If pospunt > 0 Then 'si no hi ha imatge no fem l'autoenllaç
                                            ...
                                            End If
                                            If esEML = "N" Then
                                                celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTmp) & """ " & estilsCSSSenseFons  & " title=""" & GAIA.obtenirTitolContingut(objConn, rel, idioma) & """>" & celdaFormateada.Substring(pospunt - 1, InStrRev(celdaFormateada, "</div>") - pospunt) & "</a></div>"
                                            Else
                                                celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTmp) & """ " & estilsCSSSenseFons  & " title=""" & GAIA.obtenirTitolContingut(objConn, rel, idioma) & """>" & celdaFormateada.Substring(pospunt - 1) & "</a>"
                                            End If
                                        End If
                                End Select
                            End If
                ...
                If codiRelacioArxiuPerVisorImatges <> 0 Then
                    ...
                    If plantillaTMP.innod > 0 Then
                        ...
                        celdaFormateada = "<a href=""" & GAIA.netejaHTML(enllaç) & """ title=""" & GAIA.netejaHTML(titolContingut) & IIf(target = 0, "", IIf(idioma = 2, " (nueva ventana)", IIf(idioma = 3, " (new window)", " (nova finestra)"))) & """ " & estilsCSSSenseFons & IIf(target = 1, " target=""_blank"" ", " target=""_self"" ") & ">" & GAIA.netejaHTML(titolContingut) & "</a></div>"
...
Public Shared Sub dibuixaPreview(ByVal objConn As OleDbConnection, ByVal plantilla As clsPlantilla, ByVal estructura As String, ByVal formaEstructura As String(), ByVal thor As String, ByVal tver As String, ByVal tcont As String, ByVal tplant As String, ByRef html As String, ByRef css As String, ByVal tMaxhor As Double, ByVal tMaxVer As Double, ByVal prefix As String, ByVal rel As clsRelacio, ByVal pathRelacio As String, ByRef fDesti As String, ByRef urlDesti As String, ByRef nrocontingut As Integer, ByRef llistaDocuments As String(), ByRef publicar As Integer, ByVal idioma As Integer, ByVal estilCSS As String, ByVal relIni As clsRelacio, ByVal dataSimulacio As DateTime, ByVal codiUsuari As Integer, ByVal autolink As Integer, ByVal plantillesSecundaries As String, ByVal heretaPropietatsWeb As Boolean, ByVal estilFLW As String, ByRef titol As String, ByRef nivell As Integer, ByRef estilbody As String, ByVal llistaCND As String, ByRef esForm As String, ByRef esEML As String, ByRef esSSL As String, ByRef tagsMeta As String, ByRef htmlPeu As String, ByRef strCSSPantalla As String, ByRef strCSSImpressora As String, ByVal llistaNivellsTextos As String, Optional ByVal pareTeEnllac As Boolean = False)
        ...
        If estructura.Contains(index + subindex) Then
            ...
            If arrayEstilCSS.Length >= estructura(index + subindex) + 1 Then ' Si hi ha un estil css el poso									
                If arrayEstilCSS(estructura(index + subindex + "")).Length > 0 Then
                    strCSS = GAIA.trobaEstilCSS(objConn, arrayEstilCSS(estructura(index + subindex + "")), desfaseHoritzontal, heretaPropietatsWeb, strCSSTamanyText, strCssSenseFons, esEml, posaWidthGAIA, posaVoreres, posaFloatGAIA).ToString()
                                        ...
                                        If estatCircuit(objConn, relFill, idioma, relIni, dataSimulacio, codiUsuari, publicar, estatSituacio) = 1 Then
                                            ...
                                            If relFill.tipdsdes = "node web" Or relFill.tipdsdes = "fulla web" Then
                                                If relFill.tipdsdes = "node web" Or (relFill.tipdsdes = "fulla web" And relFill.incod = relIni.incod) Or (relFill.tipdsdes = "fulla web" And InStr(pathRelacio, relFill.infil) > 0) Then
                                                    ...
                                                    GAIA.obreFullaWeb(objConn, relFill, htmlTMP, css, 1, width, height, pathRelacio, fDesti, urlDesti, nrocontingut, llistaDocuments, 1, idioma, relIni, dataSimulacio, codiUsuari, titol, nivell, estilbody, esForm, esEml, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, False)
                                                End If
                                            Else
                                                '******************************************************************
                                                ...
                                                '******************************************************************
                                                htmlTMP &= GAIA.representaCodiWeb(objConn, plantilla, relFill, estructura(index + subindex), idioma, relIni, width, codiUsuari, forçarCodi, css, tagsMeta)
            ...
            Else
                If prefix.Substring(0) = "p" Or prefix.Substring(0) = "f" Then
                    GAIA.pintarContingut(objConn, plantilla, rel, estructura(index + subindex), rel.infil, Math.Round(width, 0).ToString(), llistaDocuments, urlDesti, idioma, relIni, 3, fDesti, publicar, heretaPropietatsWeb, codiUsuari, dataSimulacio, htmlNou, css, estilbody, llistaCND, esForm, esEml, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, pareTeEnllac)
                End If
    ...
    Public Shared Sub datesPublicacio(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal idioma As Integer, ByRef dataIni As DateTime, ByRef dataFi As DateTime)
        ...
        Else
            ...
            If DS.Tables(0).Rows.Count > 0 Then
                ...
                DS.Dispose()
            End If
            GAIA.datesPublicacioPerNode(objconn, rel.infil, idioma, dataIniContingut, dataFiContingut)
    ...
    Public Shared Sub datesPublicacioPerNode(ByVal objconn As OleDbConnection, ByVal codiNode As Integer, ByVal idioma As Integer, ByRef dataIni As DateTime, ByRef dataFi As DateTime)
        ...
        GAIA.bdR(objconn, "SELECT TBLDSTXT FROM METLNOD WITH(NOLOCK) , METLTBL WITH(NOLOCK)  WHERE NODINNOD=" & codiNode.ToString() & " AND  NODCDTIP=TBLINTFU AND TBLINTFU<>9  ", DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
        ...
        If taula <> "METLNWE" AND taula<>"METLFOR" Then
            If taula.Length > 0 Then
				...
				try
					...
					GAIA.bdR(objconn, "SELECT  * FROM  " + taula.ToString() + " WITH(NOLOCK)  WHERE " + prefixCamp.ToString() + "INNOD=" + codiNode.ToString() + " AND " + prefixCamp.ToString() + "INIDI=" + idioma.ToString(), DS)
					If DS.Tables(0).Rows.Count > 0 Then
						dbRow = DS.Tables(0).Rows(0)
...
strErr = "1"
                        ...
                        Select Case tipusIni
                            ...
                            Case "node web"
                                If Not TypeOf dbRow("NWEINNOD") Is DBNull Then
                                    ...
                                    GAIA.dibuixaPreview(objConn, plantilla, dbRow("NWEDSEST"), dbRow("NWEDSATR").split(","), dbRow("NWEDSTHOR"), dbRow("NWEDSTVER"), dbRow("NWEDSTCO"), dbRow("NWEDSPLA"), html, css, tamanyHoritzontalTotal, tamanyVerticalTotal, "n", relTmp, pathRelacio, fDesti, urlDesti, nrocontingut, llistaDocuments, publicar, idioma, estilsCSS, relIni, dataSimulacio, codiUsuari, False, "", 1, "", titol, nivell, estilbody, "", esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, "")
                                End If
                            ...
                            Case Else 'fulla web
                                ...
                                End If
								If esEML = "S" Then  'Teresa: para poder conservar el historico de boletines, se añade la fecha de publicacion al nombre del archivo.
                                        Dim pos As String = InStrRev(fDesti, ".")
                                        fDesti = fDesti.Substring(0, pos - 1) & CDate(Now).ToString("yyyyMMdd") & ".aspx"

                                        pos = InStrRev(urlDesti, ".")
                                        urlDesti = urlDesti.Substring(0, pos - 1) & CDate(Now).ToString("yyyyMMdd") & ".aspx"
                                End If
                                If esSSL <> "S" Then
        ...
        css = GAIA.netejaCSS(css)
        Catch ex As Exception
            GAIA.f_logError(objConn, "obrefullaweb", ex.Message," Rel.incod=" & rel.incod & "_" & strErr)
        End Try
    ...
    Public Shared Function estatCircuit(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal idioma As Integer, ByVal relIni As clsRelacio, ByVal dataSimulacio As DateTime, ByVal codiUsuari As Integer, ByVal publicar As Integer, ByVal estatsituacio As String) As Integer
        ...
        nouEstat = GAIA.contingutEsPublicable(objConn, rel, dataSimulacio, idioma, estatsituacio)
        ...
        If (rel.cdsit = 3 Or nouEstat = 1) And Not (nouEstat = 4) Then 'ja s'ha aprovat i per tant no comprovo els circuits o bé si nouestat=1 no faig res perque es un node de tipus "node *" que no té data de publicació o caducitat
            If nouEstat <> -1 Then
                GAIA.canviEstatRelacio(objConn, rel, nouEstat, 0)
            End If
            estatCircuit = 1
        Else
            codiRelacio = rel.incod
            GAIA.obtenirCodiCircuitFullaWeb(objConn, relIni.infil, idioma, codiCircuitPublicacio, codiCircuitCaducitat)
            Select Case nouEstat
                Case ctPENDENTPUBLICAR
                    ...
                    Else
                        If codiCircuitPublicacio > 0 Then
                            relCircuitActual = GAIA.obtenirPasDeCircuitRelacio(objConn, rel)
                            Select Case relCircuitActual.infil
                                Case 0 'no està en cap circuit						
                                    GAIA.iniciarCircuit(objConn, rel, codiCircuitPublicacio, codiUsuari, 0, relIni)
                                    If DateDiff("n", dataSimulacio, Now) = 0 Then
                                        GAIA.canviEstatRelacio(objConn, rel, nouEstat, 1)
                                    End If
                                Case codiCircuitPublicacio  'està dins d'un circuit de publicació
                                    GAIA.tractarPasCircuit(objConn, rel, relCircuitActual)
                                Case codiCircuitCaducitat  'està dins d'un circuit de caducitat
                                    ...
                                    GAIA.anularPasCircuit(objConn, rel)
                                    GAIA.iniciarCircuit(objConn, rel, codiCircuitPublicacio, codiUsuari, 0, relIni)
                                    If DateDiff("n", dataSimulacio, Now) = 0 Then
                                        GAIA.canviEstatRelacio(objConn, rel, nouEstat, 1)
                                    End If
                ...
                Case ctPUBLICAR
                    ...
                    If codiCircuitPublicacio > 0 Then
                        ...
                        relCircuitActualNodePare = GAIA.obtenirRelacioSuperior(objConn, relCircuitActual)
                ...
                Case ctPENDENTCADUCAR
                    ...
                    If codiCircuitCaducitat > 0 Then
                        ...
                        Select Case relCircuitActualNodePare.infil
                            Case 0 'no està en cap circuit					
                                GAIA.iniciarCircuit(objConn, rel, codiCircuitCaducitat, codiUsuari, 0, relIni)
                    ...
                    Else 'no hi ha circuit associat de caducitat			
                        ...
                        GAIA.caducarContinguts(objConn, rel, codiUsuari)
                        If DateDiff("n", dataSimulacio, Now) = 0 Then
        ...
        If DS.Tables(0).Rows.Count > 0 Then
            ...
            If Split(codiweb, ",").Length > index Then
                ...
                If lcw.Length > 0 Then
                    ...
                    celdaFormateada += GAIA.trobaCodiWeb(objConn, plantilla, lcw, idioma, rel, relIni, forçarCodi, index, "", width, codiusuari, strCss, tagsMeta)
        ...
        Else
            If codiEstat > 0 Then
                ...
                If codiEstat <> rel.cdsit Then
                    GAIA.bdSR(objConn, "UPDATE METLREL SET RELCDSIT=" & codiEstat.ToString() + " WHERE RELINCOD=" + rel.incod.ToString())					
                End If
    ...
    Public Shared Function trobaCodiWeb(ByVal objConn As OleDbConnection, ByVal plantilla As clsPlantilla, ByVal codiWeb As string, ByVal codiIdioma As Integer, ByVal rel As clsRelacio, ByVal relIni As clsRelacio, ByRef forçarCodi As Integer, ByVal index As Integer, ByVal parametres As String, ByVal width As String, ByVal codiUsuari As Integer, ByRef strCss As String, ByRef tagsMeta As String) As String
        ...
        Try
            ...
            For Each element In Split(codiWeb, "|")
                ...
                GAIA.bdR(objConn, "SELECT LCWCDTIP,  LCWDSTXT, LCWTPFOR, LCWTPFOL FROM METLLCW  WITH(NOLOCK) WHERE LCWINNOD=" & element & " AND LCWINIDI=" + codiIdioma.ToString(), DS)
                If DS.Tables(0).Rows.Count = 0 Then 'busco un altre idioma
                    GAIA.bdR(objConn, "SELECT LCWCDTIP,  LCWDSTXT, LCWTPFOR, LCWTPFOL FROM METLLCW  WITH(NOLOCK) WHERE LCWINNOD=" & element & " ORDER BY LCWINIDI ASC", DS)
                End If
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Function contingutEsPublicable(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal dataSimulacio As DateTime, ByVal codiIdioma As Integer, ByVal estatSituacio As String) As Integer
        ...
        taula = ""
        DS = New DataSet()
        GAIA.bdR(objConn, "SELECT TBLDSTXT FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) , METLTBL  WITH(NOLOCK) WHERE RELINCOD=" & codiRelacio.ToString() & " AND RELINFIL=NODINNOD AND NODCDTIP=TBLINTFU AND RELCDSIT<" + estatSituacio, DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
            taula = dbRow("TBLDSTXT")
        End If
        ...
        If taula.Length > 0 Then
            ...
            GAIA.bdR(objConn, "SELECT  " + prefixCamp.ToString() + "DTPUB," + prefixCamp.ToString() + "DTCAD FROM  METLREL WITH(NOLOCK) , " + taula.ToString() + " WITH(NOLOCK)  WHERE RELCDSIT<" + estatSituacio + " AND RELINFIL=" + prefixCamp.ToString() + "INNOD AND RELINCOD=" + codiRelacio.ToString() + " AND " + prefixCamp.ToString() + "INIDI=" + codiIdioma.ToString(), DS)
            If DS.Tables(0).Rows.Count > 0 Then
                dbRow = DS.Tables(0).Rows(0)
        ...
        If contingutEsPublicable <> 4 Then
            ...
            dataIni = CDate("01/01/1900")
            dataFi = CDate("01/01/2050")
            GAIA.datesPublicacio(objConn, rel, codiIdioma, dataIni, dataFi)
    ...
    Public Shared Function publica(ByVal objConn As OleDbConnection, ByVal codiHtml As String, ByVal fDesti As String, ByVal llistaDocumentsPerAfegir As String(), ByVal llistaDocumentsPerEsborrar As String(), ByVal tipusAccio As Integer, ByVal relIni As clsRelacio, ByVal idioma As Integer, ByRef servidor As String, ByRef ftpSession As Session) As Integer
	   ...
	   TRY
        ...
        Else 'tipusAccio=1 -> copiar
                ...
                GAIA.trobaServidorDesti(objConn, relIni, servidorNou, usuari, pwd, pathDestiArrel, pathDocsDesti, port, URL)
                strErr &= "3"
...
strErr &= "(6.3)"
                        If nomFitxerPublicat = "" Or UPD = 1 Then
                            If UPD = 1 Then
								...
								strErr &="7: pathDocsDesti=" & pathDocsDesti & ", document=" & document
								 llistatErrors &=   GAIA.ftpCopiarFitxer(objConn, servidorNou, usuari, pwd, pathOrigen + document, pathDocsDesti + "/" + document, relacio, "", ftpSession,pathDestiArrel)
								 strErr &= llistatErrors
							
								GAIA.bdSR(objConn, "UPDATE METLREL SET RELDSFIT='" & pathDocsDesti.replace(pathDestiArrel,"") &  "/" & document.Replace("/webs/auditoribarradas", "").Replace("/webs/teatrejoventut", "") + "' WHERE RELINCOD=" + relacio.ToString())
                                If document.Length > 0 Then
                                    GAIA.bdR(objConn, "SELECT * FROM METLREI  WITH(NOLOCK) WHERE REIINCOD=" + relacio.ToString() + " AND REIINIDI=" + idioma.ToString(), ds)
                                    If ds.Tables(0).Rows.Count > 0 Then

										
										GAIA.bdSR(objConn, "UPDATE METLREI SET REIDSFIT='" + (pathDocsDesti.replace("\","/").replace(pathDestiArrel,"") & "/" & document).Replace("/webs/auditoribarradas", "").Replace("/webs/teatrejoventut", "").Replace("/webs/teatreJoventut", "") & "', REIDSFI2='" & nomFitxer & "' WHERE REIINCOD=" & relTmp.incod & " AND REIINIDI = " & idioma)
										
										
                                    Else
                                        Dim dataIni, dataFi As DateTime
                                        GAIA.datesPublicacio(objConn, relTmp, idioma, dataIni, dataFi)
										
										GAIA.bdSR(objConn, "INSERT INTO METLREI VALUES(" & relTmp.incod & "," & idioma & ",'" & dataIni.ToString() & "','" & dataFi.ToString() & "','" & (pathDocsDesti.replace("\","/").replace(pathDestiArrel,"").Replace("/GAIA06", "").Replace("/GAIA", "") & "/" & document).Replace("/webs/auditoribarradas", "").Replace("/webs/teatreJoventut", "") & "','" & document & "','')")
                                    End If
								...
								strErr &="8"
                                     llistatErrors &=   GAIA.ftpCopiarFitxer(objConn, servidorNou, usuari, pwd, pathOrigen.ToString() + document.Substring(0, posTMP - 1) + "P" + document.Substring(posTMP - 1), pathDocsDesti + "/" + document.Substring(0, posTMP - 1) + "P" + document.Substring(posTMP - 1), relacio, "P", ftpSession,pathDestiArrel)
									strErr &="9" & llistatErrors
									
						
                                   llistatErrors &=  GAIA.ftpCopiarFitxer(objConn, servidorNou, usuari, pwd, pathOrigen.ToString() + document.Substring(0, posTMP - 1) + "P100" + document.Substring(posTMP - 1), pathDocsDesti + "/" + document.Substring(0, posTMP - 1) + "P100" + document.Substring(posTMP - 1), relacio, "P100", ftpSession,pathDestiArrel)
									strErr &="10" & llistatErrors
                                End If
                                If codiDocument > 0 Then
                                    GAIA.bdSR(objConn, "UPDATE METLDOC SET DOCWNUPD=0 WHERE DOCINIDI=" + idioma.ToString() + " AND DOCINNOD=" + codiDocument.ToString())
                                End If
                            End If
                        End If
                    Else
            ...
            codiHtml = codiHtml.Replace("<u>", "<span class='subratllat'>").Replace("<em>", "<span class='cursiva'>").Replace("<i>", "<span class='cursiva'>")
            ...
            Dim strHash As String = Convert.ToBase64String(sha1Obj.ComputeHash(enc.GetBytes(codiHtml)))
                ...
                GAIA.bdR(objConn, "SELECT isnull(REIDSHAS,'') as REIDSHAS FROM METLREI  WITH(NOLOCK) WHERE REIINCOD=" & relIni.incod & " AND REIINIDI=" & idioma, ds)
            If ds.Tables(0).Rows.Count > 0 Then
                If ds.Tables(0).Rows(0)("REIDSHAS") = strHash Then
                    GAIA.llistatWarnings &= "No s'ha publicat la pàgina per que és igual a la que ja existia"
                    publica = 0
                End If
            End If
                ...
                If servidorNou <> "" Then
                    ...
                    If fitxerTMP.Length > 0 Then
                        ...
                        If InStr(UCase(pathDestiArrel + fDesti), "PROVES") <= 0 AND servidor<>"intranet" Then
                            ...
                            llistatErrors &=  GAIA.ftpCopiarFitxer(objConn, servidorNou, usuari, pwd, destilocal, pathDestiArrel + fDesti, relIni.incod, "", ftpSession,pathDestiArrel)
                       ...
                       ' GAIA.bdSR(objConn, "UPDATE METLREL SET RELDSFIT='" + (pathDestiArrel.replace("\","/").Replace("/GAIA06", "").Replace("/gaia", "").Replace("/GAIA", "").Replace("/joventut", "").Replace("/lh2010", "").Replace("/museu", "").Replace("/ccbobila", "").Replace("/ccteclasala", "").Replace("/arranzbravo", "").Replace("/EMCA", "").Replace("/seuelectronica", "").Replace("/ccSantJosep", "").Replace("/promocio", "").Replace("/ccct", "").Replace("/cooperacio", "").Replace("/xarxasida", "") + fDesti).Replace("/webs/auditoribarradas", "").Replace("/webs/teatrejoventut", "").Replace("/lhon", "").replace("/TorreBarrina","").replace("/pmc","").replace("/pmlh","").replace("/pmsf","") + "' WHERE RELINCOD=" + relIni.incod.ToString())
					    GAIA.bdSR(objConn, "UPDATE METLREL SET RELDSFIT='" & fDesti.Replace("/webs/teatrejoventut", "") & "' WHERE RELINCOD=" & relIni.incod.ToString())
						strErr &="17"
                        GAIA.bdR(objConn, "SELECT * FROM METLREI  WITH(NOLOCK) WHERE REIINCOD=" & relIni.incod & " AND REIINIDI=" & idioma, ds)
						strErr &="18"
                        If ds.Tables(0).Rows.Count > 0 Then
						   ...
						   GAIA.bdSR(objConn, "UPDATE METLREI SET REIDSFIT='" &  fDesti.replace("\","/").Replace("/webs/teatrejoventut", "") & "', REIDSFI2='" & fitxerTMP & "' , REIDSHAS='" & strHash & "' WHERE REIINCOD=" + relIni.incod.ToString() + " AND REIINIDI = " & idioma)
	   						strErr &="19"
 						Else

                            Dim dataIni, dataFi As DateTime
                            GAIA.datesPublicacio(objConn, relIni, idioma, dataIni, dataFi)
                           ' GAIA.bdSR(objConn, "INSERT INTO METLREI VALUES(" + relIni.incod.ToString() + "," + idioma.ToString() + ",'" + dataIni.ToString() + "','" + dataFi.ToString() + "','" + (pathDestiArrel.replace("\","/").Replace("/GAIA06", "").Replace("/gaia", "").Replace("/GAIA", "").Replace("/cooperacio", "").Replace("/joventut", "").Replace("/lh2010", "").Replace("/museu", "").Replace("/ccbobila", "").Replace("/ccct", "").Replace("/ccteclasala", "").Replace("/arranzbravo", "").Replace("/EMCA", "").Replace("/seuelectronica", "").Replace("/ccSantJosep", "").Replace("/promocio", "").Replace("/xarxasida", "") + fDesti).Replace("/webs/auditoribarradas", "").Replace("/webs/teatrejoventut", "").Replace("/lhon", "").replace("/TorreBarrina","").replace("/pmc","").replace("/pmlh","").replace("/pmsf","") + "','" + fitxerTMP + "','" & strHash & "')")
							...
							try
							strErr &="a"	
							GAIA.bdSR(objConn, "INSERT INTO METLREI VALUES(" & relIni.incod & "," & idioma & ",'" & dataIni.ToString() & "','" & dataFi.ToString() & "','" &  fDesti.Replace("/webs/teatrejoventut", "") & "','" & fitxerTMP & "','" & strHash & "')")
							strErr &="b"	
							catch
								gaia.debug(objconn, "publica: problema al fer aquesta acció: " &  "INSERT INTO METLREI VALUES(" & relIni.incod & "," & idioma & ",'" & dataIni.ToString() & "','" & dataFi.ToString() & "','" &  fDesti & "','" & fitxerTMP & "','" & strHash & "')")
							end try
							strErr &="21"							
                        End If
			...
			GAIA.bdsr(objconn, "DELETE FROM METLMAN WHERE MANDTTEM=0 AND (MANCDREL=" & relIni.incod & " OR MANCDNOD=" & relIni.infil & ") AND MANDTDAT<getdate()  AND MANCDIDI=" & idioma )
    ...
    Public Shared Sub generarDatesPublicacio(ByVal objConn As OleDbConnection, ByVal codiNode As Integer, ByVal idioma As Integer, ByVal dataPublicacio As String, ByVal dataCaducitat As String)
        ...
        For Each dbRow In ds.Tables(0).Rows
            ...
            GAIA.bdR(objConn, sSQL, ds2)

            If ds2.Tables(0).Rows.Count = 0 Then
                'si no existeix l'entrada fem l'insert amb els nous valors
				
                sSQL = "INSERT INTO METLREI (REIINCOD,REIINIDI,REIDTPUB,REIDTCAD,REIDSFIT, REIDSFI2,REIDSHAS) VALUES " & _
            ...
            Else
                For Each dbRow2 In ds2.Tables(0).Rows
                    If dbRow2("REIDTPUB").ToString() <> dataPublicacio Or dbRow2("REIDTCAD").ToString() <> dataCaducitat Then
                        GAIA.bdSR(objConn, "UPDATE METLREI SET REIDTPUB=" + dataPublicacio + ", REIDTCAD=" + dataCaducitat + " WHERE REIINCOD=" + dbRow2("REIINCOD").ToString() + " AND REIINIDI=" + dbRow2("REIINIDI").ToString())
                    End If
    ...
    Public Shared Function trobaPosicioEstructura(ByVal objConn As OleDbConnection, ByVal codiRelacio As Integer) As Integer
        ...
        GAIA.bdR(objConn, "SELECT RELCDEST FROM METLREL  WITH(NOLOCK) WHERE RELINCOD=" + codiRelacio.ToString() + " AND RELCDSIT<98", DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Function posaFormat(ByVal objConn As OleDbConnection, ByVal strContingut As String, ByVal css As String, ByVal enllaç As String, ByVal esImatge As String, ByVal width As String, ByVal tamanyH As Integer, ByVal tamanyV As Integer, ByVal estat As Integer, ByVal textalternatiu As String, ByVal tancaEnllaç As boolean, ByVal heretaPropietatsWeb As Integer, ByVal target As Integer, ByVal rel As clsRelacio, ByVal relini As clsRelacio, ByVal idioma As Integer, ByVal esEML As String, ByVal botoMesImatge As String, ByVal titolContingut As String, ByVal plt As clsPlantilla, ByVal pareTeEnllac As Boolean, byRef cssGenerat as string) As String
        ...
        If enllaç.Trim().Length > 0 Then
            If titolContingut.Length > 0 Then
                textalternatiu = GAIA.netejaHTML(titolContingut)
            Else
                textalternatiu = GAIA.obtenirTitolContingut(objConn, relini, idioma)
            End If
        Else

            textalternatiu = GAIA.netejaHTML(textalternatiu.Trim())
            textAlternatiuImatge = textalternatiu
        ...
        If css <> "" Then
            strCss = GAIA.trobaEstilCSS(objConn, css, desfaseHoritzontal, heretaPropietatsWeb, strCssTamanyText, strCssSenseFons, esEML, posaWidthGAIA, "", posaFloatGAIA, estilsHref)
                ...
                If InStr(strContingut, ".jpg") > 0 Or InStr(strContingut, ".swf") > 0 Or InStr(strContingut, "obreFitxer") > 0 Then
                    ...
                    If enllaç.Trim().Length > 0 Then
                        ...
                        If botoMesImatge <> "" And esImatge Then
                            ...
                            target = 1
                        End If
                        posaFormat = "<a href=""" + enllaç + """  target=""" & IIf(target = 1, "_blank", "_self") & """ title=""" & GAIA.netejaHTML(textAlternatiuExtes) & IIf(target = 1, IIf(idioma = 1, " (nova finestra)", " (nueva ventana) "), "") & """ class=""mesImatges"">" & posaFormat
                        tancaEnllaç = True
                    ...
                    If InStr(strContingut, "swf") > 0 Then
                        posaFormat += "<object classid=""clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"" codebase=""http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0""  " + strCss + " alt=""" & GAIA.netejaHTML(textalternatiu.Replace("""", "´").Trim()) & """><param name=""scale"" value=""ExactFit""/><param name=""movie"" value=""" + strContingut + """><param name=""quality"" value=""high""><embed src=""" + strContingut + """ quality=""high"" pluginspage=""http://www.macromedia.com/go/getflashplayer"" type=""application/x-shockwave-flash"" " + strCss + "><param name=""scale"" value=""ExactFit ""/></embed></object>"
                    '******************************************************************
					'  és una imatge														
                    '******************************************************************					
                    Else
            ...
            Else 'No és una imatge.
                If enllaç.Trim().Length > 0 Then
                        ...
                        If InStr(enllaç, "obreFitxer") > 0 Or InStr(enllaç, "visorImatges") > 0 Then 'si és un document l'obro en una finestra nova.                           
                        	...
                        	strCss =""
                        posaFormat = "<a href=""" & enllaç & """ " & estilsHref & " target=""_blank"" title=""" & GAIA.netejaHTML(textalternatiu) & IIf(idioma = 1, " (nova finestra) ", " (nueva ventana) ") & """>" & strContingut & "</a>"
                ...
                Else
                    ...
                    strCss = ""
                        ...
                        posaFormat = "<a href=""" & enllaç & """ " & estilsHref & " target=""" & IIf(target = 1, "_blank", "_self") & """ title=""" & GAIA.netejaHTML(textalternatiu) & IIf(target = 1, IIf(idioma = 1, " (nova finestra)", " (nueva ventana) "), "") & """>" & strContingut & "</a>"
    ...
    Public Shared Function posaFormat2(ByVal objConn As OleDbConnection, ByVal strContingut As String, ByVal css As String, ByVal enllaç As String, ByVal esImatge As String, ByVal width As String, ByVal tamanyH As Integer, ByVal tamanyV As Integer, ByVal estat As Integer, ByVal textalternatiu As String, ByVal tancaEnllaç As Boolean, ByVal heretaPropietatsWeb As Integer, ByVal target As Integer, ByVal rel As clsRelacio, ByVal relini As clsRelacio, ByVal idioma As Integer, ByVal esEML As String, ByVal botoMesImatge As String, ByVal titolContingut As String, ByVal plt As clsPlantilla2, ByVal pareTeEnllac As Boolean, ByRef cssGenerat As String) As String
        ...
        strCss = GAIA.trobaEstilCSS2(objConn, css, desfaseHoritzontal, heretaPropietatsWeb, strCssTamanyText, strCssSenseFons, esEML, posaWidthGAIA, "", posaFloatGAIA, estilsHref, esImatge)
            ...
            If InStr(strContingut, ".jpg") > 0 Or InStr(strContingut, ".swf") > 0 Or InStr(strContingut, "obreFitxer") > 0 Then
                ...
                If enllaç.Trim().Length > 0 Then
                    If botoMesImatge <> "" And esImatge Then
                        ...
                        target = 1
                    End If
                    posaFormat2 = "<a href=""" + enllaç + """  target=""" & IIf(target = 1, "_blank", "_self") & """ title=""" & GAIA.netejaHTML(textAlternatiuExtes) & IIf(target = 1, IIf(idioma = 1, " (nova finestra)", " (nueva ventana) "), "") & """ class="""">" & posaFormat2
                    tancaEnllaç = True
                    'el strcss només l'he de posar si és una imatge (no un swf)                 
                End If
                '******************************************************************
                ...
                Else
                    ...
                    If (esEML = "N") Then
                        ...
                        posaFormatJS = "<a href=""" & imatgeSenseRatio & """ target=""_blank"" title=""" & GAIA.netejaHTML(textalternatiu) & """ ><img src=""" & strContingut & """ alt=""" & textAlternatiuImatge & """ " & strCss & "/></a>"
        ...
        Else 'No és una imatge.
            If enllaç.Trim().Length > 0 Then
                ...
                If InStr(enllaç, "obreFitxer") > 0 Or InStr(enllaç, "visorImatges") > 0 Then 'si és un document l'obro en una finestra nova.                           
                    ...
                    strCss = ""
                    posaFormat2 = "<a href=""" & enllaç & """ " & estilsHref & " target=""_blank"" title=""" & GAIA.netejaHTML(textalternatiu) & IIf(idioma = 1, " (nova finestra) ", " (nueva ventana) ") & """>" & strContingut & "</a>"
                ...
                Else
                    ...
                    posaFormat2 = "<a href=""" & enllaç & """ " & estilsHref & " target=""" & IIf(target = 1, "_blank", "_self") & """ title=""" & GAIA.netejaHTML(textalternatiu) & IIf(target = 1, IIf(idioma = 1, " (nova finestra)", " (nueva ventana) "), "") & """>" & strContingut & "</a>"
        ...
        Else
            ...
            If rel.inpla > 0 Then
                ...
                If rel.inpla = 9 Then
                    ...
                    While rel.tipdsdes <> "fulla web" And codiRelacioAnt <> rel.incod
                        ...
                        rel = GAIA.obtenirRelacioSuperior(objConn, rel)
                    End While
                    If rel.tipdsdes = "fulla web" Then
                        GAIA.bdR(objConn, "SELECT WEBDSPLA FROM METLWEB  WITH(NOLOCK) WHERE WEBINNOD LIKE '" + rel.infil.ToString() + "' UNION SELECT WEBDSPLA FROM METLWEB2  WITH(NOLOCK) WHERE WEBINNOD LIKE '" + rel.infil.ToString() + "'", DS)
                        If DS.Tables(0).Rows.Count > 0 Then
                            dbRow = DS.Tables(0).Rows(0)
            ...
            Else
                ...
                While descTipusNodeSUP <> "fulla web" And descTipusNodeSUP <> "node web" And descTipusNodeSUP <> "node estructura" And codiRelacioAnt <> codiRelacioPare And strPlantilla = "" And codiRelacioPare <> 0
                    ...
                    relPare = GAIA.obtenirRelacioSuperior(objConn, relPare)
                    ...
                    If codiRelacioPare = 0 Then
                        ...
                        If codiRelacioAnt <> codiRelacioPare Then
                            plantillaPare = GAIA.plantillaPerDefecte(objConn, relPare, idioma)
                            If plantillaPare > 0 Then
                                GAIA.bdR(objConn, "SELECT PLTDSPLT,RELCDEST  FROM METLPLT WITH(NOLOCK) , METLREL  WITH(NOLOCK) WHERE RELINCOD=" + codiRelacio.ToString() + " AND  PLTINNOD=" + plantillaPare.ToString(), DS)
                ...
                Else
                    If descTipusNodeSUP = "node web" And plantillaPerDefecte = 0 Then
                        ...
                        Else
                            ...
                            GAIA.bdR(objConn, "SELECT NWEDSPLA FROM METLNWE WITH(NOLOCK) , METLREL WITH(NOLOCK)  WHERE RELINCOD=" + codiRelacioPlantilla.ToString() + " AND RELINFIL=NWEINNOD AND NWEINIDI=" + idioma.ToString() + " AND RELCDSIT<98", DS)
                            If DS.Tables(0).Rows.Count > 0 Then
                                dbRow = DS.Tables(0).Rows(0)
                ...
                If plantillaPerDefecte = 0 Then
                    If descTipusNode = "fulla document" Then
                        GAIA.bdR(objConn, "SELECT TDOINPLA FROM METLTDO WITH(NOLOCK) , METLREL WITH(NOLOCK) , METLDOC WITH(NOLOCK)  WHERE RELINCOD=" & codiRelacio.ToString() & " AND RELINFIL=DOCINNOD  AND DOCINTDO=TDOCDTDO AND RELCDSIT<98", DS)
                        If DS.Tables(0).Rows.Count > 0 Then
                            dbRow = DS.Tables(0).Rows(0)
                            plantillaPerDefecte = dbRow("TDOINPLA")
                        End If
                    Else
                        GAIA.bdR(objConn, "SELECT TBLINNOD FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) , METLTBL WITH(NOLOCK)  WHERE RELINCOD=" & codiRelacio.ToString() & " AND RELINFIL=NODINNOD AND NODCDTIP=TBLINTFU AND RELCDSIT<98", DS)
                        If DS.Tables(0).Rows.Count > 0 Then
                            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Function assignaAutomaticamentCella(ByVal objConn As OleDbConnection, ByVal tipusNodeAMoure As Integer, ByVal rel As clsRelacio, ByVal forcarCella As Integer, ByVal idioma As Integer, ByRef cellaAutolink As Integer, ByVal posicioEstructura As Integer) As Integer
        ...
        Else
            ...
            Select Case descTipus.Trim
                Case "arbre web"
                    GAIA.bdR(objConn, "SELECT AWEDSTCO as tipusContingut,''  as plantillaAutoLink  FROM METLAWE WITH(NOLOCK)  WHERE AWEINNOD=" + nroNodeDesti.ToString() + " AND AWEINIDI=" + idioma.ToString(), DS)
                Case "node web"
                    GAIA.bdR(objConn, "SELECT NWEDSTCO as tipusContingut,''  as plantillaAutoLink FROM METLNWE  WITH(NOLOCK) WHERE NWEINNOD=" + nroNodeDesti.ToString() + " AND NWEINIDI=" + idioma.ToString(), DS)

                Case "fulla web"
                    GAIA.bdR(objConn, "SELECT WEBDSTCO as tipusContingut, ''  as plantillaAutoLink FROM METLWEB WITH(NOLOCK)  WHERE WEBINNOD=" + nroNodeDesti.ToString() + " AND WEBINIDI=" + idioma.ToString(), DS)
                Case Else ' si el destí no és una fulla web serà un objecte amb una plantilla per defecte o bé donada per una pag web.			
                    GAIA.bdR(objConn, "SELECT PLTDSTCO AS tipusContingut, PLTCDPAL as plantillaAutoLink FROM METLPLT  WITH(NOLOCK) WHERE PLTINNOD=" + plantillaPerDefecte(objConn, rel, idioma).ToString(), DS)
            End Select
            ...
            If DS.Tables(0).Rows.Count > 0 Then
                ...
                If strTmp.Trim().length > 0 Then
                    ...
                    Else
                        ...
                        Dim dbRow2 As DataRow
                        DS2 = New DataSet()
                        GAIA.bdR(objConn, "SELECT PLTDSTCO AS tipusContingut, PLTCDPAL as plantillaAutoLink FROM METLPLT  WITH(NOLOCK) WHERE PLTINNOD=" + plantillaAL.ToString(), DS2)
                        dbRow2 = DS2.Tables(0).Rows(0)
    ...
    Public Shared Function trobaCellaAutolink(ByVal objConn As OleDbConnection, ByVal tipusNodeAMoure As Integer, ByVal rel As clsRelacio, ByVal forcarCella As Integer, ByVal idioma As Integer) As Integer
        ...
        Else
            ...
            codiRelacioAnt = 0
            codiTipusFULLAWEB = GAIA.tipusNodeByTxt(objConn, "fulla web")
            ...
            If codiTipusNodeSUP = codiTipusFULLAWEB Then
                relAL = GAIA.obtenirRelacioAL(objConn, relPare)
    ...
    Public Shared Function trobaEstilCSS(ByVal objConn As OleDbConnection, ByVal codiEstilCss As String, ByRef desfaseHoritzontal As Integer, ByVal heretaPropietatsWeb As Integer, ByRef strCSSAmbtamanyText As String, ByRef strCSSSenseFons As String, ByVal esEMl As String, ByRef PosaWidthGAIA As Integer, ByRef posaVoreres As String, ByRef posaFloatGaia As Boolean, Optional ByRef estilsHref As String = "") As String
        ...
        If codiEstilCss.Replace("0", "").Replace(",", "").Trim().Length <> 0 Then
            If codiEstilCss <> "undefined" Then
                ...
                GAIA.bdr(objConn, "SELECT CSSINCOD,CSSWNWIG, CSSINTIP, CSSDSTHO,  CSSDSTHP,CSSDSTXT, CSSDSCSS, CSSWNFLT FROM METLCSS  WITH(NOLOCK) WHERE CSSINCOD IN (" + codiEstilCss + ")", DS)
                For Each dbRow In DS.Tables(0).Rows
...
Public Shared Function esGAIA2(ByVal objConn As OleDbConnection, ByVal nroNode As Integer) As Boolean
...
strErr &= "1"
                GAIA.bdr(objConn, "SELECT * FROM METLCEL WITH(NOLOCK) WHERE CELINREL=" & rel.incod & " AND CELCDEST=" & est & " AND CELCDPLT=" & codiplantilla & " AND CELCDUSU=" & usuari & " AND  CELINIDI=" & codiIdioma, DS)
strErr &= "2"
                If DS.Tables(0).Rows.Count > 0 Then
strErr &= "3"				
...
strErr &= "4"		
				If codiLCW <> 0 Then
					...
					strErr &= "5"
					GAIA.bdr(objConn, "SELECT * FROM METLCEL WITH(NOLOCK) WHERE CELINPAR LIKE '" & strPage & "'", DS)
					strErr &= "6"
					If DS.Tables(0).Rows.Count > 0 Then
					strErr &= "7"					
						dbRow = DS.Tables(0).Rows(0)
						...
						If dbRow("CELDTCAD") <> "1/1/1900" And Now > dbRow("CELDTCAD") Then
							If codiLCW <> 0 Then
								GAIA.esborrarCelles(nothing,"CELINLCW=" & codiLCW)
				...
				Else
					...
					If InStr(strPage, "contingutCella") > 0 Then
						strErr &= "9"
						GAIA.bdr(objConn, "SELECT * FROM METLCEL WITH(NOLOCK) WHERE CELINPAR='" & strPage & "'", DS)
						strErr &= "a"
						If DS.Tables(0).Rows.Count > 0 Then
							dbRow = DS.Tables(0).Rows(0)
							If dbRow("CELDTCAD") <> "1/1/1900" And Now > dbRow("CELDTCAD") Then
...
strErr &= "e"								
    ...
    Public Shared Sub log(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiUsuari As String, ByVal descripcio As String, ByVal tipusAccio As Integer)
        GAIA.log(objConn, rel, codiUsuari, descripcio, tipusAccio, 0)
    End Sub
    ...
    Public Shared Sub log(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiUsuari As String, ByVal descripcio As String, ByVal tipusAccio As Integer, ByVal node As Integer)
        ...
        If rel.incod = 0 Then
            GAIA.bdR(objConn, "SELECT * FROM METLNOD WITH(NOLOCK)  WHERE NODINNOD=" & node, DS)
            If DS.Tables(0).Rows.Count > 0 Then
                dbRow = DS.Tables(0).Rows(0)
        ...
        Else
            If rel.cdher.Length > 0 Then
                GAIA.bdR(objConn, "SELECT * FROM METLNOD WITH(NOLOCK) ,METLREL  WITH(NOLOCK) WHERE NODINNOD IN (" + rel.cdher.Substring(1).Replace("_", ",") + ") AND RELINFIL=NODINNOD ORDER BY LEN(RELCDHER)", DS)
                For Each dbRow In DS.Tables(0).Rows
                    descripcioObjecteLog += ">" + dbRow("NODDSTXT")
                Next dbRow
            End If
        ...
        descripcio = GAIA.obtenirTextCodi(objConn, 2, tipusAccio) + " sobre el node: " + descripcioObjecteLog + IIf(descripcio.Length > 0, " --> ", " ") + descripcio

        GAIA.bdSR(objConn, "INSERT INTO METLLOG (LOGCDREL, LOGDTDAT, LOGTPTIP, LOGCDUSR, LOGDSTXT, LOGINNOD) VALUES (" & rel.incod & ",'" + Now.ToString() + "'," + tipusAccio.ToString() + ",'" + codiUsuari + "','" + descripcio.Replace("'", "''") + "'," + node.ToString() + ")")
    ...
    Public Shared Function obtenirTextCodi(ByVal objConn As OleDbConnection, ByVal tipusCodi As Integer, ByVal codi As Integer) As String
        ...
        DS = New DataSet()
        GAIA.bdR(objConn, "SELECT CODDSTXT FROM METLCOD  WITH(NOLOCK) WHERE CODINTIP=" + tipusCodi.ToString() + " AND CODINCOD=" + codi.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Sub obtenirCodiCircuitFullaWeb(ByVal objConn As OleDbConnection, ByVal codiNode As Integer, ByVal codiIdioma As Integer, ByRef circuitPublicacio As Integer, ByRef circuitCaducitat As Integer)
        ...
        DS = New DataSet()
        GAIA.bdR(objConn, "SELECT WEBDSCIP,WEBDSCIC  FROM METLWEB  WITH(NOLOCK) WHERE WEBINNOD=" + codiNode.ToString() + " AND WEBINIDI=" + codiIdioma.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Function obtenirPasDeCircuitRelacio(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio) As clsRelacio
        ...
        DS = New DataSet()
        GAIA.bdR(objConn, "SELECT PCRCDFCI FROM METLPCR  WITH(NOLOCK) WHERE PCRINCOD=" + rel.incod.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
    ...
    Public Shared Sub tractarPasCircuit(ByVal objConn As OleDbConnection, ByVal relContingut As clsRelacio, ByVal relCircuit As clsRelacio)
        ...
        DS = New DataSet()
        GAIA.bdR(objConn, "SELECT FCIWNTMD, FCIWNTMH, PCRDTDAT, PCRCDRCI FROM METLFCI WITH(NOLOCK) , METLPCR WITH(NOLOCK) , METLREL WITH(NOLOCK)  WHERE RELINCOD=" + codiRelacioCircuit.ToString() + " AND PCRINCOD=" + codiRelacioContingut.ToString() + " AND FCIINNOD=RELINFIL  AND RELCDSIT<98", DS)
        ...
        If DS.Tables(0).Rows.Count > 0 Then
            ...
            If DateDiff("n", CDate(dbRow("PCRDTDAT")), Now) >= hores Then 'S'ha superat el temps màxim en aquest pas
                ...
                GAIA.enviaMissatge(objConn, relContingut, "S'ha superat el temps màxim d'espera", "Compte: s'ha superat el temps màxim en aquest pas", codiRelacioCircuit, "N", "N", "S", "S", arrayDestinataris, dbRow("PCRCDRCI"))
            End If
        End If
        DS.Dispose()
    ...
    Public Shared Sub iniciarCircuit(ByVal objConn As OleDbConnection, ByVal relContingut As clsRelacio, ByVal codiRelacioNodeCircuit As Integer, ByVal codiUsuari As Integer, ByVal codiRelacioNodePasCircuit As Integer, ByVal relContingutIniciador As clsRelacio)
        ...
        If DS.Tables(0).Rows.Count > 0 Then
            ...
            GAIA.bdSR(objConn, "INSERT INTO METLPCR (PCRINCOD,PCRINORG,PCRDTDAT,PCRCDFCI, PCRCDRCI) VALUES (" + codiRelacioContingut.ToString() + "," + codiUsuari.ToString() + ",'" + Now.ToString() + "'," + relFullaCircuit.incod.ToString() + "," + codiRelacioContingutIniciador.ToString() + ")")

            GAIA.trobarTextCircuit(objConn, relFullaCircuit, 0, misTitolCircuit, misSolicitudAprovacio, misAprovacioPeticionari, misCancelacioPeticionari, misModificacioPeticionari)
            ...
            'envio el missatge				
            GAIA.enviaMissatge(objConn, relContingut, misTitolCircuit, misSolicitudAprovacio, codiRelacioFullaCircuit, "S", "S", "N", "S", arrayDestinataris, relContingutIniciador)
                ...
                For Each destinatari In arrayDestinataris
                    If destinatari <> 0 Then
                        ...
                        If descTipus = "fulla organigrama" Then
                            ...
                            GAIA.insertaNodeArbrePersonal(objConn, GAIA.tipusNodeByTxt(objConn, "fulla missatge"), codiNode, destinatari, "Missatges")
                        Else
    ...
    Public Shared Sub anularPasCircuit(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio)
        ...
        GAIA.bdSR(objConn, "DELETE FROM METLPCR WHERE PCRINCOD=" + rel.incod.ToString())
        'Busco tots els missatges associats a aquest pas del circuit i els esborro
        GAIA.bdSR(objConn, "DELETE FROM METLREL WHERE RELINCOD IN (SELECT RELINCOD FROM METLREL,METLMIS WHERE RELINFIL=MISINNOD AND MISINCOD=" + rel.incod.ToString() + ")")
        GAIA.bdSR(objConn, "DELETE FROM METLNOD WHERE NODINNOD IN (SELECT MISINNOD FROM METLMIS WHERE MISINCOD=" + rel.incod.ToString() + ")")
        GAIA.bdSR(objConn, "DELETE FROM METLMIS WHERE MISINCOD=" + rel.incod.ToString())
                    ...
                    If (plantilla.arrayLCW(index).Trim().Length > 0) Then
                        ...
                        For Each item In plantilla.arrayLCW(index).Split("|")
                            If width = "" Then
                                width = "0"
                            End If
                            lcwANT = lcwANT + GAIA.trobaCodiWeb(objConn, plantilla, item, idioma, rel, relIni, forçarCodiAnt, index, "", width, codiUsuari, css, tagsMeta)
                            ...
                            If dataIni <= CDate(dataSimulacio) And dataFi > CDate(dataSimulacio) Then
                                ...
                                Else 'no és imatge					
                                                ...
                                                Select Case plantilla.arrayEnllaços(index).Substring(plantilla.arrayEnllaços(index).IndexOf(".") + 1)
                                                    Case "IMATGE"
                                                        ...
                                                        If plantilla.arrayPLTDSPLT(index).Length = 0 Then
                                                            GAIA.f_logError(objConn, "PintarContingut", "", "S'ha intentat representar una imatge o document dins d'un contingut sense plantilla secund&agrave;ria per representar-lo")
                                                        End If
                                                        codiRelacioArxiuPerVisorImatges = dbRow("RELINCOD")
                                    ...
                                    If camp.Trim() <> "" Then
                                        ...
                                        If plantilla.arrayDSNUM(index) > 0 Then
                                            strContingut = GAIA.tallaText(dbRow(camp), plantilla.arrayDSNUM(index)) & "..."
                                        Else
                                            If Not IsDBNull(dbRow(camp)) Then
                                                strContingut = dbRow(camp)
                                            End If
                                ...
                                If (rel.tipdsdes = "fulla document" And idiomaDiferent) Or (Not idiomaDiferent) Then
                                    If (enllaç + "").Trim().Length > 0 Then
                                        ...
                                        If codiRelacioArxiuPerVisorImatges = 0 Then
                                            ...
                                            celdaFormateada += GAIA.posaFormat(objConn, strContingut, plantilla.arrayCSSPlantilla(index), enllaç, esImatge, width, tamanyH, tamanyV, estat, textalternatiu, 1, heretaPropietatsWeb, target, rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, pareTeEnllac, css)
                                        End If
                                    ...
                                    Else
                                        ...
                                        End If
                                        celdaFormateada += GAIA.posaFormat(objConn, strContingut, plantilla.arrayCSSPlantilla(index), "", esImatge, width, tamanyH, tamanyV, estat, textalternatiu, 1, heretaPropietatsWeb, target, rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, pareTeEnllac, css)
                                    End If
                                End If 'comprobació de l'idioma i del tipus de node
                            End If
                        End If 'TINC CONTINGUT
                            ...
                            If ((plantilla.arrayEnllaços(index) = "IMATGE" And plantillaTMP.innod = 0) Or (plantilla.arrayEnllaços(index) = "DOCUMENT")) And plantilla.arrayEnllaços(index) <> plantilla.arrayCampsPlantilla(index) Then
                                ...
                                If ds3.Tables(0).Rows.Count > 0 Then
                                    ...
                                    If pospunt > 0 Then
                                        ...
                                        strvoid = GAIA.trobaEstilCSS(objConn, plantilla.arrayCSSPlantilla(index), desfaseHoritzontal, heretaPropietatsWeb, strvoid, estilsCSSSenseFons, esEML, posaWidthGAIA, "", posafloatGAIA)
                            ...
                            Else
                                ...
                                Select Case plantilla.arrayEnllaços(index)
                                    ...
                                    Case "METLLNK.LNKDSLNK"
                                        ...
                                        End If
                                    Case "METLNOT.NOTDSLNK"
                                        GAIA.bdr(objConn, "SELECT " & plantilla.arrayEnllaços(index) & " FROM METLNOT WITH(NOLOCK) WHERE NOTINNOD=" & rel.infil & " AND NOTINIDI=" & idioma, ds3)
                                        If ds3.Tables(0).Rows.Count > 0 Then
                                            dbrow3 = ds3.Tables(0).Rows(0)
                                            If titolContingut = "" Then
                                                titolContingut = GAIA.obtenirTitolContingut(objConn, rel, idioma)
                                    ...
                                    Case "METLINF.INFDSLNK"
                                        ...
                                        End If
                                    Case "METLAGD.AGDDSLNK"
                                            GAIA.bdr(objConn, "SELECT " & plantilla.arrayEnllaços(index) & " FROM METLAGD WITH(NOLOCK) WHERE AGDINNOD=" & rel.infil & " AND AGDINIDI=" & idioma, ds3)
                                            If ds3.Tables(0).Rows.Count > 0 Then
                                                dbrow3 = ds3.Tables(0).Rows(0)
                                                If titolContingut = "" Then
                                                    titolContingut = GAIA.obtenirTitolContingut(objConn, rel, idioma)
                                    ...
                                    Case "AUTO-ENLLAÇ"
                                            ...
                                            If pospunt > 0 Then 'si no hi ha imatge no fem l'autoenllaç
                                                ...
                                                End If                                            
												If esEML = "N" Then
                                                celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTMp) & """ " & estilsCSSSenseFons & IIf(target = 1, " target=""_blank"" ", " target=""_self"" ") & " title=""" & GAIA.obtenirTitolContingut(objConn, rel, idioma) & """>" & celdaFormateada.Substring(pospunt - 1, InStrRev(celdaFormateada, "</div>") - pospunt) & "</a></div>"
												Else
                                                celdaFormateada = celdaFormateada.Substring(0, pospunt - 1) & "<a href=""" & GAIA.netejaHTML(strTMp) & """ " & estilsCSSSenseFons & IIf(target = 1, " target=""_blank"" ", " target=""_self"" ") & " title=""" & GAIA.obtenirTitolContingut(objConn, rel, idioma) & """>" & celdaFormateada.Substring(pospunt - 1) & "</a>"
												End If
                                            End If
                                End Select
                            End If
                    ...
                    If codiRelacioArxiuPerVisorImatges = 0 And plantilla.arrayPAL.Length > index Then
                        If (plantilla.arrayPAL(index).Trim.Length > 0) Then
                            ...
                            If plantilla.arrayALK(index).Length > 0 Then
                                If esEML = "N" Then
                                    celdaFormateada += GAIA.posaFormat(objConn, plantilla.arrayALK(index) & "<span class=""visibilidadoculta"">(" & titolContingut & ")</span>", plantilla.arrayCSSPlantilla(index), fitxerAutoLink, esImatge, width, tamanyH, tamanyV, estat, titolContingut, tancaEnllaç, heretaPropietatsWeb, plantilla.arrayALF(index), rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, pareTeEnllac, css)
                                Else
                                    celdaFormateada += GAIA.posaFormat(objConn, plantilla.arrayALK(index), plantilla.arrayCSSPlantilla(index), fitxerAutoLink, esImatge, width, tamanyH, tamanyV, estat, titolContingut, tancaEnllaç, heretaPropietatsWeb, plantilla.arrayALF(index), rel, relIni, idioma, esEML, botoMesImatge, titolContingut, plantilla, pareTeEnllac, css)
...
if obrirNovaFinestra THEN crida = "<script language=""JavaScript"" type=""text/javascript"">window.open(""" & crida &  """)</script>"
    ...
    Public Shared Sub aprovarPeticio(ByVal objConn As OleDbConnection, ByVal relContingut As clsRelacio, ByVal relCircuit As clsRelacio, ByVal codiNodePasCircuit As Integer, ByVal codiUsuariPeticionari As Integer, ByVal relContingutIniciador As clsRelacio)
        ...
        GAIA.anularPasCircuit(objConn, relContingut)
        seguentPasCircuit = GAIA.obtenirGermaSeguentRelacio(objConn, relCircuit)

        If seguentPasCircuit > 0 Then ' hi ha següent pas del circuit
            ' Faig el canvi d'estat i envio missatges als nous aprovadors
            GAIA.iniciarCircuit(objConn, relContingut, 0, codiUsuariPeticionari, seguentPasCircuit, relContingutIniciador)
        Else    'no hi ha seguent pas del circuit		
            ...
            textAprovacioUsuariPeticionari = GAIA.obtenirValorRegistreCircuit(objConn, codiNodePasCircuit, "FCIDSMA1")
            If textAprovacioUsuariPeticionari <> "-1" Then
                If arrayDestinataris.Length > 0 Then
                    If arrayDestinataris(0) = 0 And arrayDestinataris.Length = 1 Then
                        GAIA.enviaMissatge(objConn, relContingut, textAprovacioUsuariPeticionari, textAprovacioUsuariPeticionari, codiNodePasCircuit, "N", "N", "S", "S", arrayDestinataris, relContingutIniciador)

                        GAIA.log(objConn, relContingut, arrayDestinataris(0), textAprovacioUsuariPeticionari, TAAPROVAR)
                    End If
                End If
            Else
            ...
            GAIA.obtenirAccioNodeCircuit(objConn, GAIA.obtenirRelacioSuperior(objConn, relCircuit), accAprovacio, accDenegacio)
            ...
            If accAprovacio > 0 Then
                Select Case accAprovacio
                    Case 1 'Publicar Continguts
                        ...
                        GAIA.canviEstatRelacio(objConn, relContingut, relContingut.cdsit, 0)
                        Dim heretaPropietatsWeb As Integer = GAIA.heretarPropietatsWeb(objConn, relContingutIniciador.incod, width)
                        GAIA.obreFullaWeb(objConn, relContingutIniciador, html, css, heretaPropietatsWeb, width, 500, "", fdesti, urlDesti, 1, llistaDocuments, 1, 1, relContingutIniciador, Now, 0, titol, 1, estilbody, esform, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, True)
    ...
    Public Shared Sub denegarPeticio(ByVal objConn As OleDbConnection, ByVal relContingut As clsRelacio, ByVal relCircuit As clsRelacio, ByVal codiNodePasCircuit As Integer, ByVal codiUsuariPeticionari As Integer, ByVal relContingutIniciador As clsRelacio)
        ...
        ReDim arrayDestinataris(0)
        arrayDestinataris(0) = codiUsuariPeticionari
        textCancelacioUsuariPeticionari = GAIA.obtenirValorRegistreCircuit(objConn, codiNodePasCircuit, "FCIDSMIC")

        If textCancelacioUsuariPeticionari <> "-1" Then
            If arrayDestinataris.Length > 0 Then
                If arrayDestinataris(0) = 0 And arrayDestinataris.Length = 1 Then
                    GAIA.enviaMissatge(objConn, relContingut, textCancelacioUsuariPeticionari, textCancelacioUsuariPeticionari, codiNodePasCircuit, "N", "N", "S", "S", arrayDestinataris, relContingutIniciador)
                    GAIA.log(objConn, relContingut, arrayDestinataris(0), textCancelacioUsuariPeticionari, TADENEGAR)
                End If
            End If
        Else
            'No hi ha text previst. NO envio res.
        ...
        If accDenegacio > 0 Then
            Select Case accDenegacio
                Case 2 'CaducarContinguts
                    ...
                    GAIA.caducarContinguts(objConn, relContingut, codiUsuariPeticionari)
    ...
    Public Shared Function obtenirValorRegistreCircuit(ByVal objConn As OleDbConnection, ByVal codiNodePasCircuit As Integer, ByVal camp As String) As String
        ...
        GAIA.bdR(objConn, "SELECT " + camp + " FROM METLFCI WITH(NOLOCK) WHERE  FCIINNOD=" + codiNodePasCircuit.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Sub obtenirAccioNodeCircuit(ByVal objConn As OleDbConnection, ByVal relNodeCircuit As clsRelacio, ByRef accAprovacio As Integer, ByRef accDenegacio As Integer)
        ...
        GAIA.bdR(objConn, "SELECT * FROM METLNCI  WITH(NOLOCK) WHERE NCIINNOD=" + relNodeCircuit.infil.ToString(), DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
    ...
    Public Shared Sub caducarContinguts(ByVal objConn As OleDbConnection, ByVal relContingut As clsRelacio, ByVal codiUsuari As Integer)
        ...
        GAIA.esborrarNode(objConn, relContingut.infil, relContingut, 1, codiUsuari, 1, ctESBORRATCADUCAT)
    ...
    Public Shared Sub f_establirPermisos(ByVal objConn As OleDbConnection, ByVal relacio As Integer, ByVal thumbnail As String, ByVal fitxerServidor As String)
        ...
        If 2 = 3 Then
            ...
            Try
                GAIA.bdR(objConn, "SELECT RELDSFIT,RELCDHER FROM METLREL WITH(NOLOCK) WHERE RELINCOD = " & relacio, ds)
                If ds.Tables(0).Rows.Count() > 0 Then
                ...
                End If
                nomNode = GAIA.obtenirNomNode(objConn, nodeArrel)

                If Trim(nomNode) = "Intranet" Then
                    GAIA.bdR(objConn, "SELECT DISTINCT PELINORG,isnull(FORDSWIN,'') AS USUARI,MIN(PERCDTIP) AS PERMIS " & _
                          " FROM METLPER WITH(NOLOCK) LEFT JOIN METLPEL WITH(NOLOCK) ON PERCDREL = METLPEL.PELCDREL LEFT JOIN METLFOR  WITH(NOLOCK) " & _
                          " ON METLPEL.PELINORG=METLFOR.FORINNOD WHERE PELCDREL=" & relacio & _
                          " AND (isnull(FORDSWIN,'') <> '' OR PELINORG = 115969) " & _
                    ...
                    For Each dbRow In ds.Tables(0).Rows
                        ...
                        If dbRow("PELINORG") = "115969" Then
                            GAIA.SetPermissions(nomFitxerFisic, "Todos", permis)
                        End If
                        If Trim(dbRow("USUARI")) <> "" Then
                            GAIA.SetPermissions(nomFitxerFisic, "LH\" & Trim(dbRow("USUARI")), permis)
    ...
    Public Shared Sub f_logError(ByVal objConn As OleDbConnection, ByVal origen As String, ByVal ex_source As String, ByVal text As String)
        GAIA.bdSR(objConn, "INSERT INTO METLERR (ERRTPORG,ERRDSSRC,ERRDSDES) VALUES ('" & _
    ...
    Public Shared function ftpCopiarFitxer(ByVal objConn As OleDbConnection, ByVal servidor As String, ByVal usuari As String, ByVal pwd As String, ByVal fOrigen As String, ByVal fdesti As String, ByVal relacio As Integer, ByVal thumbnail As String, ByVal ftpsession As Session, Optional ByVal pathDestiArrel As String = "") As String
		...
		Catch ex As Exception
        	GAIA.debug(objConn, "error al ftp: Forigen=" & forigen  & "--Fdesti=" & fdesti & "-- servidor=" & servidor & "-- dades d'error: " & ex.Source  & "--" & ex.Message & "strERr=" & strErr )
    ...
    Public Shared Sub ftpEsborrarFitxer(ByVal objConn As OleDbConnection, ByVal servidor As String, ByVal usuariFTP As String, ByVal pwdFTP As String, ByVal fitxer As String, ByVal codiUsuari As Integer, ByVal rel As clsRelacio, ByVal borrarFitxerDefinitivament As Char)
		...
		Try
			...
			GAIA.log(objConn, rel, codiUsuari, "Fitxer:" + rel.noddstxt + "(" + fitxer + ") esborrat del servidor Intranet", TAESBORRAR)
			If borrarFitxerDefinitivament = "S" Then
				File.Delete("e:\docs\GAIA\" & fitxer.Substring(InStrRev(fitxer, "/")))
				GAIA.bdSR(objConn, "UPDATE METLDOC SET DOCDSFIT='' WHERE DOCINNOD=" & rel.infil.ToString())
			End If
		...
		Try
			...
			GAIA.log(objConn, rel, codiUsuari, "Fitxer:" + rel.noddstxt + "(" + fitxer + ")  esborrat del servidor:" + servidor, TAESBORRAR)
    ...
    Public Shared Sub nouFitxer(ByRef nomFitxer As String, ByRef tipusDocument As String, ByRef txtThor As String, ByRef txtTver As String, ByRef txtTamany As String, ByRef postedFile As HttpPostedFile)
        ...
        Dim nomFitxerImatge1366px As String
        ...
        If Not (postedFile Is Nothing) Then
            ...
            If nomFitxer.Length = 0 Then
                tipusDocument = GAIA.trobaTipusDocument(objconn, postedFile.ContentType)
                ...
                nomFitxerImatge1366px = nomAleatori + "P1366"
            Else
                'He d'esborrar el fitxer actual (i l'imatge petita si hi hagués)
                File.Delete(savePath + nomFitxer)

                sufixVell = nomFitxer.Substring(InStrRev(nomFitxer, ".") - 1)
                nomFitxer = nomFitxer.Substring(0, InStrRev(nomFitxer, ".") - 1)

                nomFitxerImatgePetita = nomFitxer + "P" + sufixVell
                nomFitxerImatge100px = nomFitxer + "P100" + sufixVell
                nomFitxerImatge1366px = nomFitxer + "P1366" + sufixVell

                If File.Exists(savePath + nomFitxerImatgePetita) Then
                    File.Delete(savePath + nomFitxerImatgePetita)
                End If
                If File.Exists(savePath + nomFitxerImatge100px) Then
                    File.Delete(savePath + nomFitxerImatge100px)
                End If
                If File.Exists(savePath + nomFitxerImatge1366px) Then
                    File.Delete(savePath + nomFitxerImatge1366px)
                End If

                'Si el nou fitxer que substitueix a l'anterior no és una imatge em quedo la seva extensió
                If InStr(UCase(postedFile.ContentType), "IMAGE") > 0 Then
                    sufixVell = filename.Substring(InStrRev(filename, ".") - 1)
                    Select Case UCase(sufixVell)
                        Case ".PNG"
                            sufix = ".png"
                        Case ".GIF"
            ...
            If InStr(UCase(postedFile.ContentType), "IMAGE") > 0 Then
                ...
                If UCase(sufixVell) <> ".JPG" And UCase(sufixVell) <> ".JPEG" And UCase(sufixVell) <> ".PNG" And UCase(sufixVell) <> ".GIF" Then
            ...
            If InStr(UCase(postedFile.ContentType), "IMAGE") > 0 Then
                ...
                Dim tamanyHorMitja As Integer = 1366

                nomFitxerImatgePetita = nomFitxer + "P"
                nomFitxerImatge100px = nomFitxer + "P100"
                nomFitxerImatge1366px = nomFitxer + "P1366"
                ...
                If sufix <> ".gif" Then
                    ...
                    thumbNailImg.Save(savePath + nomFitxerImatgePetita + sufix)

                    thumbNailImg = fullSizeImg.GetThumbnailImage(tamanyHorPetit2, (tver * 100) / ((thor * 100) / tamanyHorPetit2), dummyCallBack, IntPtr.Zero)
                    thumbNailImg.Save(savePath + nomFitxerImatge100px + sufix)

                    thumbNailImg = fullSizeImg.GetThumbnailImage(tamanyHorMitja, (tver * 100) / ((thor * 100) / tamanyHorMitja), dummyCallBack, IntPtr.Zero)
                    thumbNailImg.Save(savePath + nomFitxerImatge1366px + sufix)
    ...
    Public Shared Sub trobaServidorDesti(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByRef servidor As String, ByRef usuari As String, ByRef pwd As String, ByRef pathDesti As String, ByRef pathDocsDesti As String, ByRef port As String, ByRef URL As String)
        ...
        Dim dbRow As DataRow
        DS = New DataSet()
        'Teresa: añado la UNION con METLAWE2
        GAIA.bdr(objConn, "SELECT SERDSDIR,  SERDSUSR, SERDSPWD, SERDSCAM, SERDSPRT, SERDSURL, AWEDSROT, AWEDSDOC FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE  WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_',SUBSTRING(METLREL.RELCDHER,2,LEN(METLREL.RELCDHER)))-1) = METLAWE.AWEINNOD INNER JOIN METLSER WITH(NOLOCK) ON METLAWE.AWEDSSER = METLSER.SERINCOD WHERE (METLREL.RELINCOD=" + rel.incod.ToString() + ") UNION ALL SELECT SERDSDIR,  SERDSUSR, SERDSPWD, SERDSCAM, SERDSPRT, SERDSURL, AWEDSROT, AWEDSDOC FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE2  WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_',SUBSTRING(METLREL.RELCDHER,2,LEN(METLREL.RELCDHER)))-1) = METLAWE2.AWEINNOD INNER JOIN METLSER WITH(NOLOCK) ON METLAWE2.AWEDSSER = METLSER.SERINCOD WHERE (METLREL.RELINCOD=" + rel.incod.ToString() + ")", DS)
        If DS.Tables(0).Rows.Count > 0 Then
            dbRow = DS.Tables(0).Rows(0)
	...
	' 1-2-2019: adaptada a GAIA2
	' Se usa para crear la url de destino de las fullas link a partir de una relación
    Public Shared Function trobaURLDesti(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio) As String
        Dim DS As DataSet
        DS = New DataSet()
        Dim URL As String = ""

        GAIA.bdr(objConn, "SELECT  RELCDHER,SERDSURL FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_',SUBSTRING(METLREL.RELCDHER,2,LEN(METLREL.RELCDHER)))-1) = METLAWE.AWEINNOD INNER JOIN METLSER WITH(NOLOCK) ON METLAWE.AWEDSSER = METLSER.SERINCOD WHERE (METLREL.RELINCOD=" + rel.incod.ToString() + ") UNION SELECT  RELCDHER,SERDSURL FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE2 WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_',SUBSTRING(METLREL.RELCDHER,2,LEN(METLREL.RELCDHER)))-1) = METLAWE2.AWEINNOD INNER JOIN METLSER WITH(NOLOCK) ON METLAWE2.AWEDSSER = METLSER.SERINCOD WHERE (METLREL.RELINCOD=" + rel.incod.ToString() + ")", DS)
   ...
   Public Shared Function obtenirEnllacSimple(ByVal objconn As OleDbConnection, ByVal codiEnllaç As Integer, ByVal idioma As Integer, ByRef target As Integer, ByVal urlDesti As String) As String
        ...
        If DS.Tables(0).Rows.Count > 0 Then
            ...
            If enllaç = "" Then
                If IsDBNull(dbRow("DOCINNOD")) Then
                    ...
                    If IsDBNull(dbRow("LNKCDREL")) Then
                        ...
                        ds2 = New DataSet()
                        GAIA.bdr(objconn, "SELECT RELCDRSU, RELINFIL FROM METLREL WHERE RELINPAR=" & codiEnllaç & " AND RELCDSIT=97 ORDER BY RELCDORD", ds2)
                        If DS2.Tables(0).Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        GAIA.obtenirEnllacContingut(objconn, relContingut, idioma, relContingut.tipintip, "", "", 0, 0, "", 0, "", enllaç, params, "")
            ...
            If InStr(enllaç, "http") = 0 Then
                ...
                If Not relEnllaç = 0 Then
                    relContingut.bdget(objconn, relEnllaç)
                    urlDestiEnllaç = GAIA.trobaURLDesti(objconn, relContingut)
                End If
    ...
    Public Shared Function preparaPagina(ByVal objConn As OleDbConnection, ByVal html As String, ByVal css As String, ByVal heretatPropietatsNodesSuperiors As Integer, ByVal titol As String, ByVal heretaPropietats As Integer, ByVal estilBody As String, ByVal esForm As String, ByVal esEML As String, ByVal esSSL As String, ByVal tagsMeta As String, ByRef htmlPeu As String, ByVal strCSSPantalla As String, ByVal strCSSImpressora As String, ByVal idioma As Integer, Optional ByVal urlDesti As String = "") As String
        ...
        End If
		'Que cada butlleti tenga una direccion unica con la fecha de publicación. 'URLDESTIBUTLLETIAMBLADATA' esta en el cap de cada butlleti
        If esEML = "S" Then
            preparaPagina = preparaPagina.Replace("URLDESTIBUTLLETIAMBLADATA", urlDesti)
        End If
    End Function 'preparaPagina

    '****************************************************************************************************************
    '	Funció: preparaPagina2 para GAIA2
    '	Entrada: 
    '		html: string amb el codi html que he de representar
    '		css:  string amb la definició de l'estructura de la pàgina
    '		heretatPropietatsNodesSuperiors: si 1 llavors heretarà les propietats dels nodes "node web" i "arbre web" superiors.	
    ...
    '		Retorna un string que consisteix en una pàgina web ben formada i accessible per que es pugui publicar (gravar)
    '****************************************************************************************************************	
    Public Shared Function preparaPagina2(ByVal objConn As OleDbConnection, ByVal html As String, ByVal css As String, ByVal heretatPropietatsNodesSuperiors As Integer, ByVal titol As String, ByVal heretaPropietats As Integer, ByVal estilBody As String, ByVal esForm As String, ByVal esEML As String, ByVal esSSL As String, ByVal tagsMeta As String, ByRef htmlPeu As String, ByVal strCSSPantalla As String, ByVal strCSSImpressora As String, ByVal idioma As Integer, Optional ByVal urlDesti As String = "") As String
        Dim textCSS As String = ""
        Dim arrayCSS1 As String()
        Dim arrayCSS2 As String()
        Dim item As String
        ...
        'Si és un correu poso un cap reduït
        If esEML = "S" Then
            preparaPagina2 = "<html><head runat=""server""><asp:Literal id=""lblhead"" runat=""server""></asp:Literal></head><body  id=""cosPagina"" style=""" + estilBody + """>" + html + "</body></html>"
        Else
            titol = GAIA.netejaHTML(titol)
            tagsMetaComuns &= "<meta charset=""UTF-8""/>"
            tagsMetaComuns &= "<meta name=""viewport"" content=""width=device-width, initial-scale=1, shrink-to-fit=no"">"
            tagsMetaComuns &= "<asp:Literal id=""lblhead"" runat=""server""></asp:Literal>"
            tagsMetaComuns &= "<meta http-equiv=""Content-Style-Type"" content=""text/css""/>"
            tagsMetaComuns &= "<meta http-equiv=""Content-Language"" content=""" + strIdioma + """/>"
            tagsMetaComuns &= "<meta http-equiv=""Content-Script-Type"" content=""text/javascript""/>"
            tagsMetaComuns &= "<meta name=""Author"" content=""Ajuntament de L'Hospitalet""/>"
            tagsMetaComuns &= "<meta name=""DC.Creator"" content=""Ajuntament de L'Hospitalet""/>"
            tagsMetaComuns &= "<meta name=""DC.Identifier"" content=""http://www.l-h.cat""/>"
            tagsMetaComuns &= "<meta id=""metatitle"" name=""title"" content=""" + titol + """ runat=""server"" />"
            tagsMetaComuns &= "<meta name=""DC.title"" content=""" + titol + """/>"
            tagsMetaComuns &= "<link rel=""schema.DC"" href=""http://purl.org/dc/elements/1.1/""/>"
            tagsMetaComuns += "<meta name=""DC.Language"" content=""" + strIdioma + """/>"
            tagsMetaComuns += "<meta name=""Copyright"" content=""http://www.l-h.cat/gdocs/d6779062.pdf""/>"
            tagsMeta = tagsMetaComuns & tagsMeta

            preparaPagina2 = "<!doctype html><html lang=""" & strIdioma & """ ><head runat=""server"">" & css & tagsMeta 
            preparaPagina2 &= textCSS & "<title>" & titol & "</title></head><body " & IIf(estilBody.Trim.Length > 0, " class=""" & estilBody & """", "") & ">" & html & htmlPeu & "</body></html>"
        End If

        'l'editor de texte enriquit que utilitzem posa <p> i <br /> i no ens interessa per l'accessibilitat.
        preparaPagina2 = preparaPagina2.Replace("<p>", "").Replace("</p>", "<br/>") '.replace("<br />","<br>").replace("<br/>","<br>")
        If esSSL = "S" Then
            preparaPagina2 = preparaPagina2.Replace("<img src=""http:", "<img src=""https:")
        End If

        'Que cada butlleti tenga una direccion unica con la fecha de publicación. 'URLDESTIBUTLLETIAMBLADATA' esta en el cap de cada butlleti
        If esEML = "S" Then
            preparaPagina2 = preparaPagina2.Replace("URLDESTIBUTLLETIAMBLADATA", urlDesti)
        End If
    End Function 'preparaPagina2
    ...
    Public Shared Function obreFulla(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal plantilla As clsPlantilla, ByVal idioma As Integer, ByVal codiUsuari As Integer, ByVal publicar As Integer, ByVal width As Double, ByVal node As Integer, ByVal dataSimulacio As DateTime, ByRef head As String, ByVal nrocontingut As Integer, ByVal escorreu As String) As String
		...
		IF  GAIA.contingutVisibleAInternet(objconn, rel.infil)=0 THEN
        ...
        If obreFulla = "" Then
            ...
            If node > 0 Then
                GAIA.bdr(objConn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE (RELINFIL = " + node.ToString() + ") AND (RELCDSIT<99)", DS)
                If DS.Tables(0).Rows.Count > 0 Then
            ...
            If plantilla.innod = 0 And rel.tipintip <> 5 Then
                plantilla.bdget(objConn, GAIA.plantillaPerDefecte(objConn, rel, idioma))
            End If
            ...
            If plantilla.innod > 0 Then
                ...
                GAIA.dibuixaPreview(objConn, plantilla, plantilla.est, plantilla.arrayAtr, plantilla.hor, plantilla.ver, plantilla.tco, "", html, css, width, height, "p", rel, 0, fdesti, urldesti, nrocontingut, llistaDocuments, publicar, idioma, plantilla.css, rel, dataSimulacio, codiUsuari, 0, "", 0, plantilla.flw, titol, nivell, estilBody, "", esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, plantilla.niv)
                Dim regexp As Regex
                Dim strInPattern As String
            ...
            TRY
				GAIA.bdSR(objConn, " INSERT INTO METLCEL VALUES (0," & idioma & "," & rel.incod & "," & rel.incod & ",-1," & codiUsuari & "," & rel.infil & "," & plantilla.innod & ",getdate(),'" & dataSimulacio & "','" & css & "','" & obreFulla.Replace("'", "''") & "','obreFulla?" & idioma & "_" & rel.incod & "_" & rel.incod & "_" & codiUsuari & "_" & plantilla.innod & "')")
			catch
			end try
        END IF
    ...
    Public Shared Sub afegeixAccioMantenimentPDF(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio)
        Try
            Dim DS As DataSet
            DS = New DataSet()
            GAIA.bdr(objConn, "SELECT * FROM METLMAI WHERE MAICDREL=" & rel.incod & " AND MAIDTTEM=0", DS)
            'si ja existia la petició de manteniement, n
            If DS.Tables(0).Rows.Count = 0 Then
                GAIA.bdSR(objConn, "INSERT INTO METLMAI (MAICDREL, MAIDTDAT, MAICDNOD, MAIDTTEM) VALUES (" & rel.incod & ",getdate()," & rel.infil & ", 0)")
            End If
            DS.Dispose()
        Catch
            GAIA.f_logError(objConn, "GAIA", "afegeixAccioMantenimentPDF", Err.Description)
        End Try
    End Sub
    ...
    Public Shared Function mantenimentPDFS() As Integer
        ...
        Try
            GAIA.bdr(objconn, "SELECT * FROM METLMAI WITH(NOLOCK) WHERE MAIDTTEM=0 ORDER BY MAIINCOD", DS)
            For Each dbRow In DS.Tables(0).Rows
                rel.bdget(objconn, dbRow("MAICDREL"))

                If rel.incod = 0 Then 'han esborrat el contingut i per tant no cal fer res
                    GAIA.bdSR(objconn, "DELETE FROM  METLMAI WHERE MAIINCOD=" & dbRow("MAIINCOD"))
                Else
                    dataIni = Now
                    cont += GAIA.generaPDF(objconn, rel)
                    GAIA.bdSR(objconn, "UPDATE METLMAI SET MAIDTTEM=DATEDIFF(MS, '" & dataIni & "', GETDATE()) WHERE  MAICDREL=" & rel.incod & " AND MAIDTTEM=0 ")
                End If
		...
		IF now.hour()=1 and now.minute()>0 and now.minute < 11 THEN
			...
			gaia.bdr(objconn, "select DISTINCT RELINFIL FROM METLREL  WITH(NOLOCK),METLFTR WITH(NOLOCK) WHERE RELINFIL=FTRINNOD AND RELCDHER LIKE '%103017%' AND RELCDSIT<98 AND FTRDSEEX like '' AND (FTRSWVSE=1 OR FTRSWVWE=1)",ds)
			For each dbrow in ds.tables(0).rows 
				IF  NOT File.Exists("e:\docs\GAIA\contingutsPDF\1_" & dbrow("RELINFIL") & ".pdf")
					'response.write("falta pdf:" & dbrow("RELINFIL") & "<br />")
					rel.bdget(objconn, -1,dbrow("RELINFIL"))
					
					GAIA.generaPDF(objconn, rel)
				END IF
    ...
    Public Shared Sub mantenimentContinguts()
        ...
        GAIA.bdSR(objconn, "UPDATE METLREL SET RELCDSIT=98 WHERE RELINCOD IN (	SELECT RELINCOD FROM METLAGD WITH(NOLOCK),METLREL WITH(NOLOCK) WHERE AGDINNOD=RELINFIL  AND RELCDSIT<98 AND ((AGDDTCAD<= getdate() AND AGDDTCAD<>'01/01/1900') OR (AGDDTCAD='01/01/1900' AND (AGDDTFIN<='" + CDate(Today.AddDays(1)).ToString() + "' OR (AGDDTFIN IS NULL AND AGDDTINI<='" + CDate(Today.AddDays(1)).ToString() + "'))))) AND RELCDHER NOT  LIKE '_5286%'")
        GAIA.bdSR(objconn, "UPDATE METLREL SET RELCDSIT=98 WHERE RELINCOD IN (SELECT RELINCOD FROM METLREL WITH(NOLOCK),METLNOT WITH(NOLOCK) WHERE NOTINNOD=RELINFIL  AND RELCDSIT<98 AND ( NOTDTCAD<=getdate()  AND NOTDTCAD<>'01/01/1900') AND RELCDHER NOT  LIKE '_5286%')")
		...
		gaia.bdsr(objconn, "DELETE FROM METLCEL WHERE CELCDPLT=199611")
		...
		gaia.bdsr(nothing, "delete from METLMPA WHERE  MPADTTEM=0 AND MPACDREL NOT IN (SELECT RELINCOD FROM METLREL) AND MPACDREL<>0")


		While Not fin				
			GAIA.bdR(objconn, "select top 1 *  FROM METLMPA WHERE MPADTTEM=0 order by MPAINMPA", DS)
		  	If DS.Tables(0).Rows.Count = 0 Then				
                fin = True
            Else
				dbrow=ds.tables(0).rows(0)
				...
				GAIA.afegeixAccioManteniment(objconn, rel, dbrow("MPACDNOD"), dbrow("MPACDIDI"), dbrow("MPADTPUB"), dbrow("MPADTCAD"), relOriginal, dbrow("MPAWNASC"), dbrow("MPAWNEST"), false)
				GAIA.bdSR(nothing, "UPDATE METLMPA SET MPADTTEM=(select DATEDIFF(MS, '" & dataIni & "', GETDATE() )) WHERE MPADTTEM=0 AND MPACDREL=" & rel.incod & " AND MPACDNOD=" & dbrow("MPACDNOD") & " AND MPACDIDI=" & dbrow("MPACDIDI") & " AND MPADTPUB='" & dbrow("MPADTPUB") & "' AND MPADTCAD='" & dbrow("MPADTCAD") & "' AND MPAWNASC="  & dbrow("MPAWNASC") & " AND MPAWNEST=" & dbrow("MPAWNEST")) 
        ...
        While Not fin
            ...
            Else
                ...
                If dbRow("MANCDREL") <> 0 Then
                    ...
                    If rel.incod <> 0 Then
                        If rel.tipdsdes <> "fulla web" And rel.tipdsdes <> "node web" Then
                            ...
                            GAIA.afegeixAccioMantenimentPDF(objconn, rel)
                            ' If Not GAIA.generaPDF(objconn, rel) Then
                            'Si no és una fulla web, és un contingut
                            'Esborro el manteniment
                            GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni, dbRow("MANCDREL"), dbRow("MANCDNOD"), dbRow("MANCDIDI"))
                            ' End If
                            'Elimino totes les cel·les on apareix aquest contingut
							GAIA.esborrarCelles(nothing,"CELCDNOD=" & rel.infil)
                        ...
                        Else
                            Try
                                If rel.tipdsdes = "node web" Then
                                    ...
                                    DS2 = New DataSet()
                                    'Teresa: incluyo GAIA2
                                    GAIA.bdr(objconn, "	SELECT WEBINIDI, RELINCOD, '' AS WEBDSHTM, 1 AS versioGAIA FROM METLWEB WITH(NOLOCK) INNER JOIN METLREL WITH(NOLOCK) ON METLWEB.WEBINNOD = METLREL.RELINFIL  INNER JOIN METLNOD ON METLREL.RELINFIL = METLNOD.NODINNOD INNER JOIN METLTIP ON METLNOD.NODCDTIP = METLTIP.TIPINTIP AND METLTIP.TIPDSDES LIKE 'fulla web' WHERE (METLREL.RELCDHER LIKE '" + rel.cdher + "_" + rel.infil.ToString() + "%') AND RELCDSIT<98 UNION ALL SELECT WEBINIDI, RELINCOD,WEBDSHTM, 2 AS versioGAIA FROM METLWEB2 WITH(NOLOCK) INNER JOIN METLREL WITH(NOLOCK)  ON METLWEB2.WEBINNOD = METLREL.RELINFIL  INNER JOIN METLNOD ON METLREL.RELINFIL = METLNOD.NODINNOD INNER JOIN METLTIP ON METLNOD.NODCDTIP = METLTIP.TIPINTIP AND METLTIP.TIPDSDES LIKE 'fulla web' WHERE (METLREL.RELCDHER LIKE '" + rel.cdher + "_" + rel.infil.ToString() + "%') AND RELCDSIT<98", DS2)
                                    For Each dbRow2 In DS2.Tables(0).Rows
                                        dataIni = Now 'moment en que comença a generarse la publicació
                                        Try
                                            html = ""
                                            ...
                                            heretaPropietats = GAIA.heretarPropietatsWeb(objconn, dbRow2("RELINCOD"), width)
                                            strTmp &= GAIA.llistatErrors
                                            GAIA.llistatErrors = ""
											'Teresa: incluyo GAIA2
											If dbRow2("versioGAIA") = 2 Then
                                                html = GAIA.maquetarHTML(objconn, html, relFullaWeb, fDesti, urlDesti, 0, llistaDocuments, 1, dbRow2("WEBINIDI"), relFullaWeb, Now, usuari, tagsMeta, titol, estilbody, esForm, esEML, esSSL, htmlPeu, strCSSPantalla, strCSSImpressora)
                                            Else
                                            GAIA.obreFullaWeb(objconn, relFullaWeb, html, css, heretaPropietats, width, 500, "", fDesti, urlDesti, 1, llistaDocuments, 1, dbRow2("WEBINIDI"), relFullaWeb, Now, usuari, titol, 1, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, True)
                                            End If
											If GAIA.llistatErrors.Length > 0 Then
                                                'he trobat un error i no publico. 												
                                            Else
                                                If GAIA.contingutEsPublicable(objconn, relFullaWeb, Now, dbRow2("WEBINIDI"), 99) = ctPUBLICAR Then
												If esEML <> "S" Then  'Que no publique los boletines de manera automática
                                                    ...
                                                    If InStr(fDesti, ".vb") Or InStr(fDesti, ".js") > 0 Then
                                                        GAIA.publica(objconn, html, fDesti, llistaDocuments, llistaDocumentsEliminar, 1, relFullaWeb, dbRow2("WEBINIDI"), "Manteniment", ftpsession)
                                                    Else
													If dbRow2("versioGAIA") = 2 Then
                                                                If GAIA.publica(objconn, GAIA.preparaPagina2(objconn, html, css, 1, titol, heretaPropietats, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, dbRow2("WEBINIDI"), urlDesti), fDesti, llistaDocuments, llistaDocumentsEliminar, 1, relFullaWeb, dbRow2("WEBINIDI"), "Manteniment", ftpsession) Then
                                                                    GAIA.log(objconn, relFullaWeb, 0, "", TAPUBLICAR)
                                                                End If
                                                            Else														
                                                        If GAIA.publica(objconn, GAIA.preparaPagina(objconn, html, css, 1, titol, heretaPropietats, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, dbRow2("WEBINIDI"), urlDesti), fDesti, llistaDocuments, llistaDocumentsEliminar, 1, relFullaWeb, dbRow2("WEBINIDI"), "Manteniment", ftpsession) Then
                                                            GAIA.log(objconn, relFullaWeb, 0, "", TAPUBLICAR)
                                                        End If
                                                    End If
													End If
											...
											End If
                                        Catch
                                            GAIA.debug(objconn, "MANTENIMENT. PROBLEMES AL PUBLICAR: " + relFullaWeb.incod.ToString() + "(" + relFullaWeb.noddstxt.ToString() + ") a l'adreça " + fDesti.ToString() + "-" + Err.Description)
                                ...
                                Else    'és una fulla web						
                                    heretaPropietats = GAIA.heretarPropietatsWeb(objconn, dbRow("MANCDREL"), width)
                                    esSSL = "N"
                                    esEML = "N"
                                    esForm = "N"
                                    strTmp &= GAIA.llistatErrors
									...
									'Teresa: incluyo GAIA2
									If esGAIA2(objconn, rel.infil) Then
                                        html = GAIA.maquetarHTML(objconn, html, rel, fDesti, urlDesti, 0, llistaDocuments, 1, dbRow("MANCDIDI"), rel, Now, usuari, tagsMeta, titol, estilbody, esForm, esEML, esSSL, htmlPeu, strCSSPantalla, strCSSImpressora)
                                    Else
                                    GAIA.obreFullaWeb(objconn, rel, html, css, heretaPropietats, width, 500, "", fDesti, urlDesti, 1, llistaDocuments, 1, dbRow("MANCDIDI"), rel, Now, usuari, titol, 1, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, True)
                                    End If
									If GAIA.llistatErrors.Length > 0 Then
                                        'he trobat un error i no publico. 												
                                    Else
                                        If GAIA.contingutEsPublicable(objconn, rel, Now, dbRow("MANCDIDI"), 99) = ctPUBLICAR Then
										  If esEML <> "S" Then 'Que no publique los boletines de manera automática
                                            ...
                                            If InStr(fDesti, ".vb") Or InStr(fDesti, ".js") > 0 Then
                                                GAIA.publica(objconn, html, fDesti, llistaDocuments, llistaDocumentsEliminar, 1, rel, dbRow("MANCDIDI"), "Manteniment", ftpsession)
                                            Else
												If esGAIA2(objconn, rel.infil) Then
                                                        GAIA.publica(objconn, GAIA.preparaPagina2(objconn, html, css, 1, titol, heretaPropietats, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, dbRow("MANCDIDI"), urlDesti), fDesti, llistaDocuments, llistaDocumentsEliminar, 1, rel, dbRow("MANCDIDI"), "Manteniment", ftpsession)
                                                    Else
                                                GAIA.publica(objconn, GAIA.preparaPagina(objconn, html, css, 1, titol, heretaPropietats, estilbody, esForm, esEML, esSSL, tagsMeta, htmlPeu, strCSSPantalla, strCSSImpressora, dbRow("MANCDIDI"),urlDesti), fDesti, llistaDocuments, llistaDocumentsEliminar, 1, rel, dbRow("MANCDIDI"), "Manteniment", ftpsession)
                                            End If
											End If
                                            GAIA.log(objconn, rel, 0, "", TAPUBLICAR)
                                        End If
                                    End If
									End If
                                    '  GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni)
                            ...
                            Catch
                                strTmp &= "MANTENIMENT. PROBLEMES AL PUBLICAR: " + relFullaWeb.incod.ToString() + "(" + relFullaWeb.noddstxt.ToString() + ") a l'adreça " + fDesti.ToString() + "-" + Err.Description
                                GAIA.debug(objconn, "MANTENIMENT. PROBLEMES AL PUBLICAR: " + rel.incod.ToString() + "(" + rel.noddstxt.ToString() + ") a l'adreça " + fDesti.ToString() + "-" + Err.Description)
                                'GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni)
                                GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni, dbRow("MANCDREL"), dbRow("MANCDNOD"), dbRow("MANCDIDI"))
                            End Try
                        End If
                    Else
                        ...
                        GAIA.bdSR(objconn, "DELETE FROM METLMAN WHERE MANCDREL=" + dbRow("MANCDREL").ToString())
                    End If
                Else 'Tinc un manteniment d'un contingut per MANCDNOD. Aquest manteniment pot ser una caducitat o un canvi del contingut que modifica METLCEL 		
					...
					GAIA.esborrarCelles(nothing,"CELCDNOD=" & dbRow("MANCDNOD"))
				    ...
				    DS2 = New DataSet()
                    GAIA.bdr(objconn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK)  WHERE RELCDSIT<98 AND RELINFIL=" & dbRow("MANCDNOD"), DS2)
                    Dim llistaCanvis As String = ""
                    Dim codirelacio As Integer = 0
                    Dim llistaEsborrar As String = ""
                    For Each dbRow2 In DS2.Tables(0).Rows
                        rel.bdget(objconn, dbRow2("RELINCOD"))
	                        llistaCanvis = GAIA.obtenirPaginesAfectadesPerCanvi(objconn, rel, 0, 1, Now, Now, rel, 1, "", 0)
                    ...
                    If llistaEsborrar.Length > 0 Then
						GAIA.esborrarCelles(nothing," CELINREL IN (" & llistaEsborrar & ")")
                    ...
                    If rel.incod <> 0 Then 'si es=0 és que està esborrat
                        GAIA.datesPublicacioPerNode(objconn, dbRow("MANCDNOD"), dbRow("MANCDIDI"), datapub, datacad)
                        If rel.tipdsdes.Substring(0, 5) = "fulla" And rel.tipdsdes.Trim() <> "fulla web" Then
                            ...
                            If CDate(datacad) <> CDate("01/01/1900") And CDate(datacad) <= CDate(Now) Then
                                Try
                                    GAIA.esborrarNode(objconn, dbRow("MANCDNOD"), relbuida, 1, 115969, 1, GAIA.ctESBORRATCADUCAT)

                                    'He tractat el manteniment i ja el puc esborrar				
                                    'GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni)
                                    GAIA.esborraAccioManteniment(objconn, dbRow("MANINMAN"), dataIni, dbRow("MANCDREL"), dbRow("MANCDNOD"), dbRow("MANCDIDI"))
                                Catch
                                    GAIA.debug(objconn, "Manteniment automàtic. Problemes esborrant " + dbRow("MANCDNOD").ToString())
                                End Try
    ...
    Public Shared Function afegeixAccioManteniment(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiNode As Integer, ByVal codiIdioma As Integer, ByRef dataPubOriginal As String, ByRef dataCadOriginal As String, ByVal relOriginal As clsRelacio, ByVal mantenirAscendents As Integer, Optional ByVal estructura As Integer = 0, optional byVal endiferit as boolean=false) As String
		...
		strResultat=""
		if  endiferit THEN
			GAIA.bdSR(objconn, "INSERT INTO METLMPA VALUES (" & rel.incod & "," & codiIdioma & ",'" & Cdate(dataPubOriginal) & "','" & cdate(dataCadOriginal) & "'," & codiNode & "," & relOriginal.incod & "," & mantenirAscendents & "," & estructura & ",0)")
		ELSE
			...
			If rel.incod = 0 Then
				GAIA.creaAccioManteniment(objConn, 0, codiIdioma, dataPubOriginal, codiNode)
				GAIA.creaAccioManteniment(objConn, 0, codiIdioma, dataCadOriginal, codiNode)
			End If
			...
			For Each item In strPagines.Split(",")
				If item.Length > 0 Then
					...
					GAIA.creaAccioManteniment(objConn, arrValors(0),99, arrValors(2), arrValors(1))
						...
						If relTemp.tipintip = 8 Or relTemp.tipintip = 9 Or relTemp.tipintip = 10 Then
							...
							If relTemp.cdher.Length > 0 Then
								  If relTemp.cdrsu <> 0 Then
									GAIA.bdr(objConn, "select NODDSTXT FROM METLNOD WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE RELINPAR=NODINNOD AND RELINCOD=" & relTemp.cdrsu, ds)
								End If
	
							Else
								GAIA.bdr(objConn, "SELECT NODDSTXT FROM METLNOD  WITH(NOLOCK) WHERE NODINNOD=" & relTemp.infil, ds)
							End If
    ...
    Public Shared Function obtenirPaginesAfectadesPerCanvi(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiNode As Integer, ByVal codiIdioma As Integer, ByRef dataPubOriginal As String, ByRef dataCadOriginal As String, ByVal relOriginal As clsRelacio, ByVal mantenirAscendents As Integer, ByRef strResultat As String, Optional ByVal estructura As Integer = 0, Optional ByRef relTractades As Hashtable = Nothing, optional byVal esborrarCEL as boolean=true) As String
        ...
        If rel.incod = 0 Then
            If codiNode <> 0 Then 'em demanen fer el manteniment d'un node, i per tant he de cercar on apareix		
                GAIA.bdr(objConn, "SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE RELCDSIT<95 AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%' AND RELINFIL=" & codiNode, DS)
                For Each dbRow In DS.Tables(0).Rows
                    relTmp.bdget(objConn, dbRow("RELINCOD"))
                    GAIA.obtenirPaginesAfectadesPerCanvi(objConn, relTmp, 0, codiIdioma, CDate(dataPubOriginal), CDate(dataCadOriginal), relTmp, 1, strResultat, 0, relTractades)
        ...
        Else  'He de fer el manteniment d'una relació concreta
            ...
            If Not relTractades.Contains(CInt(rel.incod)) Then
                ...
                If dataPubOriginal = CDate("01/01/1900") Then
                    GAIA.datesPublicacio(objConn, relOriginal, i, dataPub, dataCad)
                Else
                ...
                Else
                    Select Case rel.tipdsdes
                        Case "fulla codiWeb" 'llibreria de codi
                            ...
                            strPlant = ""
                            ' Teresa: UNION con METLPLT2 ok
                            GAIA.bdr(objConn, "SELECT DISTINCT PLTINNOD FROM METLPLT WITH(NOLOCK)  WHERE (PLTDSLCW LIKE '%" & relOriginal.infil & "%') OR (PLTDSLC2 LIKE '%" & relOriginal.infil & "%') UNION SELECT DISTINCT PLTINNOD FROM METLPLT2 WITH(NOLOCK)  WHERE (PLTDSLCW LIKE '%" & relOriginal.infil & "%') OR (PLTDSLC2 LIKE '%" & relOriginal.infil & "%')", DS)
                            ...
                            For Each dbRow In DS.Tables(0).Rows
                                ...
                                End If
                                ' Teresa: UNION con METLWEB2 ok
                                GAIA.bdr(objConn, "select RELINCOD FROM METLWEB WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE RELCDHER LIKE '" & elements & "%' AND RELINFIL=WEBINNOD AND RELCDSIT<95 UNION Select RELINCOD FROM METLWEB2 WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE RELCDHER LIKE '" & elements & "%' AND RELINFIL=WEBINNOD AND RELCDSIT<95 ", DS2)
                                For Each dbRow2 In DS2.Tables(0).Rows
                            ...
                            If llistaPlantilles.Length > 0 Then
                                GAIA.bdr(objConn, "SELECT DISTINCT CELCDEST FROM METLCEL with(NOLOCK) WHERE CELINLCW=0 AND CELCDPLT IN (" & llistaPlantilles & ")", DS)
                                For Each dbRow In DS.Tables(0).Rows
							...
							IF esborrarCEL THEN
								...
								If relacionsPerEsborrar.Length > 0 Then
									...
									GAIA.esborrarCelles(nothing, "CELINREL IN (" & relacionsPerEsborrar & ") ")
								End If
	
								'********************************************************************************************************************
								' Esborro totes les cel·les amb llibreria de codi donada.
								'********************************************************************************************************************
								GAIA.esborrarCelles(nothing, "CELINLCW =" & rel.infil)
                        ...
                        Case "fulla directori"
                            ...
                            GAIA.bdr(objConn, "select DISTINCT FTRDSNOM,FTRINNOD FROM METLASS with(NOLOCK),METLFTR with(NOLOCK) WHERE  ASSCDNRL=" & rel.infil & "  AND ASSCDNOD=FTRINNOD AND FTRINIDI=1", DS)
                            For Each dbRow In DS.Tables(0).Rows
							...
							GAIA.bdr(objConn, "select DISTINCT EQPINACT FROM METLEQP WITH(NOLOCK), METLREL WITH(NOLOCK) WHERE EQPINDIR=" & rel.infil & " AND RELINFIL=EQPINACT AND RELCDSIT<98", DS)
							For Each dbRow In DS.Tables(0).Rows
				...
				IF esborrarCEL THEN
					...
					GAIA.esborrarCelles(nothing, strsql)
                ...
                'Esborro totes les execucions de la llibreria afectada per que ja no són vàlides. S'hauran de refer automàticament quan s'hagi de publicar la pàgina al manteniment
				 ...
				 If esborrarCEL Then
					GAIA.esborrarCelles(nothing,("CELINLCW IN (select ASSCDNOD FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) ,METLASS WITH(NOLOCK)  WHERE  RELINFIL=NODINNOD AND ASSCDTPA=41 AND ASSCDNRL=RELINFIL AND RELCDSIT<95 AND (RELINFIL=" & rel.infil & " OR RELINFIL IN (" & IIf(rel.cdher = "", rel.infil.ToString(), rel.cdher.Replace("_", ",")) & ")))").replace("(,", "("))
                   ' GAIA.bdSR(objConn, ("DELETE FROM METLCEL WHERE CELINLCW IN (select ASSCDNOD FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) ,METLASS WITH(NOLOCK)  WHERE  RELINFIL=NODINNOD AND ASSCDTPA=41 AND ASSCDNRL=RELINFIL AND RELCDSIT<95 AND (RELINFIL=" & rel.infil & " OR RELINFIL IN (" & IIf(rel.cdher = "", rel.infil.ToString(), rel.cdher.Replace("_", ",")) & ")))").replace("(,", "("))
                    GAIA.esborrarCelles(nothing,"CELINLCW IN (select ASSCDNOD FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) ,METLASS WITH(NOLOCK)  WHERE  RELINFIL=NODINNOD AND ASSCDTPA=41 AND ASSCDNRL=RELINFIL AND RELCDSIT<95  AND RELINFIL= " & rel.infil & ")")
               ...
               GAIA.bdr(objConn, ("select ASSCDNOD FROM METLREL WITH(NOLOCK) ,METLNOD WITH(NOLOCK) ,METLASS WITH(NOLOCK)  WHERE  RELINFIL=NODINNOD AND ASSCDTPA=41 AND ASSCDNRL=RELINFIL AND RELCDSIT<95 AND (RELINFIL IN (" & IIf(rel.cdher = "", rel.infil.ToString(), rel.cdher.Replace("_", ",")).replace("(,", "(") & ") OR RELINFIL=" & rel.infil & ")").Replace("(,", "("), DS)
                ...
                For Each dbRow In DS.Tables(0).Rows
                    ...
                    'Teresa: UNION con METLWEB2, METLNWE2 y METLAWE2 ok
                    GAIA.bdr(objConn, " (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL WITH(NOLOCK) INNER JOIN METLWEB WITH(NOLOCK) ON METLREL.RELINFIL = METLWEB.WEBINNOD AND ((METLWEB.WEBDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%') " + strPlant + " ) WHERE (METLREL.RELCDSIT < 95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%') UNION (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE WITH(NOLOCK) ON METLREL.RELINFIL = METLAWE.AWEINNOD AND ((METLAWE.AWEDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%')) WHERE (METLREL.RELCDSIT < 95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%') UNION (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL WITH(NOLOCK) INNER JOIN METLNWE WITH(NOLOCK) ON METLREL.RELINFIL = METLNWE.NWEINNOD AND ((METLNWE.NWEDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%')  " + strPlant.Replace("WEB", "NWE") + ") WHERE (METLREL.RELCDSIT <95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%') UNION (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL WITH(NOLOCK) INNER JOIN METLWEB2 WITH(NOLOCK) ON METLREL.RELINFIL = METLWEB2.WEBINNOD AND ((METLWEB2.WEBDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%') " + strPlant + " )  WHERE (METLREL.RELCDSIT < 95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%') UNION (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL WITH(NOLOCK) INNER JOIN METLAWE2 WITH(NOLOCK) ON METLREL.RELINFIL = METLAWE2.AWEINNOD AND ((METLAWE2.AWEDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%')) WHERE(METLREL.RELCDSIT < 95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%') UNION (SELECT DISTINCT METLREL.RELINCOD,METLREL.RELCDHER FROM METLREL  WITH(NOLOCK) INNER JOIN METLNWE2 WITH(NOLOCK)  ON METLREL.RELINFIL = METLNWE2.NWEINNOD AND ((METLNWE2.NWEDSLCW LIKE '%" + dbRow("ASSCDNOD").ToString() + "%')  " + strPlant.Replace("WEB", "NWE") + ") WHERE(METLREL.RELCDSIT < 95) AND NOT RELCDHER LIKE '_5286%' AND NOT RELCDHER LIKE '_184845%' AND NOT RELCDHER LIKE '_145043%')", DS2)
                    ...
                    For Each dbRow2 In DS2.Tables(0).Rows
                        If InStr(dbRow2("RELCDHER"), "_5286") <= 0 And dbRow2("RELINCOD") <> relIni.incod And dbRow2("RELINCOD") <> rel.incod Then 'només mantindré la pàgina si no està a dins d'un arbre personal, faig al comprovació que no obtingui el mateix objecte que ha iniciat el manteniment
                            ...
                            If Not relTractades.Contains(CInt(dbRow2("RELINCOD"))) Then
                                ...
                                GAIA.obtenirPaginesAfectadesPerCanvi(objConn, relTmp, 0, 99, CDate(dataPub), CDate(dataCad), rel, 0, strResultat, 0, relTractades)
                            End If
                        End If
                    Next dbRow2
                Next dbRow
    ...
    Public Shared Function esborrarCelles(ByVal objconn As OleDbConnection, ByVal strWhere As String) As Integer
        ...
        Dim ds As New DataSet
        Dim cont As Integer
        GAIA.bdr(Nothing, "SELECT COUNT(*) as numCelles FROM METLCEL WITH(NOLOCK) WHERE " & strWhere, ds)
        cont = ds.Tables(0).Rows(0)("numCelles")
        esborrarCelles = cont
        While cont > 0
            GAIA.bdSR(Nothing, "delete TOP (500) FROM METLCEL WHERE " & strWhere)
            cont = cont - 500
        End While
	...
	'	ELSE
		...
		IF rel.tipintip<> 13 THEN  ' No creo accions manteniment per fulles organigrama
			...
			If data < Now Then
				...
				ds = New DataSet
				If codiRelacio <> 0 Then
					GAIA.bdr(objconn, "SELECT COUNT(*) as valor  FROM METLMAN with(NOLOCK) WHERE MANDTTEM=0 AND MANCDREL=" & codiRelacio & " AND MANDTDAT<getdate() AND MANCDIDI=" & IIf(codiIdioma = 99, 1, codiIdioma), ds)
				Else
					GAIA.bdr(objconn, "SELECT COUNT(*) as valor  FROM METLMAN with(NOLOCK)  WHERE MANDTTEM=0 AND  MANCDNOD=" & codiNode & " AND MANDTDAT<getdate() AND MANCDIDI=" & IIf(codiIdioma = 99, 1, codiIdioma), ds)
				End If
			...
			If valor = 0 Then
				If CDate(data) <> CDate("01/01/2050") Then
					if codiIdioma=99 THEN
						If gaia.existeixContingut(objconn, codiNode, codiRelacio, 1) = 1 Then
							GAIA.bdSR(objconn, "INSERT INTO METLMAN (MANCDREL, MANCDIDI, MANDTDAT,MANCDNOD, MANDTTIM, MANDTTEM) VALUES (" + codiRelacio.ToString() + ",1,'" + data + "'," + codiNode.ToString() + ", getdate(),0)")
						END IF
						If gaia.existeixContingut(objconn, codiNode, codiRelacio, 2) = 1 Then
							GAIA.bdSR(objconn, "INSERT INTO METLMAN (MANCDREL, MANCDIDI, MANDTDAT,MANCDNOD, MANDTTIM, MANDTTEM) VALUES (" + codiRelacio.ToString() + ",2,'" + data + "'," + codiNode.ToString() + ", getdate(),0)")
						END IF							
						If gaia.existeixContingut(objconn, codiNode, codiRelacio, 3) = 1 Then
							GAIA.bdSR(objconn, "INSERT INTO METLMAN (MANCDREL, MANCDIDI, MANDTDAT,MANCDNOD, MANDTTIM, MANDTTEM) VALUES (" + codiRelacio.ToString() + ",3,'" + data + "'," + codiNode.ToString() + ", getdate(),0)")
						END IF							
						If gaia.existeixContingut(objconn, codiNode, codiRelacio, 4) = 1 Then
							GAIA.bdSR(objconn, "INSERT INTO METLMAN (MANCDREL, MANCDIDI, MANDTDAT,MANCDNOD, MANDTTIM, MANDTTEM) VALUES (" + codiRelacio.ToString() + ",4,'" + data + "'," + codiNode.ToString() + ", getdate(),0)")
						END IF							
					ELSE
						If gaia.existeixContingut(objconn, codiNode, codiRelacio, codiIdioma) = 1 Then
							GAIA.bdSR(objconn, "INSERT INTO METLMAN (MANCDREL, MANCDIDI, MANDTDAT,MANCDNOD, MANDTTIM, MANDTTEM) VALUES (" + codiRelacio.ToString() + "," & codiIdioma & ",'" + data + "'," + codiNode.ToString() + ", getdate(),0)")
						END IF
					END IF
				End If
			END IF
    ...
    Public Shared Sub esborraAccioManteniment(ByVal objConn As OleDbConnection, ByVal codiManteniment As Integer, ByVal dataIniciManteniment As String, ByVal codiRelacio As Integer, ByVal codiNode As Integer, ByVal codiIdioma As Integer)
        If codiRelacio = 0 Then
            GAIA.bdSR(objConn, "UPDATE METLMAN SET MANDTTEM=(select DATEDIFF(MS, '" & dataIniciManteniment & "', GETDATE() )) WHERE  MANCDNOD=" & codiNode & " AND MANDTTEM=0 AND MANDTDAT<=getdate() AND MANCDIDI=" & codiIdioma)

        Else
            GAIA.bdSR(objConn, "UPDATE METLMAN SET MANDTTEM=(select DATEDIFF(MS, '" & dataIniciManteniment & "', GETDATE() )) WHERE  MANCDREL=" & codiRelacio & " AND MANDTTEM=0 AND MANDTDAT<=getdate() AND MANCDIDI=" & codiIdioma)
        End If
    ...
    Public Shared Function filtreCercador(ByVal objconn As OleDbConnection, ByVal selectNodes As String, ByVal strUbicacions As String, ByVal strUbicacionsFulla As String, ByVal strCodificacions As String, ByVal numNodeOrganigrama As Integer, ByVal estat As String, ByVal comprobarPermisos As Integer, ByVal internet As Integer) As Integer()
        ...
        Next
        DS = New DataSet()
        strSQL = "SELECT DISTINCT METLREL.RELINCOD, METLREL.RELINFIL FROM METLREL  WITH(NOLOCK) INNER JOIN (" & selectNodes.ToString() & _
        ...
        If Trim(strUbicacions) <> "" Or Trim(strUbicacionsFulla) <> "" Then
            ...
            strSQL += " AND RELINCOD IN (SELECT RELINCOD FROM METLREL WITH(NOLOCK) WHERE (RELINCOD IN (" + strUbicacions + ") " & _
               sqlUbicacionsFulla & " )"
    ...
    Public Shared Function obtenirEnllacContingut(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiIdioma As Integer, ByRef tipus As Integer, ByVal estilenllaç As String, ByVal estiltext As String, ByVal target As Integer, ByVal icona As Integer, ByVal estilIcona As String, ByVal descripcio As Integer, ByVal estildescripcio As String, ByRef link As String, ByRef params As String, ByVal textEnllaç As String, ByVal plantilla As Integer, byVal user as integer) As String
        ...
        Select Case rel.tipdsdes
            Case "fulla link"
                ...
                If DS.Tables(0).Rows.Count > 0 Then
					...
					If link = "" And dbRow("LNKCDREL") <> rel.incod Then
		                ...
		                Link = GAIA.obtenirEnllacContingut(objConn, relLink, codiIdioma)
            ...
            Case "fulla directori"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLDIR  WITH(NOLOCK) WHERE DIRINNOD=" + rel.infil.ToString() + " AND DIRINIDI=" + codiIdioma.ToString(), DS)
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
                    If textEnllaç.Length = 0 Then
                        text = dbRow("DIRDSNOM")
            ...
            Case "fulla projecte"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLFPR  WITH(NOLOCK) WHERE FPRINNOD=" & rel.infil & " AND FPRINIDI=" & codiIdioma, DS)
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
                    If textEnllaç.Length = 0 Then
                        text = dbRow("FPRDSNOM")
            ...
            Case "fulla tramit"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLFTR  WITH(NOLOCK) WHERE FTRINNOD=" & rel.infil & " AND FTRINIDI=" & codiIdioma , DS)
            ...
            Case "fulla agenda"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLAGD  WITH(NOLOCK) WHERE AGDINNOD=" + rel.infil.ToString() + " AND AGDINIDI=" + codiIdioma.ToString(), DS)
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
                    If textEnllaç.Length = 0 Then
                        text = dbRow("AGDDSTIT")
            ...
            Case "fulla noticia"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLNOT  WITH(NOLOCK) WHERE NOTINNOD=" + rel.infil.ToString() + " AND NOTINIDI=" + codiIdioma.ToString(), DS)
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
                    If textEnllaç.Length = 0 Then
                        text = dbRow("NOTDSTIT")
            ...
            Case "fulla info"
                ...
                GAIA.bdr(objconn, "SELECT * FROM METLINF  WITH(NOLOCK) WHERE INFINNOD=" & rel.infil & " AND INFINIDI=" & codiIdioma, DS)
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
                ...
                End If
            Case "fulla document"
                GAIA.bdr(objconn, "SELECT TDODSNOM,METLDOC.* FROM METLDOC WITH(NOLOCK) ,METLTDO WITH(NOLOCK)  WHERE DOCINTDO=TDOCDTDO AND DOCINNOD=" + rel.infil.ToString() + " AND DOCINIDI=" + codiIdioma.ToString(), DS)
                If DS.Tables(0).Rows.Count = 0 Then
                    GAIA.bdr(objconn, "SELECT TDODSNOM,METLDOC.* FROM METLDOC WITH(NOLOCK) ,METLTDO  WITH(NOLOCK) WHERE DOCINTDO=TDOCDTDO AND DOCINNOD=" + rel.infil.ToString() + " ORDER BY DOCINIDI ASC", DS)
                End If
                If DS.Tables(0).Rows.Count > 0 Then
                    dbRow = DS.Tables(0).Rows(0)
            ...
            Case "fulla web"
				...
				'Teresa: Incluyo GAIA2
                GAIA.bdr(objconn, "SELECT METLSER.SERDSURL , METLREI.REIDSFIT, WEBDSTIT, METLREL.RELINCOD FROM METLWEB  WITH(NOLOCK) INNER JOIN METLREL  WITH(NOLOCK) ON METLWEB.WEBINNOD = METLREL.RELINFIL INNER JOIN METLREI  WITH(NOLOCK) ON METLREI.REIINCOD = METLREL.RELINCOD INNER JOIN METLAWE  WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_', SUBSTRING(METLREL.RELCDHER, 2, LEN(METLREL.RELCDHER))) - 1) = METLAWE.AWEINNOD INNER JOIN METLSER  WITH(NOLOCK) ON METLAWE.AWEDSSER = METLSER.SERINCOD WHERE     (METLREL.RELINCOD = " + rel.incod.ToString() + ") UNION SELECT METLSER.SERDSURL, METLREI.REIDSFIT, WEBDSTIT, METLREL.RELINCOD FROM METLWEB2  WITH(NOLOCK) INNER JOIN METLREL  WITH(NOLOCK) ON METLWEB2.WEBINNOD = METLREL.RELINFIL INNER JOIN METLREI  WITH(NOLOCK) ON METLREI.REIINCOD = METLREL.RELINCOD INNER JOIN METLAWE2 WITH(NOLOCK) ON SUBSTRING(METLREL.RELCDHER, 2, CHARINDEX('_', SUBSTRING(METLREL.RELCDHER, 2, LEN(METLREL.RELCDHER))) - 1) = METLAWE2.AWEINNOD INNER JOIN METLSER  WITH(NOLOCK) ON METLAWE2.AWEDSSER = METLSER.SERINCOD WHERE (METLREL.RELINCOD =" + rel.incod.ToString() + ") ORDER BY METLREL.RELINCOD DESC", DS)
        ...
        resultat = resultat + ">"
        If icona Then
            resultat = "<img src=""http://www.l-h.cat/img/" + GAIA.obtenirIconaTipusContingutPerRelacio(objconn, rel) + """ class=""" + estilIcona + """ hspace=""5"" align=""absmiddle""/>" + resultat
        End If
        resultat += text
        If descripcio Then
    ...
    Public Shared Function obtenirTitolContingut(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiIdioma As Integer) As String
        ...
        If rel.incod = 0 Then
            ...
            GAIA.bdr(objconn, "select TOP 1 relOk.RELINCOD FROM METLREL as relEsborrada WITH(NOLOCK) , METLREL as relOk  WITH(NOLOCK) where relEsborrada.RELINCOD=171604 and relOk.RELINFIL=relEsborrada.RELINFIL AND relOk.RELCDSIT<98 AND relOk.RELCDHER LIKE '_57135%'", DS)
            If DS.Tables(0).Rows.Count > 0 Then
                rel.bdget(objconn, DS.Tables(0).Rows(0)("RELINCOD"))
                DS.Dispose()
        ...
        Else
            ...
            Select Case rel.tipintip
                ...
                Case 5
                 ...
                 'Teresa: Se incluye GAIA2
				Case 10
                    cons = "select  isnull(WEBDSTIT,'') as titol FROM METLWEB  WITH(NOLOCK) WHERE WEBINNOD=" & rel.infil & " AND WEBINIDI=" & codiIdioma & " UNION select isnull(WEBDSTIT,'') as titol FROM METLWEB2 WITH(NOLOCK) WHERE WEBINNOD=" & rel.infil & " AND WEBINIDI=" & codiIdioma
                Case 40
            ...
            GAIA.bdr(objconn, cons, DS)
            If DS.Tables(0).Rows.Count > 0 Then
                obtenirTitolContingut = GAIA.netejaHTML(DS.Tables(0).Rows(0)("titol"))
            Else
    ...
    '	1-2-2019: añado UNION a la query para GAIA2					
    '****************************************************************************************************************	
    ...
    Public Shared Sub obtenirNomWeb(ByVal objconn As OleDbConnection, ByVal rel As clsRelacio, ByVal codiIdioma As Integer, ByRef nomWeb As String, ByRef nomPagina As String, ByRef descPagina As String)
        ...
        End If
        sql = "SELECT WEBDSTIT, WEBDSDES, NWEDSTIT, AWEDSTIT FROM METLWEB   WITH(NOLOCK) LEFT JOIN METLNWE  WITH(NOLOCK) ON NWEINNOD=" & nodeWeb & " LEFT JOIN METLAWE  WITH(NOLOCK) ON AWEINNOD=" & arbreWeb & " WHERE WEBINNOD = " & rel.infil & " AND WEBINIDI = " & codiIdioma & " UNION SELECT WEBDSTIT, WEBDSDES, NWEDSTIT, AWEDSTIT FROM METLWEB2   WITH(NOLOCK) LEFT JOIN METLNWE2  WITH(NOLOCK) ON NWEINNOD=" & nodeWeb & " LEFT JOIN METLAWE2  WITH(NOLOCK) ON AWEINNOD=" & arbreWeb & " WHERE WEBINNOD = " & rel.infil & " AND WEBINIDI = " & codiIdioma
    ...
    Public Shared Function modificaCodificacions(ByVal objconn As OleDbConnection, ByVal objArbre As RadTreeView, ByVal codiNode As Integer, ByVal nodeVisibleInternet As Integer, ByVal usuari As Integer, Optional ByVal codiNodeArbre As integer = 0, optional byval tractaPermisosEnDiferit as Boolean  =false) As String 
		...
		try
			...
			IF codiNodeArbre=0 THEN  
				codiNodeArbre = GAIA.codiNodeByTxt(objconn, "arbre codificació", "arbre codificació")
			End IF
			strErr &= "a"
			Dim cont As Integer = 0
			Dim treeNode As RadTreeNode
			...
			For Each treeNode In objArbre.CheckedNodes
				...
				End If
				cont += 1
				llistarel += GAIA.obtenirRelacioPantalla(treeNode).ToString()
			Next treeNode
		...
		'	IF llistaRel <> "" THEN
				...
				If llistarelacionsPerEsborrar.Length > 0 Then
					...
					If llistaAmbPermisos.Length > 0 Then
						...
						GAIA.bdSR(objconn, "delete from METLREL WHERE RELINCOD IN (" & llistaAmbPermisos & ")")
					End If
				End If
				...
				For Each treeNode In objArbre.CheckedNodes
					...
					If usuari = 0 Then
						tePermis = 1
					Else
						reltmp.bdget(objconn, GAIA.obtenirRelacioPantalla(treeNode))
		...
		''''''''arbol=46 es un problema a la hora de borrar contenidos.. me lo tengo que mirar, problemas con el 8
						GAIA.creaRelacio(objconn, 46, GAIA.obtenirNodePantalla(objconn, treeNode), codiNode, 0, GAIA.obtenirPathRelacioPantalla(objconn, treeNode) & "_" & GAIA.obtenirNodePantalla(objconn, treeNode), -1, 1, -1, nodeVisibleInternet,tractaPermisosEnDiferit,usuari)
					
						'si l'objecte té documents i/o notícies les poso també
						GAIA.bdr(objconn, "SELECT DISTINCT RELINPAR,RELINFIL,RELCDEST, RELCDORD, RELCDSIT FROM METLREL  WITH(NOLOCK) WHERE RELINPAR=" & codiNode, DS)
						For Each dbRow In DS.Tables(0).Rows
							GAIA.creaRelacio(objconn, 46, dbRow("RELINPAR"), dbRow("RELINFIL"), 0, GAIA.obtenirPathRelacioPantalla(objconn, treeNode) & "_" & GAIA.obtenirNodePantalla(objconn, treeNode) & "_" & dbRow("RELINPAR"), dbRow("RELCDEST"), dbRow("RELCDSIT"), dbRow("RELCDORD"), nodeVisibleInternet, true, usuari)
						Next dbRow
				...
				If llistaSensePermisos.Length > 0 Then
					...
					If resultat.Length > 0 Then
						resultat &= "<br />"
					End If
					GAIA.bdr(objconn, "SELECT NODDSTXT FROM METLREL with(NOLOCK),METLNOD with(NOLOCK) WHERE NODINNOD=RELINPAR AND RELINCOD IN (" & llistaSensePermisos & ")", DS)
					For Each dbRow In DS.Tables(0).Rows
						If strError.Length > 0 Then
							strError &= ","
						End If
						strError &= HttpUtility.HtmlEncode(GAIA.netejaHTML(dbRow("NODDSTXT")))
					Next dbRow
		...
		'	End if
		Catch ex As Exception
			gaia.debug(objconn,"GAIA.Modifica codificacions." & strerr & ex.Message )
...
dim midaTmp as integer=0
        try
			...
			gaia.debug(nothing, strErr & "_ "& str & "_" & mida)
			return(str)
		end try
    End Function
  ...
  '*******************************************************************************************************
    ...
    Public Shared Function DameLonLatDeCoordenada(ByVal punto As String) As String
        ...
        Dim csFrom As ICoordinateSystem = CoordinateSystemWktReader.Parse(wktFrom)
    ...
    Public Shared Function generaPDF(ByVal objConn As OleDbConnection, ByVal rel As clsRelacio) As Integer
        Try
            ...
            If rel.tipdsdes = "fulla tramit" Then
                DS = New DataSet()
                GAIA.bdr(objConn, "SELECT DISTINCT FTRSWVSE,FTRSWVWE, FTRSWVIN FROM METLFTR  WITH(NOLOCK) WHERE FTRINIDI=1 AND FTRINNOD=" & rel.infil, DS)
                If DS.Tables(0).Rows.Count > 0 Then
        ...
        Catch
            GAIA.f_logError(objConn, "GeneraPDF", "", Err.Description)
            Return (0)
        End Try
    ...
    'Si cssDiv1 i cssDivN <>"" envolcaré el <ul> en un <div>.  Els estils són per quan només hi ha 1 element i per quan hi ha N respectivament
     ...
     Public Shared Function menuLinks(ByVal objConn As OleDbConnection, ByVal codiRelacio As Integer, ByVal codiIdioma As Integer, Optional ByVal cssDiv1 As String = "", Optional ByRef cssDivN As String = "", Optional ByVal cssDivFill1 As String = "", Optional ByRef cssDivFillN As String = "", Optional ByVal codiRelacioDestacada As Integer = 0, Optional ByVal cssDestacat As String = "") As String
        ...
        For Each dbRow In DS.Tables(0).Rows
            ...
            If Link = "" Then
                Link = GAIA.obtenirEnllacContingut(objConn, rel, codiIdioma)
            End If
            ...
            Else
                ...
                Link = IIf(InStr(Link, "http") <= 0, IIf(dbRow("paginaSegura") = "S", "https://", "http://") & GAIA.trobaURLDesti(objConn, rel), "") & Link
            End If
            ...
            strMenu &= IIf(cont > 1, "</li><li" & IIf(codiRelacioDestacada = rel.incod, " class=""" & cssDestacat & """", "") & ">", "") & "<a href=""" & GAIA.netejaHTML(Link) & """ target=""" & IIf(dbRow("LNKWNTIP") = 1, "_blank", "_self") & """ title=""" & IIf(codiIdioma = "1", "Anar a ", "Ir a ") & dbRow("LNKDSTXT") & IIf(dbRow("LNKWNTIP") = 1, txtNovaFinestra, "") & """>" & dbRow("LNKDSTXT") & IIf(dbRow("LNKWNTIP") = 1, strImatgeFinestraNova, "") & "</a>"

            '*********************************************
            ' Tracto els fills
            '********************************************
            If cssDivFill1.Length > 0 And dbRow("nroFills") > 0 Then
                strMenu &= GAIA.menuLinks(objConn, dbRow("RELINCOD"), codiIdioma, cssDivFill1, cssDivFillN, "", "", codiRelacioDestacada, cssDestacat)
            End If
        Next dbRow
    ...
    Public Shared Function obtenirEnllacosDS(ByVal objConn As OleDbConnection, ByVal codiIdioma As Integer, ByVal codiRelacio As Integer, ByVal codiUsuari As Integer) As DataSet
        ...
        For cont = 0 To DS.Tables(0).Rows.Count - 1
            ...
            contingut = GAIA.obtenirEnllacContingut(objConn, rel, codiIdioma, tipus, "", "", 1, 1, "", 0, "", link, params, "", 0, codiUsuari)
                            ...
                            If Month(dataInici) = Month(dataFi) Then
                                ...
                                If Day(dataInici) <> Day(dataFi) Then
                                    If mostrarDiaSetmana Then
                                        sortida = IIf(codiIdioma = 1, "del " & StrConv(diaCat(Weekday(dataInici) - 1), 2) & " " & Day(dataInici) & " al " & StrConv(diaCat(Weekday(dataFi) - 1), 2) & " " & Day(dataFi) & " " & mesCat(Month(dataFi) - 1), "del " & StrConv(diaCas(Weekday(dataInici) - 1), 2) & " " & Day(dataInici) & " al " & StrConv(diaCas(Weekday(dataFi) - 1), 2) & " " & Day(dataFi) & " " & mesCas(Month(dataFi) - 1))
                                    Else
...
Public Class clsPlantilla
   ...
   Public Property vis() As Integer
    ...
    Public Sub bdget(ByVal objconn As OleDbConnection, ByVal pltinnod As Integer)
        ...
        If pltinnod <> 0 Then
            GAIA.bdr(objconn, "SELECT PLTDSATR,PLTINNOD, PLTDSEST, PLTDSCMP,PLTDSCSS, PLTDSLNK, PLTDSIMG, PLTDSLCW, PLTDSNUM, PLTDSLC2, PLTDSALK, PLTCDPAL, PLTDSAAL, PLTDSALF, PLTDSNIV, PLTSWALT, PLTDSALT, PLTDSTCO, PLTDSPLT,PLTDSHOR, PLTDSVER,PLTDSFLW, PLTSWVIS FROM METLPLT WITH(NOLOCK) where PLTINNOD=" & pltinnod, DS)
            If DS.Tables(0).Rows.Count > 0 Then
                trobat = True
                dbRow = DS.Tables(0).Rows(0)
...
Public Class clsPlantilla2
    ...
    Public Sub bdget(ByVal objconn As OleDbConnection, ByVal pltinnod As Integer)
        ...
        If pltinnod <> 0 Then
            GAIA.bdr(objconn, "SELECT * FROM METLPLT2 WITH(NOLOCK) where PLTINNOD=" & pltinnod, DS)
            If DS.Tables(0).Rows.Count > 0 Then
                trobat = True
                dbRow = DS.Tables(0).Rows(0)
                _innod = dbRow("PLTINNOD")
