...
<asp:literal id="lblDocs" runat="server"/>
<%
...
proves=0
	...
	IF clsPermisos.tepermis(objconn,3,idusuari,idusuari,  rel,0,usuariXarxa,"")=1 THEN			
		txtAfegirDocuments = "<a href=""/gaia/aspx/documents/carregaDocuments.aspx?" + HttpUtility.UrlEncode(oCodParam.encriptar("cr=" + codiRelacio.ToString())) + """ target=""_blank""><img src=""/img/common/afegir_documents.png"" border=""0""/></a>"
		txtAfegirEnllaços= "<a href=""/gaia/aspx/links/carregalinks.aspx?" + HttpUtility.UrlEncode(oCodParam.encriptar("cr=" + codiRelacio.ToString())) + """ target=""_blank""><img src=""/img/common/afegir_enllas.png"" border=""0""/></a>"
	END IF


	txtCercador="<a href=""/GAIA/aspx/buscador/buscador.aspx?"+HttpUtility.UrlEncode(oCodParam.encriptar("codiRelacioInicial=" + codiRelacio.tostring() ))+""" target=""_blank""><img src=""/img/common/cercar_documents.png"" border=""0""/></a>"
...
<link rel="stylesheet" href="/css/gaiaIntranet_1024.css" type="text/css"/>
...
<%
	...
	IF Request("nomollespa") is nothing THEN
		...
		if trim(cami) <> ""  then
			i=0
			aCami=split(cami,"|")							
			llistaCami+="<div class=""paddingsupinf10 paddingDretaEsquerra20 fonsBlauTurquesa"">"
			nouCami=""
			
			for each item in aCami
				if i > 0 then 
					llistaCami+="<a class=""blanc arialNarrow nodeco t12"">&nbsp;>&nbsp;</a>"	
				END IF
				...
				if item <> codiRelacio then
					llistaCami+="<a href=""/gaia/aspx/llibreriacodiweb/documents/visordocumentshtmlNew.aspx?codiRelacio=" & item & "&cami=" & nouCami 
					IF NOT (Request("mp") is nothing) THEN llistaCami+="&mp=1"
				  if NOT (Session("ed") is nothing) THEN llistaCami+="&ed=1"
						llistaCami+=  """ class=""blanc nodeco arialNarrow t075"">" + ds.tables(0).rows(0).item(0) + "</a>"
					else
						'actual
						nomCarpeta= ds.tables(0).rows(0).item(0)
						llistaCami+="<span class=""blanc arialNarrow nodeco t12"">" & nomCarpeta & "</span></div>"
					end if
...
'***************************************************************************	
	...
	For each dbRow in ds.tables(0).Rows		
			...
			llistaCarpetes+="<li style=""background:url(/img/common/iconografia/"+dbROW("TIPDSIMG")+") no-repeat left 2px;""><a href=""/gaia/aspx/llibreriacodiweb/documents/visordocumentshtmlNew.aspx?codiRelacio="+dbRow("RELINCOD").tostring()+"&cami=" & cami
			IF NOT (Request("mp") is nothing) THEN llistaCarpetes+="&mp=1"
			if NOT (Session("ed") is nothing) THEN llistaCarpetes+="&ed=1"
						
			llistaCarpetes+= """>"+dbRow("NODDSTXT")+"</a></li>"
	...
	IF strRelacionsAmbPermis.length>0 THEN
		...
		For each dbRow in ds.tables(0).Rows		
			...
			IF NOT dbROW("DOCDSFIT")="" andalso (instr(dbrow("DOCDSFIT"),".mp")>0  OR (instr(dbrow("DOCDSFIT"),".wm")>0)) THEN
				adreça = "<a href="""" onclick=""window.open('/gaia/aspx/documents/visorvideos.aspx?v="+dbRow("DOCDSFIT")+"','_blank','width=640,height=570');return false;"" target=""_blank"" style=""background:url(/img/common/iconografia/ico_video.png) no-repeat left 2px;"">"+dbRow("DOCDSTIT")+"</a>"
			ELSE
				...
				adreça = "<a href=""" & link & """ target=""_blank"" style=""background:url(/img/common/iconografia/"&dbROW("TDODSIMG")&") no-repeat left 2px;"">" & dbRow("DOCDSTIT") & "</a>"
			END IF
				
			
			if ambDataDescripcio THEN
				llistaDocuments &= "<li><div class=""displayBlock border1BottomDotted846540"">"
   				llistaDocuments &= "<ul><li><div class=""displayInlineBlock gris arial paddingDreta10 valignTop"">" & dbrow("DOCDTANY") & "</div><div class=""displayInlineBlock valignMiddle"">" & adreça & "<span class=""blauTurquesa cursiva displayBlock arial paddingTop5"">" & dbrow("DOCDSDES") & "</span></div></li></ul>"
				
			ELSE					
				llistaDocuments &= "<li><a href=""" + link + """ target=""_blank"" style=""background:url(/img/common/iconografia/" & dbROW("TDODSIMG") & ") no-repeat left 2px;"">"+dbRow("DOCDSTIT")+"</a></li>"
				...
				IF   not ( Session("ed") is nothing) THEN 
					IF NOT isdbNull(dbrow("RELINCOD")) THEN
						...
						IF clsPermisos.tepermis(objconn,3,idusuari,idusuari,  relTMP,0,usuariXarxa,"") THEN
							...
							llistaDocuments &= "<div class=""displayInlineBlock valignMiddle""><a href=""/gaia/aspx/documents/carregaDocuments.aspx?" & HttpUtility.UrlEncode(oCodParam.encriptar("id=" & dbRow("DOCINNOD").ToString() & "&idiarbre=" & dbRow("DOCINIDI").ToString())) & """ target=""_blank"" class=""editar"" style=""background:url(/img/common/boto_editarp.png) no-repeat top left;""></a></div>"
		...
		For each dbRow in ds.tables(0).Rows			
			...
			IF Instr(link, "_"+codiIdioma.tostring()+".")>0 OR isdbnull(dbrow("RELDSFIT"))  THEN			
				IF Instr(link,".asp") AND Instr(link,"id=")<0 THEN
					...
					END IF
				END IF				
				llistaDocuments+="<li><a href="""+link.REplace("GAIA/","")+""" target="""+target+""" style=""background:url(/img/common/iconografia/ico_web.png) no-repeat left 2px;"">"+dbROW("LNKDSTXT")+"</a>"	
					IF   not ( Session("ed") is nothing) THEN 
						IF NOT isdbNull(dbrow("relacio")) THEN
							relTMP.bdGet(objconn,dbrow("relacio"))
	
							IF clsPermisos.tepermis(objconn,3,idusuari,idusuari,  relTMP,0,usuariXarxa,"") THEN
								llistaDocuments +="<a href=""/gaia/aspx/links/carregalinks.aspx?" + HttpUtility.UrlEncode(oCodParam.encriptar("id=" + dbRow("LNKINNOD").ToString() + "&idiarbre=" + dbRow("LNKINIDI").ToString())) + """ target=""_blank"" class=""editar"" style=""background:url(/img/common/boto_editarp.png) no-repeat top left;""></a>"
							END IF
						END IF
					END IF
				llistaDocuments+="</li>"		
	...
	'**********************************************************************************************************
		...
		txtTitol+="<div class=""paddingEsquerra10 blanc arial t075 bold fonsBlauTurquesa mayusculas paddingsupinf3"">"+nomCarpeta+"</div>"		
		llistaCarpetes=""				
		contCarpeta=0
		...
		IF strRelacionsAmbPermis.length>0 THEN		
			...
			For each dbRow in ds.tables(0).Rows		
				...
				IF instr(dbrow("DOCDSFIT"),".mp")>0  OR (instr(dbrow("DOCDSFIT"),".wm")>0) THEN
					adreça = "<a href="""" style=""background:url(/img/common/iconografia/ico_video.png) no-repeat left 2px;"" onclick=""window.open('/gaia/aspx/documents/visorvideos.aspx?v="+dbRow("DOCDSFIT")+"','_blank','width=640,height=570');return false;"" class=""arial t075 negre valignMiddle bold floatleft"" target=""_blank"">"+dbRow("DOCDSTIT")+"</a>"
				ELSE
					link = "/GAIA/ASPX/llibreriaCodiWeb/webMunicipal/utilitats/obreFitxer/obreFitxer.aspx?"+HttpUtility.UrlEncode(oCodParam.encriptar("codiNode=" + dbrow("DOCINNOD").tostring() + "&codiIdioma=1"))
					adreça = "<a href=""" & link & """ class=""arial t075 negre bold"" style=""background:url(/img/common/iconografia/" & dbROW("TDODSIMG") & ") no-repeat left 2px;"" target=""_blank"">" & dbRow("DOCDSTIT") & "</a>"
				END IF
		...
		For each dbRow in ds.tables(0).Rows			
			...
			target=iif(isdbnull(dbrow("LNKWNTIP")),"_self",iif(dbrow("LNKWNTIP")=0,"_self","_blank"))
				IF Instr(link, "_"+codiIdioma.tostring()+".")>0 OR isdbnull(dbrow("RELDSFIT"))  THEN			
					...
					llistaDocuments+="<li><a href="""+link.REplace("GAIA/","")+""" target="""+target+""" style=""background:url(/img/common/iconografia/"+dbRow("TDODSIMG")+") no-repeat left 2px;"">"+dbROW("LNKDSTXT")+"</a></li>"			
	...
	Response.write("<div class=""contingutCapsaDocumentsASCX""><div class=""documentsASCX""><ul>")
